...
var mm = (e, t, n) => t in e ? fm(e, t, { enumerable: !0, configurable: !0, writable: !0, value: n }) : e[t] = n;
var ce = (e, t, n) => mm(e, typeof t != "symbol" ? t + "" : t, n), ri = (e, t, n) => t.has(e) || Tc("Cannot " + n);
var te = (e, t, n) => (ri(e, t, "read from private field"), n ? n.call(e) : t.get(e)), rt = (e, t, n) => t.has(e) ? Tc("Cannot add the same private member more than once") : t instanceof WeakSet ? t.add(e) : t.set(e, n), Fe = (e, t, n, r) => (ri(e, t, "write to private field"), r ? r.call(e, n) : t.set(e, n), n), yt = (e, t, n) => (ri(e, t, "access private method"), n);
import dp, { BrowserWindow as Mt, app as ze, nativeImage as hm, Tray as vm, Menu as ym, screen as gm, ipcMain as le, powerMonitor as xm, systemPreferences as Rc } from "electron";
import { fileURLToPath as bm } from "node:url";
import Z from "node:path";
import _m from "better-sqlite3";
import Nn from "child_process";
import De, { resolve as Oc } from "path";
import tt from "fs";
import wm from "constants";
import ya from "os";
import _r from "assert";
import fp, { EventEmitter as Em } from "events";
import pn from "util";
import he from "node:process";
import { promisify as Ae, isDeepStrictEqual as Ac } from "node:util";
import ne from "node:fs";
import Yn from "node:crypto";
import Pc from "node:assert";
import mp from "node:os";
import "node:events";
import "node:stream";
import Re, { Readable as $m } from "stream";
import Ms from "http";
import zs from "https";
import Aa from "url";
import hp from "crypto";
import Sm from "net";
import Tm from "tls";
import vp from "tty";
import yp from "http2";
import Dt from "zlib";
import kc from "node:fs/promises";
import { createRequire as Rm } from "node:module";
let _e = null, at = null, Jn = null;
function gp(e) {
  return _e = new Mt({
    width: 550,
    height: 730,
    ...
    webPreferences: {
      preload: Z.join(e.dirname, "preload.cjs"),
      contextIsolation: !0,
      nodeIntegration: !1,
      sandbox: !1
    }
  }), e.isDev && e.viteDevServerUrl ? (_e.loadURL(e.viteDevServerUrl), _e.webContents.openDevTools()) : _e.loadFile(Z.join(e.dirname, "../renderer/index.html")), _e.on("close", (t) => {
    ze.isQuitting || (t.preventDefault(), _e == null || _e.hide());
  }), _e;
}
function Om(e) {
  const t = hm.createFromPath(
    Z.join(e, "../../resources/icon.png")
  );
  Jn = new vm(t);
  const n = ym.buildFromTemplate([
    {
      label: "Open Tracker",
      click: () => {
        _e == null || _e.show();
    ...
    },
    {
      label: "Quit",
      click: () => {
        ze.isQuitting = !0, ze.quit();
      }
    }
  ]);
  return Jn.setToolTip("ERPNext Time Tracker"), Jn.setContextMenu(n), Jn.on("double-click", () => {
    _e == null || _e.show();
  }), Jn;
}
function Am(e) {
...
function bp(e) {
  ...
  if (at = new Mt({
    ...
    t.searchParams.set("window", "config"), at.loadURL(t.toString());
  } else
    at.loadFile(Z.join(e.dirname, "../renderer/index.html"), {
      query: {
        window: "config"
      }
    });
...
function km(e) {
  const n = e.prepare("PRAGMA table_info(activity_media)").all().find((o) => o.name === "activity_id");
  if (!n || n.notnull === 0)
    return;
  const r = jr(e, "activity_media", "session_id", "''"), a = jr(e, "activity_media", "approved", "1"), i = jr(e, "activity_media", "rejected", "0"), s = jr(e, "activity_media", "camera_id", "''");
...
function Nm(e) {
  return {
    ...
    screenshotPaths: Nc(e.screenshot_paths),
    camshotPaths: Nc(e.camshot_paths),
    synced: e.synced === 1,
...
function Be() {
  if (ve)
    return ve;
  const e = Z.join(ze.getPath("userData"), "time-tracker.sqlite");
  return console.log("Database path:", e), ve = new _m(e), ve.exec(`
  ...
  `), Ye(ve, "activities", "screenshot_paths TEXT NOT NULL DEFAULT '[]'"), Ye(ve, "activities", "camshot_paths TEXT NOT NULL DEFAULT '[]'"), Ye(ve, "activities", "synced INTEGER NOT NULL DEFAULT 0"), Ye(ve, "activities", "sync_status TEXT NOT NULL DEFAULT 'pending'"), Ye(ve, "activities", "remote_id TEXT NOT NULL DEFAULT ''"), Ye(ve, "activities", "uploaded_at TEXT NOT NULL DEFAULT ''"), Ye(ve, "activities", "synced_at TEXT NOT NULL DEFAULT ''"), Ye(ve, "activities", "upload_error TEXT NOT NULL DEFAULT ''"), ve.exec(`
  ...
  `), km(ve), Ye(ve, "activity_media", "session_id TEXT NOT NULL DEFAULT ''"), Ye(ve, "activity_media", "approved INTEGER NOT NULL DEFAULT 1"), Ye(ve, "activity_media", "rejected INTEGER NOT NULL DEFAULT 0"), Ye(ve, "activity_media", "camera_id TEXT NOT NULL DEFAULT ''"), ve.exec(`
...
function Cm(e) {
  const t = e.createdAt || (/* @__PURE__ */ new Date()).toISOString(), n = e.sessionId || "", r = e.screenshotPaths || [], a = e.camshotPaths || [], i = Be().prepare(
    `
    ...
    JSON.stringify(r),
    JSON.stringify(a)
  ), s = Number(i.lastInsertRowid), o = Be().prepare(
    `
      INSERT INTO activity_media (
        activity_id,
        media_type,
...
function Bm(e = "screenshot", t = 100) {
...
function Vm() {
  if (Cc) return ii;
  Cc = 1;
  const e = Nn.exec, t = De, n = qs().defaultAll, r = `Screen 0: minimum 320 x 200, current 5760 x 1080, maximum 8192 x 8192
  ...
  function l(u = {}) {
    return new Promise((c, p) => {
      i().then((f) => {
        ...
        Nn.execFile(y, _, d, (x, w) => x ? p(x) : c(u.filename ? t.resolve(v) : w));
      });
    });
  }
  return l.listDisplays = i, l.parseDisplaysOutput = a, l.EXAMPLE_DISPLAYS_OUTPUT = r, l.all = () => n(l), ii = l, ii;
}
var Cr = { exports: {} }, Lr = {}, Lc;
function Hm() {
  if (Lc) return Lr;
  Lc = 1;
  var e = De, t = process.platform === "win32", n = tt, r = process.env.NODE_DEBUG && /fs/.test(process.env.NODE_DEBUG);
  function a() {
    var l;
    if (r) {
      var u = new Error();
  ...
  if (t)
    ...
    var o = /^[\/]*/;
  return Lr.realpathSync = function(u, c) {
    ...
    for (; g < u.length; ) {
      ...
      if (d = m, m += _[0], v = d + _[1], g = s.lastIndex, !(h[v] || c && c[v] === v)) {
        ...
        if (c && Object.prototype.hasOwnProperty.call(c, v))
          ...
          var I = null;
          if (!t) {
            var C = w.dev.toString(32) + ":" + w.ino.toString(32);
    ...
    function x() {
      ...
      s.lastIndex = m;
      var D = s.exec(u);
      return y = v, v += D[0], d = y + D[1], m = s.lastIndex, g[d] || c && c[d] === d ? process.nextTick(x) : c && Object.prototype.hasOwnProperty.call(c, d) ? C(c[d]) : n.lstat(d, w);
    }
    function w(D, S) {
      if (D) return p(D);
      if (!S.isSymbolicLink())
...
function Ep() {
  if (Dc) return si;
  Dc = 1, si = o, o.realpath = o, o.sync = l, o.realpathSync = l, o.monkeypatch = u, o.unmonkeypatch = c;
  var e = tt, t = e.realpath, n = e.realpathSync, r = process.version, a = /^v[0-5]\./.test(r), i = Hm();
  function s(p) {
...
function Xm() {
  ...
  function c(d) {
    ...
    if (!_)
      return d.split(",");
    var x = _.pre, w = _.body, I = _.post, C = x.split(",");
    C[C.length - 1] += "{" + w + "}";
    var D = c(I);
  ...
  function v(d, y, _) {
    var x = [], w = t("{", "}", d);
    if (!w || /\$$/.test(w.pre)) return [d];
    var I = /^-?\d+\.\.-?\d+(?:\.\.-?\d+)?$/.test(w.body), C = /^[a-zA-Z]\.\.[a-zA-Z](?:\.\.-?\d+)?$/.test(w.body), D = I || C, S = w.body.indexOf(",") >= 0;
    if (!D && !S)
      return w.post.match(/,(?!,).*\}/) ? (d = w.pre + "{" + w.body + a + w.post, v(d, y, !0)) : [d];
    var R;
    if (D)
      R = w.body.split(/\.\./);
    else if (R = c(w.body), R.length === 1 && (R = v(R[0], y, !1).map(f), R.length === 1)) {
      var k = w.post.length ? v(w.post, y, !1) : [""];
      return k.map(function(X) {
        return w.pre + R[0] + X;
      });
    }
    var O = w.pre, k = w.post.length ? v(w.post, y, !1) : [""], V;
    if (D) {
...
function Bs() {
  ...
  h.sep = e.sep;
  var t = h.GLOBSTAR = g.GLOBSTAR = {}, n = Xm(), r = {
    "!": { open: "(?:(?!(?:", close: "))[^/]*?)" },
    "?": { open: "(?:", close: ")?" },
    "+": { open: "(?:", close: ")+" },
    "*": { open: "(?:", close: ")*" },
  ...
  function w(S, R) {
    ...
    for (var b = 0, E = S.length, M; b < E && (M = S.charAt(b)); b++) {
    ...
    for (U && (z = S.substr(L + 1), X = this.parse(z, x), k = k.substr(0, B) + "\\[" + X[0], V = V || X[1]), q = F.pop(); q; q = F.pop()) {
      ...
      this.debug("setting tail", k, q), W = W.replace(/((?:\\{2}){0,64})(\\?)\|/g, function(vn, St, Bt) {
        return Bt || (Bt = "\\"), St + St + Bt + "|";
      }), this.debug(`tail=%j
   %s`, W, W, q, k);
    ...
    for (var de = H.length - 1; de > -1; de--) {
      var ie = H[de], He = k.slice(0, ie.reStart), nt = k.slice(ie.reStart, ie.reEnd - 8), fe = k.slice(ie.reEnd - 8, ie.reEnd), We = k.slice(ie.reEnd);
      fe += We;
      var Kn = He.split("(").length - 1, zt = We;
      for (b = 0; b < Kn; b++)
        zt = zt.replace(/\)[+*?]?/, "");
      We = zt;
      var hn = "";
      We === "" && R !== x && (hn = "$");
      var Xn = He + nt + We + hn + fe;
      k = Xn;
    }
    ...
    var Wn = O.nocase ? "i" : "";
    try {
      var qt = new RegExp("^" + k + "$", Wn);
    } catch {
      return new RegExp("$.");
    }
    return qt._glob = S, qt._src = k, qt;
  }
  ...
  h.match = function(S, R, O) {
    ...
    for (var E = [[[], 0]], M = E[0], q = 0, z = [0], X = 0; X < U.length; X++) {
      var W = U[X];
      W === t ? (z.push(q), M = [[], 0], E.push(M)) : (M[0].push(W), q++);
    }
    for (var J = E.length - 1, ue = S.length - $, de = 0; de < E.length; de++)
      E[de][1] = ue - (z[J--] + E[de][0].length);
...
var Dr = { exports: {} }, Fr = { exports: {} }, qc;
function Wm() {
  return qc || (qc = 1, typeof Object.create == "function" ? Fr.exports = function(t, n) {
...
var Bc;
function Ym() {
  if (Bc) return Dr.exports;
  Bc = 1;
  try {
    var e = require("util");
    if (typeof e.inherits != "function") throw "";
    Dr.exports = e.inherits;
  } catch {
    Dr.exports = Wm();
  }
  return Dr.exports;
...
function Vs() {
  ...
  return Qn.exports = process.platform === "win32" ? t : e, Qn.exports.posix = e, Qn.exports.win32 = t, Qn.exports;
}
var gt = {}, Hc;
function $p() {
  if (Hc) return gt;
  Hc = 1, gt.setopts = u, gt.ownProp = e, gt.makeAbs = f, gt.finish = c, gt.mark = p, gt.isIgnored = h, gt.childrenIgnored = g;
  function e(m, v) {
    return Object.prototype.hasOwnProperty.call(m, v);
  }
  var t = tt, n = De, r = Bs(), a = Vs(), i = r.Minimatch;
  function s(m, v) {
  ...
  function p(m, v) {
    ...
    if (y) {
      var x = y === "DIR" || Array.isArray(y), w = v.slice(-1) === "/";
      if (x && !w ? _ += "/" : !x && w && (_ = _.slice(0, -1)), _ !== v) {
        var I = f(m, _);
...
function Jm() {
  if (Gc) return pi;
  Gc = 1, pi = c, c.GlobSync = p;
  var e = Ep(), t = Bs();
  t.Minimatch, Rp().Glob;
  var n = De, r = _r, a = Vs(), i = $p(), s = i.setopts, o = i.ownProp, l = i.childrenIgnored, u = i.isIgnored;
  function c(f, h) {
...
See: https://github.com/isaacs/node-glob/issues/167`);
  ...
  return p.prototype._finish = function() {
    ...
    var d = f.slice(m), y;
    v === null ? y = "." : ((a(v) || a(f.map(function(w) {
    ...
    if (y) {
      for (var _ = m[0], x = !!this.minimatch.negate, w = _._glob, I = this.dot || w.charAt(0) === ".", C = [], D = 0; D < y.length; D++) {
        var S = y[D];
        if (S.charAt(0) !== "." || I) {
          var R;
    ...
    var y = this._readdir(g, d);
    if (y) {
      var _ = m.slice(1), x = f ? [f] : [], w = x.concat(_);
      this._process(w, v, !1);
...
var Ur = { exports: {} }, Xc;
function Tp() {
  if (Xc) return Ur.exports;
  Xc = 1;
  var e = Sp();
  Ur.exports = e(t), Ur.exports.strict = e(n), t.proto = t(function() {
...
function Qm() {
  if (Wc) return fi;
  Wc = 1;
  var e = Sp(), t = /* @__PURE__ */ Object.create(null), n = Tp();
  fi = e(r);
  function r(s, o) {
...
function Rp() {
  ...
  var e = Ep(), t = Bs();
  t.Minimatch;
  var n = Ym(), r = fp.EventEmitter, a = De, i = _r, s = Vs(), o = Jm(), l = $p(), u = l.setopts, c = l.ownProp, p = Qm(), f = l.childrenIgnored, h = l.isIgnored, g = Tp();
  function m(x, w, I) {
    if (typeof w == "function" && (I = w, w = {}), w || (w = {}), w.sync) {
      if (I)
        throw new TypeError("callback provided to sync glob");
      return o(x, w);
    }
  ...
  function d(x, w) {
    if (w === null || typeof w != "object")
      return x;
    for (var I = Object.keys(w), C = I.length; C--; )
      x[I[C]] = w[I[C]];
    return x;
  }
  m.hasMagic = function(x, w) {
    var I = d({}, w);
    I.noprocess = !0;
  ...
  function y(x, w, I) {
    if (typeof w == "function" && (I = w, w = null), w && w.sync) {
      if (I)
        throw new TypeError("callback provided to sync glob");
      return new v(x, w);
    }
    if (!(this instanceof y))
      return new y(x, w, I);
    u(this, x, w), this._didRealPath = !1;
  ...
  y.prototype._finish = function() {
    ...
    if (x === 0)
      return this._finish();
    for (var w = this, I = 0; I < this.matches.length; I++)
      this._realpathSet(I, C);
    function C() {
      --x === 0 && w._finish();
    }
  }, y.prototype._realpathSet = function(x, w) {
    ...
    C.forEach(function(O, k) {
      O = D._makeAbs(O), e.realpath(O, D.realpathCache, function(V, P) {
        V ? V.syscall === "stat" ? R[O] = !0 : D.emit("error", V) : R[P] = !0, --S === 0 && (D.matches[x] = R, w());
      });
    });
  ...
  }, y.prototype._process = function(x, w, I, C) {
    if (i(this instanceof y), i(typeof C == "function"), !this.aborted) {
      if (this._processing++, this.paused) {
        this._processQueue.push([x, w, I, C]);
        return;
      }
      ...
      switch (D) {
        case x.length:
          this._processSimple(x.join("/"), w, C);
          return;
        case 0:
          S = null;
          break;
      ...
      V ? this._processGlobStar(S, O, k, R, w, I, C) : this._processReaddir(S, O, k, R, w, I, C);
    }
  }, y.prototype._processReaddir = function(x, w, I, C, D, S, R) {
    var O = this;
    this._readdir(I, S, function(k, V) {
      return O._processReaddir2(x, w, I, C, D, S, V, R);
    });
  }, y.prototype._processReaddir2 = function(x, w, I, C, D, S, R, O) {
    if (!R)
      return O();
    ...
    O();
  }, y.prototype._emitMatch = function(x, w) {
    if (!this.aborted && !h(this, w)) {
      if (this.paused) {
        this._emitQueue.push([x, w]);
        return;
      }
      var I = s(w) ? w : this._makeAbs(w);
      if (this.mark && (w = this._mark(w)), this.absolute && (w = I), !this.matches[x][w]) {
        if (this.nodir) {
          var C = this.cache[I];
        ...
        D && this.emit("stat", w, D), this.emit("match", w);
      }
    }
  }, y.prototype._readdirInGlobStar = function(x, w) {
    if (this.aborted)
      return;
    if (this.follow)
      return this._readdir(x, !1, w);
    ...
    function S(R, O) {
      ...
      C.symlinks[x] = k, !k && O && !O.isDirectory() ? (C.cache[x] = "FILE", w()) : C._readdir(x, !1, w);
    }
  }, y.prototype._readdir = function(x, w, I) {
    if (!this.aborted && (I = p("readdir\0" + x + "\0" + w, I), !!I)) {
      if (w && !c(this.symlinks, x))
  ...
  function _(x, w, I) {
    return function(C, D) {
      C ? x._readdirError(w, C, I) : x._readdirEntries(w, D, I);
    };
  }
  return y.prototype._readdirEntries = function(x, w, I) {
    if (!this.aborted) {
      if (!this.mark && !this.stat)
      ...
      return this.cache[x] = w, I(null, w);
    }
  }, y.prototype._readdirError = function(x, w, I) {
    if (!this.aborted) {
      switch (w.code) {
        case "ENOTSUP":
        case "ENOTDIR":
          ...
          if (this.cache[C] = "FILE", C === this.cwdAbs) {
            var D = new Error(w.code + " invalid cwd " + this.cwd);
            D.path = this.cwd, D.code = w.code, this.emit("error", D), this.abort();
          }
          break;
        case "ENOENT":
        case "ELOOP":
        ...
        case "UNKNOWN":
          ...
          break;
        default:
          this.cache[this._makeAbs(x)] = !1, this.strict && (this.emit("error", w), this.abort()), this.silent || console.error("glob error", w);
          break;
      }
      return I();
    }
  }, y.prototype._processGlobStar = function(x, w, I, C, D, S, R) {
    var O = this;
    this._readdir(I, S, function(k, V) {
      O._processGlobStar2(x, w, I, C, D, S, V, R);
    });
  }, y.prototype._processGlobStar2 = function(x, w, I, C, D, S, R, O) {
    if (!R)
      return O();
    ...
    O();
  }, y.prototype._processSimple = function(x, w, I) {
    var C = this;
    this._stat(x, function(D, S) {
      C._processSimple2(x, w, D, S, I);
    });
  }, y.prototype._processSimple2 = function(x, w, I, C, D) {
    if (this.matches[w] || (this.matches[w] = /* @__PURE__ */ Object.create(null)), !C)
      return D();
    if (x && s(x) && !this.nomount) {
      var S = /[\/\\]$/.test(x);
      x.charAt(0) === "/" ? x = a.join(this.root, x) : (x = a.resolve(this.root, x), S && (x += "/"));
    }
    process.platform === "win32" && (x = x.replace(/\\/g, "/")), this._emitMatch(w, x), D();
  }, y.prototype._stat = function(x, w) {
    ...
    function V(P, F) {
      if (F && F.isSymbolicLink())
        return O.fs.stat(I, function(H, j) {
          H ? O._stat2(x, I, null, F, w) : O._stat2(x, I, H, j, w);
        });
      O._stat2(x, I, P, F, w);
    }
  }, y.prototype._stat2 = function(x, w, I, C, D) {
    if (I && (I.code === "ENOENT" || I.code === "ENOTDIR"))
      return this.statCache[w] = !1, D();
    var S = x.slice(-1) === "/";
    if (this.statCache[w] = C, w.slice(-1) === "/" && C && !C.isDirectory())
      return D(null, !1, C);
    var R = !0;
    return C && (R = C.isDirectory() ? "DIR" : "FILE"), this.cache[w] = this.cache[w] || R, S && R === "FILE" ? D() : D(null, R, C);
  }, mi;
}
var hi, Jc;
function Zm() {
  if (Jc) return hi;
  Jc = 1, hi = u, u.sync = m;
  var e = _r, t = De, n = tt, r = Rp(), a = parseInt("666", 8), i = {
    nosort: !0,
    silent: !0
  ...
  function u(y, _, x) {
    ...
    function S(R, O) {
      ...
      O.forEach(function(k) {
        c(k, _, function V(P) {
          if (P) {
            if ((P.code === "EBUSY" || P.code === "ENOTEMPTY" || P.code === "EPERM") && w < _.maxBusyTries) {
              w++;
              var F = w * 100;
  ...
  function c(y, _, x) {
    e(y), e(_), e(typeof x == "function"), _.lstat(y, function(w, I) {
      if (w && w.code === "ENOENT")
        return x(null);
      if (w && w.code === "EPERM" && o && p(y, _, w, x), I && I.isDirectory())
        return h(y, _, w, x);
  ...
  function p(y, _, x, w) {
    e(y), e(_), e(typeof w == "function"), x && e(x instanceof Error), _.chmod(y, a, function(I) {
      I ? w(I.code === "ENOENT" ? null : x) : _.stat(y, function(C, D) {
        C ? w(C.code === "ENOENT" ? null : x) : D.isDirectory() ? h(y, _, x, w) : _.unlink(y, w);
      });
    });
  }
  function f(y, _, x) {
    ...
    w.isDirectory() ? v(y, _, x) : _.unlinkSync(y);
  }
  function h(y, _, x, w) {
    e(y), e(_), x && e(x instanceof Error), e(typeof w == "function"), _.rmdir(y, function(I) {
      I && (I.code === "ENOTEMPTY" || I.code === "EEXIST" || I.code === "EPERM") ? g(y, _, w) : I && I.code === "ENOTDIR" ? w(x) : w(I);
    });
  }
  function g(y, _, x) {
    e(y), e(_), e(typeof x == "function"), _.readdir(y, function(w, I) {
      if (w)
        return x(w);
      var C = I.length;
      if (C === 0)
  ...
  function v(y, _, x) {
    ...
    try {
      ...
      if (w.code === "ENOTDIR")
        throw x;
      (w.code === "ENOTEMPTY" || w.code === "EEXIST" || w.code === "EPERM") && d(y, _);
    }
  }
  function d(y, _) {
...
function eh() {
  if (Qc) return vi;
  Qc = 1;
  var e = De, t = tt, n = parseInt("0777", 8);
  vi = r.mkdirp = r.mkdirP = r;
  function r(a, i, s, o) {
...
function Op() {
  return Zc || (Zc = 1, function(e, t) {
    let n = tt, r = De, a = wm, i = ya, s = Zm(), o = eh();
    ya.tmpdir();
    const l = s.sync;
...
function th() {
  ...
  function o(v = {}) {
    const d = (y) => new Promise((_, x) => {
      ...
      const P = ["-x", "-t", S, ...V];
      Nn.execFile("screencapture", P, function(F, H) {
        if (F)
          return x(F);
...
`;
  ...
  function h(v) {
    ...
    let _ = [];
    return y.forEach((x) => {
      const w = d["Graphics/Displays"][x];
      if (w.Displays) {
        const I = Object.entries(w.Displays).map(([C, D]) => {
  ...
  function m() {
    return new Promise((v, d) => {
      g().then((y) => {
        ...
        e("screencapture -x -t jpg " + _.join(" "), function(x, w) {
          if (x)
            return d(x);
  ...
  return o.listDisplays = g, o.all = m, o.parseDisplaysOutput = h, o.EXAMPLE_DISPLAYS_OUTPUT = l, yi = o, yi;
}
var gi, tl;
function nh() {
  if (tl) return gi;
  tl = 1;
  const e = Nn.exec, t = Op(), n = De, r = qs(), a = tt, i = ya, {
    readAndUnlinkP: s,
    defaultAll: o
  } = r;
  function l() {
  ...
  function u(h = {}) {
    return new Promise((g, m) => {
      ...
      if (h.filename) {
        ...
        y = t.path({ suffix: `.${d}` });
      const x = h.screen ? String(h.screen) : "", w = l(), I = [y];
      x && I.push("/d", x);
      const C = ["/c", w, ...I];
      Nn.execFile("cmd.exe", C, {
...
`).map((d) => d.replace(/[\n\r]/g, "")).map((d) => d.match(/(.*?);(.?\d+);(.?\d+);(.?\d+);(.?\d+);(.?\d*[\.,]?\d+)/)).filter((d) => d).map((d) => ({
  ...
  return u.listDisplays = f, u.availableDisplays = f, u.parseDisplaysOutput = p, u.EXAMPLE_DISPLAYS_OUTPUT = c, u.all = () => o(u), gi = u, gi;
}
process.platform === "linux" ? cr.exports = Vm() : process.platform === "darwin" ? cr.exports = th() : process.platform === "win32" ? cr.exports = nh() : cr.exports = function() {
  return Promise.reject(new Error("Currently unsupported platform. Pull requests welcome!"));
};
var rh = cr.exports;
const xi = /* @__PURE__ */ Un(rh), rn = (e) => {
  const t = typeof e;
...
function kp(e) {
  ...
  if (/^[1-9]\d*$/.test(e)) {
    ...
    return t <= Number.MAX_SAFE_INTEGER && t <= Pp;
  }
  return !1;
}
function bi(e, t) {
  return Ap.has(e) ? !1 : (e && kp(e) ? t.push(Number.parseInt(e, 10)) : t.push(e), !0);
}
function ih(e) {
  if (typeof e != "string")
      ...
      case "]": {
        if (r === "index") {
          if (n === "")
            ...
            !Number.isNaN(o) && Number.isFinite(o) && o >= 0 && o <= Number.MAX_SAFE_INTEGER && o <= Pp && n === String(o) ? t.push(o) : t.push(n), n = "", r = "indexEnd";
          }
          break;
        }
        if (r === "indexEnd")
...
function Pa(e) {
  ...
  if (Array.isArray(e)) {
    ...
    for (const [n, r] of e.entries()) {
      ...
      if (Ap.has(r))
        return [];
      typeof r == "string" && kp(r) ? t.push(Number.parseInt(r, 10)) : t.push(r);
    }
    return t;
  }
  return [];
...
function nl(e, t, n) {
  if (!rn(e) || typeof t != "string" && !Array.isArray(t))
    return n === void 0 ? e : n;
  const r = Pa(t);
  if (r.length === 0)
    return n;
...
const It = mp.homedir(), Hs = mp.tmpdir(), { env: $n } = he, oh = (e) => {
  const t = Z.join(It, "Library");
  return {
  ...
  const t = $n.APPDATA || Z.join(It, "AppData", "Roaming"), n = $n.LOCALAPPDATA || Z.join(It, "AppData", "Local");
  return {
    // Data/config/cache/log are invented by me as Windows isn't opinionated about this
    data: Z.join(n, e, "Data"),
    config: Z.join(t, e, "Config"),
    cache: Z.join(n, e, "Cache"),
    log: Z.join(n, e, "Log"),
    temp: Z.join(Hs, e)
  };
}, lh = (e) => {
  const t = Z.basename(It);
  return {
    data: Z.join($n.XDG_DATA_HOME || Z.join(It, ".local", "share"), e),
    config: Z.join($n.XDG_CONFIG_HOME || Z.join(It, ".config"), e),
    cache: Z.join($n.XDG_CACHE_HOME || Z.join(It, ".cache"), e),
    // https://wiki.debian.org/XDGBaseDirectorySpecification#state
    log: Z.join($n.XDG_STATE_HOME || Z.join(It, ".local", "state"), e),
    temp: Z.join(Hs, t, e)
  };
};
function uh(e, { suffix: t = "nodejs" } = {}) {
  if (typeof e != "string")
    throw new TypeError(`Expected a string, got ${typeof e}`);
  return t && (e += `-${t}`), he.platform === "darwin" ? oh(e) : he.platform === "win32" ? ch(e) : lh(e);
}
const Tt = (e, t) => {
  const { onError: n } = t;
  return function(...a) {
  ...
  return function(a) {
    const { timeout: i } = a, s = a.interval ?? ph, o = Date.now() + i;
    return function l(...u) {
  ...
  isChangeErrorOk: (e) => {
    if (!Sn.isNodeError(e))
      return !1;
    const { code: t } = e;
    return t === "ENOSYS" || !dh && (t === "EINVAL" || t === "EPERM");
  },
  ...
  onChangeError: (e) => {
    if (!Sn.isNodeError(e))
      throw e;
    if (!Sn.isChangeErrorOk(e))
      throw e;
  }
}, zr = {
  onError: Sn.onChangeError
}, Ge = {
  onError: () => {
  }
}, dh = he.getuid ? !he.getuid() : !1, Pe = {
  isRetriable: Sn.isRetriableError
}, Ne = {
  attempt: {
    /* ASYNC */
    chmod: Tt(Ae(ne.chmod), zr),
    chown: Tt(Ae(ne.chown), zr),
    close: Tt(Ae(ne.close), Ge),
    fsync: Tt(Ae(ne.fsync), Ge),
    mkdir: Tt(Ae(ne.mkdir), Ge),
    realpath: Tt(Ae(ne.realpath), Ge),
    stat: Tt(Ae(ne.stat), Ge),
    unlink: Tt(Ae(ne.unlink), Ge),
    /* SYNC */
    chmodSync: xt(ne.chmodSync, zr),
    chownSync: xt(ne.chownSync, zr),
    closeSync: xt(ne.closeSync, Ge),
    existsSync: xt(ne.existsSync, Ge),
    fsyncSync: xt(ne.fsync, Ge),
    mkdirSync: xt(ne.mkdirSync, Ge),
    realpathSync: xt(ne.realpathSync, Ge),
    statSync: xt(ne.statSync, Ge),
    unlinkSync: xt(ne.unlinkSync, Ge)
  },
  retry: {
    /* ASYNC */
    close: Rt(Ae(ne.close), Pe),
    fsync: Rt(Ae(ne.fsync), Pe),
    open: Rt(Ae(ne.open), Pe),
    readFile: Rt(Ae(ne.readFile), Pe),
    rename: Rt(Ae(ne.rename), Pe),
    stat: Rt(Ae(ne.stat), Pe),
    write: Rt(Ae(ne.write), Pe),
    writeFile: Rt(Ae(ne.writeFile), Pe),
    /* SYNC */
    closeSync: Ot(ne.closeSync, Pe),
    fsyncSync: Ot(ne.fsyncSync, Pe),
    openSync: Ot(ne.openSync, Pe),
    readFileSync: Ot(ne.readFileSync, Pe),
    renameSync: Ot(ne.renameSync, Pe),
    statSync: Ot(ne.statSync, Pe),
    writeSync: Ot(ne.writeSync, Pe),
    writeFileSync: Ot(ne.writeFileSync, Pe)
  }
}, fh = "utf8", rl = 438, mh = 511, hh = {}, vh = he.geteuid ? he.geteuid() : -1, yh = he.getegid ? he.getegid() : -1, gh = 1e3, xh = !!he.getuid;
he.getuid && he.getuid();
const al = 128, bh = (e) => e instanceof Error && "code" in e, il = (e) => typeof e == "string", wi = (e) => e === void 0, _h = he.platform === "linux", Np = he.platform === "win32", Gs = ["SIGHUP", "SIGINT", "SIGTERM"];
Np || Gs.push("SIGALRM", "SIGABRT", "SIGVTALRM", "SIGXCPU", "SIGXFSZ", "SIGUSR2", "SIGTRAP", "SIGSYS", "SIGQUIT", "SIGIOT");
_h && Gs.push("SIGIO", "SIGPOLL", "SIGPWR", "SIGSTKFLT");
class wh {
  /* CONSTRUCTOR */
  constructor() {
    this.callbacks = /* @__PURE__ */ new Set(), this.exited = !1, this.exit = (t) => {
      if (!this.exited) {
        ...
        for (const n of this.callbacks)
          n();
        t && (Np && t !== "SIGINT" && t !== "SIGTERM" && t !== "SIGKILL" ? he.kill(he.pid, "SIGTERM") : he.kill(he.pid, t));
      }
    }, this.hook = () => {
...
const Eh = new wh(), $h = Eh.register, Ie = {
  /* VARIABLES */
  store: {},
  // filePath => purge
  /* API */
  ...
  get: (e, t, n = !0) => {
    const r = Ie.truncate(t(e));
    return r in Ie.store ? Ie.get(e, t, n) : (Ie.store[r] = n, [r, () => delete Ie.store[r]]);
  },
  purge: (e) => {
    Ie.store[e] && (delete Ie.store[e], Ne.attempt.unlink(e));
  },
  purgeSync: (e) => {
    Ie.store[e] && (delete Ie.store[e], Ne.attempt.unlinkSync(e));
  },
  purgeSyncAll: () => {
...
$h(Ie.purgeSyncAll);
function Ip(e, t, n = hh) {
  if (il(n))
    return Ip(e, t, { encoding: n });
  const a = { timeout: n.timeout ?? gh };
  let i = null, s = null, o = null;
  try {
    const l = Ne.attempt.realpathSync(e), u = !!l;
    e = l || e, [s, i] = Ie.get(e, n.tmpCreate || Ie.create, n.tmpPurge !== !1);
    const c = xh && wi(n.chown), p = wi(n.mode);
    if (u && (c || p)) {
      const f = Ne.attempt.statSync(e);
      f && (n = { ...n }, c && (n.chown = { uid: f.uid, gid: f.gid }), p && (n.mode = f.mode));
    }
    if (!u) {
      const f = Z.dirname(e);
      Ne.attempt.mkdirSync(f, {
        mode: mh,
        recursive: !0
      });
    }
    o = Ne.retry.openSync(a)(s, "w", n.mode || rl), n.tmpCreated && n.tmpCreated(s), il(t) ? Ne.retry.writeSync(a)(o, t, 0, n.encoding || fh) : wi(t) || Ne.retry.writeSync(a)(o, t, 0, t.length, 0), n.fsync !== !1 && (n.fsyncWait !== !1 ? Ne.retry.fsyncSync(a)(o) : Ne.attempt.fsync(o)), Ne.retry.closeSync(a)(o), o = null, n.chown && (n.chown.uid !== vh || n.chown.gid !== yh) && Ne.attempt.chownSync(s, n.chown.uid, n.chown.gid), n.mode && n.mode !== rl && Ne.attempt.chmodSync(s, n.mode);
    try {
      Ne.retry.renameSync(a)(s, e);
    } catch (f) {
      if (!bh(f) || f.code !== "ENAMETOOLONG")
        throw f;
      Ne.retry.renameSync(a)(s, Ie.truncate(e));
    }
    i(), s = null;
  } finally {
    o && Ne.attempt.closeSync(o), s && Ie.purge(s);
  }
}
var ms = { exports: {} }, Ks = {}, et = {}, jn = {}, wr = {}, ee = {}, vr = {};
(function(e) {
...
(function(e) {
  ...
  class s extends i {
    constructor(b, E, M) {
      super(), this.varKind = b, this.name = E, this.rhs = M;
    }
    render({ es5: b, _n: E }) {
      const M = b ? n.varKinds.var : this.varKind, q = this.rhs === void 0 ? "" : ` = ${this.rhs}`;
      return `${M} ${this.name}${q};` + E;
    }
    optimizeNames(b, E) {
      if (b[this.name.str])
        return this.rhs && (this.rhs = P(this.rhs, b, E)), this;
    }
    get names() {
  ...
  class o extends i {
    constructor(b, E, M) {
      super(), this.lhs = b, this.rhs = E, this.sideEffects = M;
    }
    render({ _n: b }) {
    ...
    optimizeNames(b, E) {
      if (!(this.lhs instanceof t.Name && !b[this.lhs.str] && !this.sideEffects))
        return this.rhs = P(this.rhs, b, E), this;
    }
    get names() {
  ...
  class f extends i {
    ...
    optimizeNames(b, E) {
      return this.code = P(this.code, b, E), this;
    }
    get names() {
  ...
  class h extends i {
    ...
    render(b) {
      return this.nodes.reduce((E, M) => E + M.render(b), "");
    }
    optimizeNodes() {
      const { nodes: b } = this;
      let E = b.length;
      for (; E--; ) {
        const M = b[E].optimizeNodes();
        Array.isArray(M) ? b.splice(E, 1, ...M) : M ? b[E] = M : b.splice(E, 1);
      }
    ...
    optimizeNames(b, E) {
      ...
      for (; q--; ) {
        const z = M[q];
        z.optimizeNames(b, E) || (F(b, z.names), M.splice(q, 1));
      }
      return M.length > 0 ? this : void 0;
    }
    get names() {
      return this.nodes.reduce((b, E) => k(b, E.names), {});
    }
  }
  class g extends h {
    render(b) {
  ...
  class d extends g {
    ...
    optimizeNodes() {
      ...
      let E = this.else;
      if (E) {
        const M = E.optimizeNodes();
        E = this.else = Array.isArray(M) ? new v(M) : M;
      }
      if (E)
        return b === !1 ? E instanceof d ? E : E.nodes : this.nodes.length ? this : new d(H(b), E instanceof d ? [E] : E.nodes);
      if (!(b === !1 || !this.nodes.length))
        return this;
    }
    optimizeNames(b, E) {
      var M;
      if (this.else = (M = this.else) === null || M === void 0 ? void 0 : M.optimizeNames(b, E), !!(super.optimizeNames(b, E) || this.else))
        return this.condition = P(this.condition, b, E), this;
    }
    get names() {
      const b = super.names;
  ...
  class _ extends y {
    ...
    optimizeNames(b, E) {
      if (super.optimizeNames(b, E))
        return this.iteration = P(this.iteration, b, E), this;
    }
    get names() {
  ...
  class x extends y {
    constructor(b, E, M, q) {
      super(), this.varKind = b, this.name = E, this.from = M, this.to = q;
    }
    render(b) {
      const E = b.es5 ? n.varKinds.var : this.varKind, { name: M, from: q, to: z } = this;
      return `for(${E} ${M}=${q}; ${M}<${z}; ${M}++)` + super.render(b);
    }
    get names() {
  ...
  class w extends y {
    constructor(b, E, M, q) {
      super(), this.loop = b, this.varKind = E, this.name = M, this.iterable = q;
    }
    render(b) {
    ...
    optimizeNames(b, E) {
      if (super.optimizeNames(b, E))
        return this.iterable = P(this.iterable, b, E), this;
    }
    get names() {
  ...
  class I extends g {
    constructor(b, E, M) {
      super(), this.name = b, this.args = E, this.async = M;
    }
    render(b) {
  ...
  class D extends g {
    ...
    optimizeNames(b, E) {
      var M, q;
      return super.optimizeNames(b, E), (M = this.catch) === null || M === void 0 || M.optimizeNames(b, E), (q = this.finally) === null || q === void 0 || q.optimizeNames(b, E), this;
    }
    get names() {
      const b = super.names;
...
` : "" }, this._extScope = b, this._scope = new n.Scope({ parent: b }), this._nodes = [new m()];
    ...
    scopeValue(b, E) {
      const M = this._extScope.value(b, E);
    ...
    const(b, E, M) {
      return this._def(n.varKinds.const, b, E, M);
    }
    // `let` declaration with optional assignment (`var` in es5 mode)
    let(b, E, M) {
      return this._def(n.varKinds.let, b, E, M);
    }
    // `var` declaration with optional assignment
    var(b, E, M) {
      return this._def(n.varKinds.var, b, E, M);
    }
    // assignment code
    assign(b, E, M) {
      return this._leafNode(new o(b, E, M));
    }
    // `+=` code
    add(b, E) {
      return this._leafNode(new l(b, e.operators.ADD, E));
    }
    ...
    object(...b) {
      const E = ["{"];
      for (const [M, q] of b)
        E.length > 1 && E.push(","), E.push(M), (M !== q || this.opts.es5) && (E.push(":"), (0, t.addCodeArg)(E, q));
      return E.push("}"), new t._Code(E);
    }
    // `if` clause (or statement if `thenBody` and, optionally, `elseBody` are passed)
    if(b, E, M) {
      if (this._blockNode(new d(b)), E && M)
        this.code(E).else().code(M).endIf();
      else if (E)
        this.code(E).endIf();
      else if (M)
    ...
    for(b, E) {
      return this._for(new _(b), E);
    }
    // `for` statement for a range of values
    forRange(b, E, M, q, z = this.opts.es5 ? n.varKinds.var : n.varKinds.let) {
      const X = this._scope.toName(b);
      return this._for(new x(z, X, E, M), () => q(X));
    }
    // `for-of` statement (in es5 mode replace with a normal for loop)
    forOf(b, E, M, q = n.varKinds.const) {
      const z = this._scope.toName(b);
      if (this.opts.es5) {
        const X = E instanceof t.Name ? E : this.var("_arr", E);
        return this.forRange("_i", 0, (0, t._)`${X}.length`, (W) => {
          this.var(z, (0, t._)`${X}[${W}]`), M(z);
        });
      }
      return this._for(new w("of", q, z, E), () => M(z));
    }
    // `for-in` statement.
    // With option `ownProperties` replaced with a `for-of` loop for object keys
    forIn(b, E, M, q = this.opts.es5 ? n.varKinds.var : n.varKinds.const) {
      if (this.opts.ownProperties)
        return this.forOf(b, (0, t._)`Object.keys(${E})`, M);
      const z = this._scope.toName(b);
      return this._for(new w("in", q, z, E), () => M(z));
    }
    // end `for` loop
    endFor() {
    ...
    try(b, E, M) {
      ...
      const q = new D();
      if (this._blockNode(q), this.code(b), E) {
        const z = this.name("e");
        this._currNode = q.catch = new S(z), E(z);
      }
    ...
    func(b, E = t.nil, M, q) {
      return this._blockNode(new I(b, E, M)), q && this.code(q).endFunc(), this;
    }
    ...
    _endBlockNode(b, E) {
      ...
      if (M instanceof b || E && M instanceof E)
  ...
  function P(T, b, E) {
    ...
    function q(z) {
      return z instanceof t._Code && z._items.some((X) => X instanceof t.Name && b[X.str] === 1 && E[X.str] !== void 0);
    }
  }
  function F(T, b) {
    for (const E in b)
  ...
  e.or = L;
  function $(T) {
    return (b, E) => b === t.nil ? E : E === t.nil ? b : (0, t._)`${N(b)} ${T} ${N(E)}`;
  }
  function N(T) {
...
K.toHash = Th;
function Rh(e, t) {
  return typeof t == "boolean" ? t : Object.keys(t).length === 0 ? !0 : (jp(e, t), !Cp(t, e.self.RULES.all));
}
K.alwaysValidSchema = Rh;
function jp(e, t = e.schema) {
  ...
  for (const i in t)
    a[i] || Fp(e, `unknown keyword: "${i}"`);
}
K.checkUnknownRules = jp;
function Cp(e, t) {
  if (typeof e == "boolean")
    return !e;
  for (const n in e)
...
K.schemaHasRulesButRef = Oh;
function Ah({ topSchemaRef: e, schemaPath: t }, n, r, a) {
  if (!a) {
  ...
  return (0, pe._)`${e}${t}${(0, pe.getProperty)(r)}`;
}
K.schemaRefOrVal = Ah;
function Ph(e) {
  return Lp(decodeURIComponent(e));
}
K.unescapeFragment = Ph;
function kh(e) {
...
K.escapeJsonPointer = Xs;
function Lp(e) {
  return e.replace(/~1/g, "/").replace(/~0/g, "~");
}
K.unescapeJsonPointer = Lp;
function Nh(e, t) {
  if (Array.isArray(e))
    for (const n of e)
      t(n);
...
K.eachItem = Nh;
function sl({ mergeNames: e, mergeToName: t, mergeValues: n, resultToName: r }) {
  return (a, i, s, o) => {
    const l = s === void 0 ? i : s instanceof pe.Name ? (i instanceof pe.Name ? e(a, i, s) : t(a, i, s), s) : i instanceof pe.Name ? (t(a, s, i), i) : n(i, s);
    return o === pe.Name && !(l instanceof pe.Name) ? r(a, l) : l;
  };
}
K.mergeEvaluated = {
  props: sl({
    mergeNames: (e, t, n) => e.if((0, pe._)`${n} !== true && ${t} !== undefined`, () => {
      e.if((0, pe._)`${t} === true`, () => e.assign(n, !0), () => e.assign(n, (0, pe._)`${n} || {}`).code((0, pe._)`Object.assign(${n}, ${t})`));
    }),
    mergeToName: (e, t, n) => e.if((0, pe._)`${n} !== true`, () => {
      t === !0 ? e.assign(n, !0) : (e.assign(n, (0, pe._)`${n} || {}`), Ws(e, n, t));
    }),
    mergeValues: (e, t) => e === !0 ? !0 : { ...e, ...t },
    resultToName: Dp
  }),
  items: sl({
    mergeNames: (e, t, n) => e.if((0, pe._)`${n} !== true && ${t} !== undefined`, () => e.assign(n, (0, pe._)`${t} === true ? true : ${n} > ${t} ? ${n} : ${t}`)),
    mergeToName: (e, t, n) => e.if((0, pe._)`${n} !== true`, () => e.assign(n, t === !0 ? !0 : (0, pe._)`${n} > ${t} ? ${n} : ${t}`)),
...
function Dp(e, t) {
  ...
  const n = e.var("props", (0, pe._)`{}`);
  return t !== void 0 && Ws(e, n, t), n;
}
K.evaluatedPropsToName = Dp;
function Ws(e, t, n) {
  Object.keys(n).forEach((r) => e.assign((0, pe._)`${t}${(0, pe.getProperty)(r)}`, !0));
}
K.setEvaluated = Ws;
const ol = {};
function Ih(e, t) {
  return e.scopeValue("func", {
    ref: t,
    code: ol[t.code] || (ol[t.code] = new Sh._Code(t.code))
  });
}
K.useFunc = Ih;
var vs;
...
function jh(e, t, n) {
  if (e instanceof pe.Name) {
    const r = t === vs.Num;
    return n ? r ? (0, pe._)`"[" + ${e} + "]"` : (0, pe._)`"['" + ${e} + "']"` : r ? (0, pe._)`"/" + ${e}` : (0, pe._)`"/" + ${e}.replace(/~/g, "~0").replace(/\\//g, "~1")`;
  }
  return n ? (0, pe.getProperty)(e).toString() : "/" + Xs(e);
}
K.getErrorPath = jh;
function Fp(e, t, n = e.opts.strictSchema) {
  if (n) {
...
(function(e) {
  ...
  function a(v, d = e.keywordError, y, _) {
    const { it: x } = v, { gen: w, compositeRule: I, allErrors: C } = x, D = p(v, d, y);
    _ ?? (I || C) ? l(w, D) : u(x, (0, t._)`[${D}]`);
  }
  e.reportError = a;
  function i(v, d = e.keywordError, y) {
    const { it: _ } = v, { gen: x, compositeRule: w, allErrors: I } = _, C = p(v, d, y);
    l(x, C), w || I || u(_, r.default.vErrors);
  }
  ...
  function o({ gen: v, keyword: d, schemaValue: y, data: _, errsCount: x, it: w }) {
    if (x === void 0)
      throw new Error("ajv implementation error");
    const I = v.name("err");
    v.forRange("i", x, r.default.errors, (C) => {
      v.const(I, (0, t._)`${r.default.vErrors}[${C}]`), v.if((0, t._)`${I}.instancePath === undefined`, () => v.assign((0, t._)`${I}.instancePath`, (0, t.strConcat)(r.default.instancePath, w.errorPath))), v.assign((0, t._)`${I}.schemaPath`, (0, t.str)`${w.errSchemaPath}/${d}`), w.opts.verbose && (v.assign((0, t._)`${I}.schema`, y), v.assign((0, t._)`${I}.data`, _));
    });
  }
  e.extendErrors = o;
  function l(v, d) {
  ...
  function f(v, d, y = {}) {
    const { gen: _, it: x } = v, w = [
      h(x, y),
      g(v, y)
    ];
    return m(v, d, w), _.object(...w);
  }
  ...
  function m(v, { params: d, message: y }, _) {
    const { keyword: x, data: w, schemaValue: I, it: C } = v, { opts: D, propertyName: S, topSchemaRef: R, schemaPath: O } = C;
    _.push([c.keyword, x], [c.params, typeof d == "function" ? d(v) : d || (0, t._)`{}`]), D.messages && _.push([c.message, typeof y == "function" ? y(v) : y]), D.verbose && _.push([c.schema, I], [c.parentSchema, (0, t._)`${R}${O}`], [r.default.data, w]), S && _.push([c.propertyName, S]);
  }
})(wr);
Object.defineProperty(jn, "__esModule", { value: !0 });
jn.boolOrEmptySchema = jn.topBoolOrEmptySchema = void 0;
const Lh = wr, Dh = ee, Fh = Xe, Uh = {
  message: "boolean schema is false"
};
function Mh(e) {
  const { gen: t, schema: n, validateName: r } = e;
  n === !1 ? Up(e, !1) : typeof n == "object" && n.$async === !0 ? t.return(Fh.default.data) : (t.assign((0, Dh._)`${r}.errors`, null), t.return(!0));
}
jn.topBoolOrEmptySchema = Mh;
function zh(e, t) {
  const { gen: n, schema: r } = e;
  r === !1 ? (n.var(t, !1), Up(e)) : n.var(t, !0);
}
jn.boolOrEmptySchema = zh;
function Up(e, t) {
  ...
  (0, Lh.reportError)(a, Uh, void 0, t);
}
var we = {}, an = {};
Object.defineProperty(an, "__esModule", { value: !0 });
an.getRules = an.isJSONType = void 0;
const qh = ["string", "number", "integer", "boolean", "null", "object", "array"], Bh = new Set(qh);
function Vh(e) {
...
wt.schemaHasRulesForType = Gh;
function Mp(e, t) {
  return t.rules.some((n) => zp(e, n));
}
wt.shouldUseGroup = Mp;
function zp(e, t) {
  var n;
...
const Kh = an, Xh = wt, Wh = wr, re = ee, qp = K;
var On;
(function(e) {
...
we.getSchemaTypes = Yh;
function Bp(e) {
  const t = Array.isArray(e) ? e : e ? [e] : [];
  if (t.every(Kh.isJSONType))
    return t;
  throw new Error("type must be JSONType or JSONType[]: " + t.join(","));
}
we.getJSONTypes = Bp;
function Jh(e, t) {
  const { gen: n, data: r, opts: a } = e, i = Qh(t, a.coerceTypes), s = t.length > 0 && !(i.length === 0 && t.length === 1 && (0, Xh.schemaHasRulesForType)(e, t[0]));
  if (s) {
    const o = Ys(t, r, a.strictNumbers, On.Wrong);
    n.if(o, () => {
      i.length ? Zh(e, t, i) : Js(e);
    });
  }
  return s;
}
we.coerceAndCheckDataType = Jh;
const Vp = /* @__PURE__ */ new Set(["string", "number", "integer", "boolean", "null"]);
function Qh(e, t) {
  return t ? e.filter((n) => Vp.has(n) || t === "array" && n === "array") : [];
}
function Zh(e, t, n) {
  const { gen: r, data: a, opts: i } = e, s = r.let("dataType", (0, re._)`typeof ${a}`), o = r.let("coerced", (0, re._)`undefined`);
  i.coerceTypes === "array" && r.if((0, re._)`${s} == 'object' && Array.isArray(${a}) && ${a}.length == 1`, () => r.assign(a, (0, re._)`${a}[0]`).assign(s, (0, re._)`typeof ${a}`).if(Ys(t, a, i.strictNumbers), () => r.assign(o, a))), r.if((0, re._)`${o} !== undefined`);
  for (const u of n)
    (Vp.has(u) || u === "array" && i.coerceTypes === "array") && l(u);
  r.else(), Js(e), r.endIf(), r.if((0, re._)`${o} !== undefined`, () => {
    r.assign(a, o), ev(e, o);
  });
  function l(u) {
    switch (u) {
...
function ev({ gen: e, parentData: t, parentDataProperty: n }, r) {
  e.if((0, re._)`${t} !== undefined`, () => e.assign((0, re._)`${t}[${n}]`, r));
}
function ys(e, t, n, r = On.Correct) {
  const a = r === On.Correct ? re.operators.EQ : re.operators.NEQ;
  let i;
  switch (e) {
    case "null":
  ...
  return r === On.Correct ? i : (0, re.not)(i);
  function s(o = re.nil) {
...
function Ys(e, t, n, r) {
  ...
  for (const s in i)
    a = (0, re.and)(a, ys(s, t, n, r));
  return a;
}
we.checkDataTypes = Ys;
const tv = {
...
function Js(e) {
  const t = nv(e);
  (0, Wh.reportError)(t, tv);
}
we.reportTypeError = Js;
function nv(e) {
  const { gen: t, data: n, schema: r } = e, a = (0, qp.schemaRefOrVal)(e, r, "type");
  return {
    gen: t,
    keyword: "type",
    data: n,
...
function av(e, t) {
  ...
  if (t === "object" && n)
    for (const a in n)
      cl(e, a, n[a].default);
  else t === "array" && Array.isArray(r) && r.forEach((a, i) => cl(e, i, a.default));
}
ka.assignDefaults = av;
function cl(e, t, n) {
  const { gen: r, compositeRule: a, data: i, opts: s } = e;
  if (n === void 0)
    return;
  const o = (0, yn._)`${i}${(0, yn.getProperty)(t)}`;
  if (a) {
    (0, rv.checkStrictMode)(e, `default is ignored for: ${o}`);
    return;
  }
  let l = (0, yn._)`${o} === undefined`;
  s.useDefaults === "empty" && (l = (0, yn._)`${l} || ${o} === null || ${o} === ""`), r.if(l, (0, yn._)`${o} = ${(0, yn.stringify)(n)}`);
}
var ht = {}, ae = {};
...
const me = ee, Qs = K, At = Xe, iv = K;
function sv(e, t) {
  const { gen: n, data: r, it: a } = e;
  n.if(eo(n, r, t, a.opts.ownProperties), () => {
    e.setParams({ missingProperty: (0, me._)`${t}` }, !0), e.error();
  });
}
ae.checkReportMissingProp = sv;
function ov({ gen: e, data: t, it: { opts: n } }, r, a) {
  return (0, me.or)(...r.map((i) => (0, me.and)(eo(e, t, i, n.ownProperties), (0, me._)`${a} = ${i}`)));
}
ae.checkMissingProp = ov;
function cv(e, t) {
  e.setParams({ missingProperty: t }, !0), e.error();
}
ae.reportMissingProp = cv;
function Hp(e) {
  return e.scopeValue("func", {
...
ae.hasPropFunc = Hp;
function Zs(e, t, n) {
  return (0, me._)`${Hp(e)}.call(${t}, ${n})`;
}
ae.isOwnProperty = Zs;
function lv(e, t, n, r) {
  const a = (0, me._)`${t}${(0, me.getProperty)(n)} !== undefined`;
  return r ? (0, me._)`${a} && ${Zs(e, t, n)}` : a;
}
ae.propertyInData = lv;
function eo(e, t, n, r) {
  const a = (0, me._)`${t}${(0, me.getProperty)(n)} === undefined`;
  return r ? (0, me.or)(a, (0, me.not)(Zs(e, t, n))) : a;
}
ae.noPropertyInData = eo;
function Gp(e) {
  return e ? Object.keys(e).filter((t) => t !== "__proto__") : [];
}
ae.allSchemaProperties = Gp;
function uv(e, t) {
  return Gp(t).filter((n) => !(0, Qs.alwaysValidSchema)(e, t[n]));
}
ae.schemaProperties = uv;
function pv({ schemaCode: e, data: t, it: { gen: n, topSchemaRef: r, schemaPath: a, errorPath: i }, it: s }, o, l, u) {
  const c = u ? (0, me._)`${e}, ${t}, ${r}${a}` : t, p = [
    [At.default.instancePath, (0, me.strConcat)(At.default.instancePath, i)],
  ...
  const f = (0, me._)`${c}, ${n.object(...p)}`;
  return l !== me.nil ? (0, me._)`${o}.call(${l}, ${f})` : (0, me._)`${o}(${f})`;
}
ae.callValidateCode = pv;
const dv = (0, me._)`new RegExp`;
function fv({ gen: e, it: { opts: t } }, n) {
  const r = t.unicodeRegExp ? "u" : "", { regExp: a } = t.code, i = a(n, r);
  return e.scopeValue("pattern", {
    key: i.toString(),
    ref: i,
    code: (0, me._)`${a.code === "new RegExp" ? dv : (0, iv.useFunc)(e, a)}(${n}, ${r})`
  });
}
ae.usePattern = fv;
function mv(e) {
  ...
  function s(o) {
    const l = t.const("len", (0, me._)`${n}.length`);
    t.forRange("i", 0, l, (u) => {
      e.subschema({
        keyword: r,
        dataProp: u,
...
function hv(e) {
  ...
  if (!Array.isArray(n))
    throw new Error("ajv implementation error");
  if (n.some((l) => (0, Qs.alwaysValidSchema)(a, l)) && !a.opts.unevaluated)
    return;
  ...
  t.block(() => n.forEach((l, u) => {
    ...
    t.assign(s, (0, me._)`${s} || ${o}`), e.mergeValidEvaluated(c, o) || t.if((0, me.not)(s));
...
const Ce = ee, Ht = Xe, vv = ae, yv = wr;
function gv(e, t) {
  const { gen: n, keyword: r, schema: a, parentSchema: i, it: s } = e, o = t.macro.call(s.self, a, i, s), l = Kp(n, r, o);
...
ht.macroKeywordCode = gv;
function xv(e, t) {
  var n;
  const { gen: r, keyword: a, schema: i, parentSchema: s, $data: o, it: l } = e;
  _v(l, t);
  const u = !o && t.compile ? t.compile.call(l.self, i, s, l) : t.validate, c = Kp(r, a, u), p = r.let("valid");
  e.block$data(p, f), e.ok((n = t.valid) !== null && n !== void 0 ? n : p);
  function f() {
    if (t.errors === !1)
      m(), t.modifying && ll(e), v(() => e.error());
    else {
      const d = t.async ? h() : g();
      t.modifying && ll(e), v(() => bv(e, d));
    }
  }
  function h() {
    const d = r.let("ruleErrs", null);
    return r.try(() => m((0, Ce._)`await `), (y) => r.assign(p, !1).if((0, Ce._)`${y} instanceof ${l.ValidationError}`, () => r.assign(d, (0, Ce._)`${y}.errors`), () => r.throw(y))), d;
  }
  function g() {
    const d = (0, Ce._)`${c}.errors`;
    return r.assign(d, null), m(Ce.nil), d;
  }
  function m(d = t.async ? (0, Ce._)`await ` : Ce.nil) {
    const y = l.opts.passContext ? Ht.default.this : Ht.default.self, _ = !("compile" in t && !o || t.schema === !1);
    r.assign(p, (0, Ce._)`${d}${(0, vv.callValidateCode)(e, c, y, _)}`, t.modifying);
  }
  function v(d) {
    var y;
    r.if((0, Ce.not)((y = t.valid) !== null && y !== void 0 ? y : p), d);
  }
}
ht.funcKeywordCode = xv;
function ll(e) {
...
function bv(e, t) {
  ...
  n.if((0, Ce._)`Array.isArray(${t})`, () => {
    n.assign(Ht.default.vErrors, (0, Ce._)`${Ht.default.vErrors} === null ? ${t} : ${Ht.default.vErrors}.concat(${t})`).assign(Ht.default.errors, (0, Ce._)`${Ht.default.vErrors}.length`), (0, yv.extendErrors)(e);
  }, () => e.error());
}
function _v({ schemaEnv: e }, t) {
  if (t.async && !e.$async)
...
ht.validSchemaType = wv;
function Ev({ schema: e, opts: t, self: n, errSchemaPath: r }, a, i) {
...
ht.validateKeywordUsage = Ev;
var Ft = {};
Object.defineProperty(Ft, "__esModule", { value: !0 });
Ft.extendSubschemaMode = Ft.extendSubschemaData = Ft.getSubschema = void 0;
const mt = ee, Xp = K;
function $v(e, { keyword: t, schemaProp: n, schema: r, schemaPath: a, errSchemaPath: i, topSchemaRef: s }) {
  ...
  if (t !== void 0) {
    ...
    return n === void 0 ? {
      ...
      schema: o[n],
      schemaPath: (0, mt._)`${e.schemaPath}${(0, mt.getProperty)(t)}${(0, mt.getProperty)(n)}`,
      errSchemaPath: `${e.errSchemaPath}/${t}/${(0, Xp.escapeFragment)(n)}`
    };
  }
  if (r !== void 0) {
...
Ft.getSubschema = $v;
function Sv(e, t, { dataProp: n, dataPropType: r, data: a, dataTypes: i, propertyName: s }) {
  if (a !== void 0 && n !== void 0)
    throw new Error('both "data" and "dataProp" passed, only one allowed');
  const { gen: o } = t;
  if (n !== void 0) {
    const { errorPath: u, dataPathArr: c, opts: p } = t, f = o.let("data", (0, mt._)`${t.data}${(0, mt.getProperty)(n)}`, !0);
    l(f), e.errorPath = (0, mt.str)`${u}${(0, Xp.getErrorPath)(n, r, p.jsPropertySyntax)}`, e.parentDataProperty = (0, mt._)`${n}`, e.dataPathArr = [...c, e.parentDataProperty];
  }
  if (a !== void 0) {
    const u = a instanceof mt.Name ? a : o.let("data", a, !0);
...
Ft.extendSubschemaData = Sv;
function Tv(e, { jtdDiscriminator: t, jtdMetadata: n, compositeRule: r, createErrors: a, allErrors: i }) {
  r !== void 0 && (e.compositeRule = r), a !== void 0 && (e.createErrors = a), i !== void 0 && (e.allErrors = i), e.jtdDiscriminator = t, e.jtdMetadata = n;
}
Ft.extendSubschemaMode = Tv;
var Oe = {}, Wp = function e(t, n) {
  if (t === n) return !0;
  ...
  return t !== t && n !== n;
}, Yp = { exports: {} }, Lt = Yp.exports = function(e, t, n) {
...
function sa(e, t, n, r, a, i, s, o, l, u) {
      ...
      if (Array.isArray(p)) {
        if (c in Lt.arrayKeywords)
          for (var f = 0; f < p.length; f++)
            sa(e, t, n, p[f], a + "/" + c + "/" + f, i, a, c, r, f);
      } else if (c in Lt.propsKeywords) {
        if (p && typeof p == "object")
          for (var h in p)
            sa(e, t, n, p[h], a + "/" + c + "/" + Rv(h), i, a, c, r, h);
      } else (c in Lt.keywords || e.allKeys && !(c in Lt.skipKeywords)) && sa(e, t, n, p, a + "/" + c, i, a, c, r);
    }
    n(r, a, i, s, o, l, u);
  }
}
...
const Av = K, Pv = Wp, kv = Ov, Nv = /* @__PURE__ */ new Set([
  "type",
  "format",
  "pattern",
  "maxLength",
...
function Iv(e, t = !0) {
  return typeof e == "boolean" ? !0 : t === !0 ? !gs(e) : t ? Jp(e) <= t : !1;
}
Oe.inlineRef = Iv;
...
function gs(e) {
  for (const t in e) {
    if (jv.has(t))
      return !0;
    const n = e[t];
    if (Array.isArray(n) && n.some(gs) || typeof n == "object" && gs(n))
      return !0;
  }
  return !1;
}
function Jp(e) {
  ...
  for (const n in e) {
    if (n === "$ref")
      return 1 / 0;
    if (t++, !Nv.has(n) && (typeof e[n] == "object" && (0, Av.eachItem)(e[n], (r) => t += Jp(r)), t === 1 / 0))
      return 1 / 0;
  }
  return t;
}
...
const Cv = /#\/?$/;
function An(e) {
  return e ? e.replace(Cv, "") : "";
}
Oe.normalizeId = An;
function Lv(e, t, n) {
  return n = An(n), e.resolve(t, n);
}
Oe.resolveUrl = Lv;
const Dv = /^[a-z_][-a-z0-9._]*$/i;
function Fv(e, t) {
  if (typeof e == "boolean")
    return {};
  const { schemaId: n, uriResolver: r } = this.opts, a = An(e[n] || t), i = { "": a }, s = Qp(r, a, !1), o = {}, l = /* @__PURE__ */ new Set();
  return kv(e, { allKeys: !0 }, (p, f, h, g) => {
    if (g === void 0)
      return;
    const m = s + f;
    let v = i[g];
    ...
    function d(_) {
      ...
      if (_ = An(v ? x(v, _) : _), l.has(_))
        throw c(_);
      l.add(_);
      let w = this.refs[_];
      return typeof w == "string" && (w = this.refs[w]), typeof w == "object" ? u(p, w.schema, _) : _ !== An(m) && (_[0] === "#" ? (u(p, o[_], _), o[_] = p) : this.refs[_] = m), _;
    }
    function y(_) {
      if (typeof _ == "string") {
        if (!Dv.test(_))
...
const ed = jn, ul = we, to = wt, ga = we, Uv = ka, pr = ht, Ei = Ft, Y = ee, Q = Xe, Mv = Oe, Et = K, Zn = wr;
function zv(e) {
  if (rd(e) && (ad(e), nd(e))) {
    Vv(e);
    return;
...
et.validateFunctionCode = zv;
function td({ gen: e, validateName: t, schema: n, schemaEnv: r, opts: a }, i) {
  a.code.es5 ? e.func(t, (0, Y._)`${Q.default.data}, ${Q.default.valCxt}`, r.$async, () => {
    e.code((0, Y._)`"use strict"; ${pl(n, a)}`), Bv(e, a), e.code(i);
  }) : e.func(t, (0, Y._)`${Q.default.data}, ${qv(a)}`, r.$async, () => e.code(pl(n, a)).code(i));
}
function qv(e) {
  return (0, Y._)`{${Q.default.instancePath}="", ${Q.default.parentData}, ${Q.default.parentDataProperty}, ${Q.default.rootData}=${Q.default.data}${e.dynamicRef ? (0, Y._)`, ${Q.default.dynamicAnchors}={}` : Y.nil}}={}`;
}
function Bv(e, t) {
  e.if(Q.default.valCxt, () => {
    e.var(Q.default.instancePath, (0, Y._)`${Q.default.valCxt}.${Q.default.instancePath}`), e.var(Q.default.parentData, (0, Y._)`${Q.default.valCxt}.${Q.default.parentData}`), e.var(Q.default.parentDataProperty, (0, Y._)`${Q.default.valCxt}.${Q.default.parentDataProperty}`), e.var(Q.default.rootData, (0, Y._)`${Q.default.valCxt}.${Q.default.rootData}`), t.dynamicRef && e.var(Q.default.dynamicAnchors, (0, Y._)`${Q.default.valCxt}.${Q.default.dynamicAnchors}`);
  }, () => {
    e.var(Q.default.instancePath, (0, Y._)`""`), e.var(Q.default.parentData, (0, Y._)`undefined`), e.var(Q.default.parentDataProperty, (0, Y._)`undefined`), e.var(Q.default.rootData, Q.default.data), t.dynamicRef && e.var(Q.default.dynamicAnchors, (0, Y._)`{}`);
  });
}
function Vv(e) {
  const { schema: t, opts: n, gen: r } = e;
  td(e, () => {
    n.$comment && t.$comment && sd(e), Wv(e), r.let(Q.default.vErrors, null), r.let(Q.default.errors, 0), n.unevaluated && Hv(e), id(e), Qv(e);
  });
}
function Hv(e) {
...
function Kv(e, t) {
  ...
  a.$comment && n.$comment && sd(e), Yv(e), Jv(e);
  const i = r.const("_errs", Q.default.errors);
  id(e, i), r.var(t, (0, Y._)`${i} === ${Q.default.errors}`);
}
function ad(e) {
  (0, Et.checkUnknownRules)(e), Xv(e);
}
function id(e, t) {
  if (e.opts.jtd)
    return dl(e, [], !1, t);
  const n = (0, ul.getSchemaTypes)(e.schema), r = (0, ul.coerceAndCheckDataType)(e, n);
  dl(e, n, !r, t);
}
function Xv(e) {
  const { schema: t, errSchemaPath: n, opts: r, self: a } = e;
  t.$ref && r.ignoreKeywordsWithRef && (0, Et.schemaHasRulesButRef)(t, a.RULES) && a.logger.warn(`$ref: keywords ignored in schema at path "${n}"`);
}
function Wv(e) {
  const { schema: t, opts: n } = e;
  t.default !== void 0 && n.useDefaults && n.strictSchema && (0, Et.checkStrictMode)(e, "default is ignored in the schema root");
}
function Yv(e) {
  const t = e.schema[e.opts.schemaId];
  t && (e.baseId = (0, Mv.resolveUrl)(e.opts.uriResolver, e.baseId, t));
}
function Jv(e) {
  if (e.schema.$async && !e.schemaEnv.$async)
    throw new Error("async schema in sync schema");
}
function sd({ gen: e, schemaEnv: t, schema: n, errSchemaPath: r, opts: a }) {
  const i = n.$comment;
  if (a.$comment === !0)
    e.code((0, Y._)`${Q.default.self}.logger.log(${i})`);
  else if (typeof a.$comment == "function") {
    const s = (0, Y.str)`${r}/$comment`, o = e.scopeValue("root", { ref: t.root });
    e.code((0, Y._)`${Q.default.self}.opts.$comment(${i}, ${s}, ${o}.schema)`);
  }
}
function Qv(e) {
  const { gen: t, schemaEnv: n, validateName: r, ValidationError: a, opts: i } = e;
  n.$async ? t.if((0, Y._)`${Q.default.errors} === 0`, () => t.return(Q.default.data), () => t.throw((0, Y._)`new ${a}(${Q.default.vErrors})`)) : (t.assign((0, Y._)`${r}.errors`, Q.default.vErrors), i.unevaluated && Zv(e), t.return((0, Y._)`${Q.default.errors} === 0`));
}
function Zv({ gen: e, evaluated: t, props: n, items: r }) {
...
function dl(e, t, n, r) {
  ...
  if (i.$ref && (l.ignoreKeywordsWithRef || !(0, Et.schemaHasRulesButRef)(i, c))) {
    a.block(() => ld(e, "$ref", c.all.$ref.definition));
    return;
  }
  l.jtd || ey(e, t), a.block(() => {
    for (const f of c.rules)
      p(f);
    p(c.post);
  });
  function p(f) {
    (0, to.shouldUseGroup)(i, f) && (f.type ? (a.if((0, ga.checkDataType)(f.type, s, l.strictNumbers)), fl(e, f), t.length === 1 && t[0] === f.type && n && (a.else(), (0, ga.reportTypeError)(e)), a.endIf()) : fl(e, f), o || a.if((0, Y._)`${Q.default.errors} === ${r || 0}`));
  }
}
function fl(e, t) {
  const { gen: n, schema: r, opts: { useDefaults: a } } = e;
  a && (0, Uv.assignDefaults)(e, t.type), n.block(() => {
    for (const i of t.rules)
      (0, to.shouldUseRule)(r, i) && ld(e, i.keyword, i.definition, t.type);
  });
}
function ey(e, t) {
  e.schemaEnv.meta || !e.opts.strictTypes || (ty(e, t), e.opts.allowUnionTypes || ny(e, t), ry(e, e.dataTypes));
}
function ty(e, t) {
  if (t.length) {
    ...
    t.forEach((n) => {
      od(e.dataTypes, n) || no(e, `type "${n}" not allowed by context "${e.dataTypes.join(",")}"`);
    }), iy(e, t);
  }
}
function ny(e, t) {
...
function ry(e, t) {
  ...
  for (const r in n) {
    const a = n[r];
    if (typeof a == "object" && (0, to.shouldUseRule)(e.schema, a)) {
      const { type: i } = a.definition;
      i.length && !i.some((s) => ay(t, s)) && no(e, `missing type "${i.join(",")}" for keyword "${r}"`);
    }
  }
}
function ay(e, t) {
...
function iy(e, t) {
  ...
  for (const r of e.dataTypes)
    od(t, r) ? n.push(r) : t.includes("integer") && r === "number" && n.push("integer");
  e.dataTypes = n;
}
function no(e, t) {
  const n = e.schemaEnv.baseId + e.errSchemaPath;
  t += ` at "${n}" (strictTypes)`, (0, Et.checkStrictMode)(e, t, e.opts.strictTypes);
}
class cd {
  constructor(t, n, r) {
    if ((0, pr.validateKeywordUsage)(t, n, r), this.gen = t.gen, this.allErrors = t.allErrors, this.keyword = r, this.data = t.data, this.schema = t.schema[r], this.$data = n.$data && t.opts.$data && this.schema && this.schema.$data, this.schemaValue = (0, Et.schemaRefOrVal)(t, this.schema, r, this.$data), this.schemaType = n.schemaType, this.parentSchema = t.schema, this.params = {}, this.it = t, this.def = n, this.$data)
      this.schemaCode = t.gen.const("vSchema", ud(this.$data, t));
    else if (this.schemaCode = this.schemaValue, !(0, pr.validSchemaType)(this.schema, n.schemaType, n.allowUndefined))
      throw new Error(`${r} value must be ${JSON.stringify(n.schemaType)}`);
    ("code" in n ? n.trackErrors : n.errors !== !1) && (this.errsCount = t.gen.const("_errs", Q.default.errors));
  }
  result(t, n, r) {
  ...
  _error(t, n) {
    (t ? Zn.reportExtraError : Zn.reportError)(this, this.def.error, n);
  }
  $dataError() {
  ...
  invalid$data() {
    ...
    function s() {
      if (r.length) {
        ...
        return (0, Y._)`${(0, ga.checkDataTypes)(l, n, i.opts.strictNumbers, ga.DataType.Wrong)}`;
      }
      return Y.nil;
    }
    function o() {
  ...
  subschema(t, n) {
    const r = (0, Ei.getSubschema)(this.it, t);
    (0, Ei.extendSubschemaData)(r, this.it, t), (0, Ei.extendSubschemaMode)(r, t);
    const a = { ...this.it, ...r, items: void 0, props: void 0 };
    return Gv(a, n), a;
  }
  mergeEvaluated(t, n) {
    const { it: r, gen: a } = this;
    r.opts.unevaluated && (r.props !== !0 && t.props !== void 0 && (r.props = Et.mergeEvaluated.props(a, t.props, r.props, n)), r.items !== !0 && t.items !== void 0 && (r.items = Et.mergeEvaluated.items(a, t.items, r.items, n)));
  }
...
et.KeywordCxt = cd;
function ld(e, t, n, r) {
  const a = new cd(e, n, t);
  "code" in n ? n.code(a, r) : a.$data && n.validate ? (0, pr.funcKeywordCode)(a, n) : "macro" in n ? (0, pr.macroKeywordCode)(a, n) : (n.compile || n.validate) && (0, pr.funcKeywordCode)(a, n);
}
const sy = /^\/(?:[^~]|~0|~1)*$/, oy = /^([0-9]+)(#|\/(?:[^~]|~0|~1)*)?$/;
function ud(e, { dataLevel: t, dataNames: n, dataPathArr: r }) {
  let a, i;
  if (e === "")
  ...
  const o = a.split("/");
  for (const u of o)
    u && (i = (0, Y._)`${i}${(0, Y.getProperty)((0, Et.unescapeJsonPointer)(u))}`, s = (0, Y._)`${s} && ${i}`);
  return s;
  function l(u, c) {
...
const it = ee, uy = Mn, Vt = Xe, ot = Oe, ml = K, py = et;
class Na {
  constructor(t) {
    var n;
    this.refs = {}, this.dynamicAnchors = {};
    let r;
    typeof t.schema == "object" && (r = t.schema), this.schema = t.schema, this.schemaId = t.schemaId, this.root = t.root || this, this.baseId = (n = t.baseId) !== null && n !== void 0 ? n : (0, ot.normalizeId)(r == null ? void 0 : r[t.schemaId || "$id"]), this.schemaPath = t.schemaPath, this.localRefs = t.localRefs, this.meta = t.meta, this.$async = r == null ? void 0 : r.$async, this.refs = {};
  }
}
Le.SchemaEnv = Na;
function ro(e) {
  const t = pd.call(this, e);
  if (t)
    return t;
  const n = (0, ot.getFullPath)(this.opts.uriResolver, e.root.baseId), { es5: r, lines: a } = this.opts.code, { ownProperties: i } = this.opts, s = new it.CodeGen(this.scope, { es5: r, lines: a, ownProperties: i });
  let o;
  ...
  const u = {
    ...
    topSchemaRef: s.scopeValue("schema", this.opts.code.source === !0 ? { ref: e.schema, code: (0, it.stringify)(e.schema) } : { ref: e.schema }),
    validateName: l,
    ValidationError: o,
    schema: e.schema,
    schemaEnv: e,
  ...
  let c;
  try {
    this._compilations.add(e), (0, py.validateFunctionCode)(u), s.optimize(this.opts.code.optimize);
    const p = s.toString();
    c = `${s.scopeRefs(Vt.default.scope)}return ${p}`, this.opts.code.process && (c = this.opts.code.process(c, e));
    const h = new Function(`${Vt.default.self}`, `${Vt.default.scope}`, c)(this, this.scope.get());
    if (this.scope.value(l, { ref: h }), h.errors = null, h.schema = e.schema, h.schemaEnv = e, e.$async && (h.$async = !0), this.opts.code.source === !0 && (h.source = { validateName: l, validateCode: p, scopeValues: s._values }), this.opts.unevaluated) {
      ...
      h.evaluated = {
        props: g instanceof it.Name ? void 0 : g,
        items: m instanceof it.Name ? void 0 : m,
        dynamicProps: g instanceof it.Name,
        dynamicItems: m instanceof it.Name
      }, h.source && (h.source.evaluated = (0, it.stringify)(h.evaluated));
    }
...
function dy(e, t, n) {
  var r;
  n = (0, ot.resolveUrl)(this.opts.uriResolver, t, n);
  const a = e.refs[n];
  if (a)
    return a;
  let i = hy.call(this, e, n);
  if (i === void 0) {
    const s = (r = e.localRefs) === null || r === void 0 ? void 0 : r[n], { schemaId: o } = this.opts;
    s && (i = new Na({ schema: s, schemaId: o, root: e, baseId: t }));
  }
  if (i !== void 0)
    return e.refs[n] = fy.call(this, i);
}
Le.resolveRef = dy;
function fy(e) {
...
function hy(e, t) {
  ...
  for (; typeof (n = this.refs[t]) == "string"; )
    t = n;
  return n || this.schemas[t] || Ia.call(this, e, t);
}
function Ia(e, t) {
  const n = this.opts.uriResolver.parse(t), r = (0, ot._getFullPath)(this.opts.uriResolver, n);
  let a = (0, ot.getFullPath)(this.opts.uriResolver, e.baseId, void 0);
  if (Object.keys(e.schema).length > 0 && r === a)
    return Si.call(this, n, e);
  const i = (0, ot.normalizeId)(r), s = this.refs[i] || this.schemas[i];
  if (typeof s == "string") {
    const o = Ia.call(this, e, s);
    return typeof (o == null ? void 0 : o.schema) != "object" ? void 0 : Si.call(this, n, o);
  }
  if (typeof (s == null ? void 0 : s.schema) == "object") {
    if (s.validate || ro.call(this, s), i === (0, ot.normalizeId)(t)) {
      const { schema: o } = s, { schemaId: l } = this.opts, u = o[l];
      return u && (a = (0, ot.resolveUrl)(this.opts.uriResolver, a, u)), new Na({ schema: o, schemaId: l, root: e, baseId: a });
    }
...
function Si(e, { baseId: t, schema: n, root: r }) {
  ...
  for (const o of e.fragment.slice(1).split("/")) {
    if (typeof n == "boolean")
      return;
    const l = n[(0, ml.unescapeFragment)(o)];
    if (l === void 0)
      return;
    n = l;
    const u = typeof n == "object" && n[this.opts.schemaId];
    !vy.has(o) && u && (t = (0, ot.resolveUrl)(this.opts.uriResolver, t, u));
  }
  let i;
  if (typeof n != "boolean" && n.$ref && !(0, ml.schemaHasRulesButRef)(n, this.RULES)) {
    const o = (0, ot.resolveUrl)(this.opts.uriResolver, t, n.$ref);
    i = Ia.call(this, r, o);
  }
  const { schemaId: s } = this.opts;
  if (i = i || new Na({ schema: n, schemaId: s, root: r, baseId: t }), i.schema !== i.root.schema)
    return i;
}
const yy = "https://raw.githubusercontent.com/ajv-validator/ajv/master/lib/refs/data.json#", gy = "Meta-schema for $data reference (JSON AnySchema extension proposal)", xy = "object", by = [
  "$data"
], _y = {
  $data: {
    type: "string",
  ...
  required: by,
  properties: _y,
  additionalProperties: wy
};
var ao = {}, ja = { exports: {} };
const $y = RegExp.prototype.test.bind(/^[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}$/iu), dd = RegExp.prototype.test.bind(/^(?:(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)\.){3}(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]\d|\d)$/u), io = RegExp.prototype.test.bind(/^[\da-f]{2}$/iu), fd = RegExp.prototype.test.bind(/^[\da-z\-._~]$/iu), Sy = RegExp.prototype.test.bind(/^[\da-z\-._~!$&'()*+,;=:@/]$/iu);
function md(e) {
  let t = "", n = 0, r = 0;
...
const Ty = RegExp.prototype.test.bind(/[^!"$&'()*+,\-.;=_`a-z{}~]/u);
function hl(e) {
  return e.length = 0, !0;
}
function Ry(e, t, n) {
...
function Oy(e) {
  ...
  return a.length && (o === hl ? n.zone = a.join("") : s ? r.push(a.join("")) : r.push(md(a))), n.address = r.join(""), n;
}
function hd(e) {
  if (Ay(e, ":") < 2)
...
const ky = { "@": "%40", "/": "%2F", "?": "%3F", "#": "%23", ":": "%3A" }, Ny = /[@/?#:]/g, Iy = /[@/?#]/g;
function vd(e, t) {
  const n = t ? Iy : Ny;
  return n.lastIndex = 0, e.replace(n, (r) => ky[r]);
}
function jy(e, t = !1) {
  if (e.indexOf("%") === -1)
    return e;
...
function Dy(e) {
  ...
  if (e.userinfo !== void 0 && (t.push(e.userinfo), t.push("@")), e.host !== void 0) {
    ...
    if (!dd(n)) {
      const r = hd(n);
      r.isIPV6 === !0 ? n = `[${r.escapedHost}]` : n = vd(n, !1);
    }
    t.push(n);
  }
...
var yd = {
  nonSimpleDomain: Ty,
  recomposeAuthority: Dy,
  reescapeHostDelimiters: vd,
  normalizePercentEncoding: jy,
  normalizePathEncoding: Cy,
  escapePreservingEscapes: Ly,
  removeDotSegments: Py,
  isIPv4: dd,
  isUUID: $y,
  normalizeIPv6: hd
};
const { isUUID: Fy } = yd, Uy = /([\da-z][\d\-a-z]{0,31}):((?:[\w!$'()*+,\-.:;=@]|%[\da-f]{2})+)/iu;
function gd(e) {
...
function My(e) {
  return e.secure = gd(e), e.resourceName = (e.path || "/") + (e.query ? "?" + e.query : ""), e.path = void 0, e.query = void 0, e;
}
function zy(e) {
  if ((e.port === (gd(e) ? 443 : 80) || e.port === "") && (e.port = void 0), typeof e.secure == "boolean" && (e.scheme = e.secure ? "wss" : "ws", e.secure = void 0), e.resourceName) {
...
function By(e, t) {
  ...
  const n = t.scheme || e.scheme || "urn", r = e.nid.toLowerCase(), a = `${n}:${t.nid || r}`, i = so(a);
  i && (e = i.serialize(e, t));
  const s = e, o = e.nss;
...
function Vy(e, t) {
  const n = e;
  return n.uuid = n.nss, n.nss = void 0, !t.tolerant && (!n.uuid || !Fy(n.uuid)) && (n.error = n.error || "UUID is not valid."), n;
}
function Hy(e) {
  const t = e;
...
const _d = (
  ...
  {
    scheme: "wss",
    domainHost: oa.domainHost,
    parse: oa.parse,
    serialize: oa.serialize
  }
), Xy = (
  /** @type {SchemeHandler} */
  {
...
const { normalizeIPv6: Jy, removeDotSegments: lr, recomposeAuthority: Qy, normalizePercentEncoding: Zy, normalizePathEncoding: eg, escapePreservingEscapes: tg, reescapeHostDelimiters: ng, isIPv4: rg, nonSimpleDomain: ag } = yd, { SCHEMES: ig, getSchemeHandler: wd } = Yy;
function sg(e, t) {
  return typeof e == "string" ? e = /** @type {T} */
  pg(e, t) : typeof e == "object" && (e = /** @type {T} */
  Cn(sn(e, t), t)), e;
}
function og(e, t, n) {
  const r = n ? Object.assign({ scheme: "null" }, n) : { scheme: "null" }, a = Ed(Cn(e, r), Cn(t, r), r, !0);
  return r.skipEscape = !0, sn(a, r);
}
function Ed(e, t, n, r) {
  const a = {};
  return r || (e = Cn(sn(e, n), n), t = Cn(sn(t, n), n)), n = n || {}, !n.tolerant && t.scheme ? (a.scheme = t.scheme, a.userinfo = t.userinfo, a.host = t.host, a.port = t.port, a.path = lr(t.path || ""), a.query = t.query) : (t.userinfo !== void 0 || t.host !== void 0 || t.port !== void 0 ? (a.userinfo = t.userinfo, a.host = t.host, a.port = t.port, a.path = lr(t.path || ""), a.query = t.query) : (t.path ? (t.path[0] === "/" ? a.path = lr(t.path) : ((e.userinfo !== void 0 || e.host !== void 0 || e.port !== void 0) && !e.path ? a.path = "/" + t.path : e.path ? a.path = e.path.slice(0, e.path.lastIndexOf("/") + 1) + t.path : a.path = t.path, a.path = lr(a.path)), a.query = t.query) : (a.path = e.path, t.query !== void 0 ? a.query = t.query : a.query = e.query), a.userinfo = e.userinfo, a.host = e.host, a.port = e.port), a.scheme = e.scheme), a.fragment = t.fragment, a;
}
function cg(e, t, n) {
  const r = vl(e, n), a = vl(t, n);
...
function sn(e, t) {
  const n = {
    ...
    secure: e.secure,
    error: ""
  }, r = Object.assign({}, t), a = [], i = wd(r.scheme || n.scheme);
  i && i.serialize && i.serialize(n, r), n.path !== void 0 && (r.skipEscape ? n.path = Zy(n.path) : (n.path = tg(n.path), n.scheme !== void 0 && (n.path = n.path.split("%3A").join(":")))), r.reference !== "suffix" && n.scheme && a.push(n.scheme, ":");
  const s = Qy(n);
  if (s !== void 0 && (r.reference !== "suffix" && a.push("//"), a.push(s), n.path && n.path[0] !== "/" && a.push("/")), n.path !== void 0) {
    let o = n.path;
    !r.absolutePath && (!i || !i.absolutePath) && (o = lr(o)), s === void 0 && o[0] === "/" && o[1] === "/" && (o = "/%2F" + o.slice(2)), a.push(o);
  }
...
function $d(e, t) {
  ...
  if (s) {
    ...
    const l = wd(n.scheme || r.scheme);
    if (!n.unicodeSupport && (!l || !l.unicodeSupport) && r.host && (n.domainHost || l && l.domainHost) && i === !1 && ag(r.host))
      try {
        r.host = URL.domainToASCII(r.host.toLowerCase());
      } catch (u) {
        r.error = r.error || "Host's domain name can not be converted to ASCII: " + u;
      }
    if ((!l || l && !l.skipNormalize) && (e.indexOf("%") !== -1 && (r.scheme !== void 0 && (r.scheme = unescape(r.scheme)), r.host !== void 0 && (r.host = ng(unescape(r.host), i))), r.path && (r.path = eg(r.path)), r.fragment))
      try {
...
function pg(e, t) {
  return Sd(e, t).normalized;
}
function Sd(e, t) {
  const { parsed: n, malformedAuthorityOrPort: r } = $d(e, t);
  return {
    normalized: r ? e : sn(n, t),
    malformedAuthorityOrPort: r
  };
}
function vl(e, t) {
  if (typeof e == "string") {
    const { normalized: n, malformedAuthorityOrPort: r } = Sd(e, t);
    return r ? void 0 : n;
  }
  if (typeof e == "object")
    return sn(e, t);
...
ja.exports = oo;
ja.exports.default = oo;
ja.exports.fastUri = oo;
var dg = ja.exports;
...
(function(e) {
  ...
  const r = Mn, a = dn, i = an, s = Le, o = ee, l = Oe, u = we, c = K, p = Ey, f = ao, h = (L, $) => new RegExp(L, $);
  h.code = "new RegExp";
  ...
  function _(L) {
    var $, N, T, b, E, M, q, z, X, W, J, ue, de, ie, He, nt, fe, We, Kn, zt, hn, Xn, Wn, qt, vn;
    const St = L.strict, Bt = ($ = L.code) === null || $ === void 0 ? void 0 : $.optimize, $c = Bt === !0 || Bt === void 0 ? 1 : Bt || 0, Sc = (T = (N = L.code) === null || N === void 0 ? void 0 : N.regExp) !== null && T !== void 0 ? T : h, dm = (b = L.uriResolver) !== null && b !== void 0 ? b : f.default;
    return {
      strictSchema: (M = (E = L.strictSchema) !== null && E !== void 0 ? E : St) !== null && M !== void 0 ? M : !0,
      ...
      strictRequired: (ie = (de = L.strictRequired) !== null && de !== void 0 ? de : St) !== null && ie !== void 0 ? ie : !1,
      code: L.code ? { ...L.code, optimize: $c, regExp: Sc } : { optimize: $c, regExp: Sc },
      loopRequired: (He = L.loopRequired) !== null && He !== void 0 ? He : y,
      loopEnum: (nt = L.loopEnum) !== null && nt !== void 0 ? nt : y,
      meta: (fe = L.meta) !== null && fe !== void 0 ? fe : !0,
      messages: (We = L.messages) !== null && We !== void 0 ? We : !0,
      inlineRefs: (Kn = L.inlineRefs) !== null && Kn !== void 0 ? Kn : !0,
      schemaId: (zt = L.schemaId) !== null && zt !== void 0 ? zt : "$id",
      addUsedSchema: (hn = L.addUsedSchema) !== null && hn !== void 0 ? hn : !0,
      validateSchema: (Xn = L.validateSchema) !== null && Xn !== void 0 ? Xn : !0,
      validateFormats: (Wn = L.validateFormats) !== null && Wn !== void 0 ? Wn : !0,
      unicodeRegExp: (qt = L.unicodeRegExp) !== null && qt !== void 0 ? qt : !0,
      int32range: (vn = L.int32range) !== null && vn !== void 0 ? vn : !0,
      uriResolver: dm
    };
  }
  class x {
    constructor($ = {}) {
      ...
      $.validateFormats = !1, this.RULES = (0, i.getRules)(), w.call(this, v, $, "NOT SUPPORTED"), w.call(this, d, $, "DEPRECATED", "warn"), this._metaOpts = R.call(this), $.formats && D.call(this), this._addVocabularies(), this._addDefaultMetaSchema(), $.keywords && S.call(this, $.keywords), typeof $.meta == "object" && this.addMetaSchema($.meta), C.call(this), $.validateFormats = b;
    }
    _addVocabularies() {
    ...
    compileAsync($, N) {
      ...
      async function z(W) {
        ...
        this.refs[W] || await E.call(this, J.$schema), this.refs[W] || this.addSchema(J, W, N);
      }
      async function X(W) {
    ...
    addSchema($, N, T, b = this.opts.validateSchema) {
      ...
      if (typeof $ == "object") {
        ...
        if (E = $[M], E !== void 0 && typeof E != "string")
          throw new Error(`schema ${M} must be string`);
      }
      return N = (0, l.normalizeId)(N || E), this._checkUnique(N), this.schemas[N] = this._addSchema($, T, N, b, !0), this;
    }
    ...
    addKeyword($, N) {
      ...
      if (P.call(this, T, N), !N)
        return (0, c.eachItem)(T, (E) => F.call(this, E)), this;
      j.call(this, N);
      const b = {
        ...N,
      ...
      return (0, c.eachItem)(T, b.type.length === 0 ? (E) => F.call(this, E, b) : (E) => b.type.forEach((M) => F.call(this, E, b, M))), this;
    }
    getKeyword($) {
    ...
    removeKeyword($) {
      ...
      for (const T of N.rules) {
        const b = T.rules.findIndex((E) => E.keyword === $);
    ...
    errorsText($ = this.errors, { separator: N = ", ", dataVar: T = "data" } = {}) {
      return !$ || $.length === 0 ? "No errors" : $.map((b) => `${T}${b.instancePath} ${b.message}`).reduce((b, E) => b + N + E);
    }
    $dataMetaSchema($, N) {
    ...
    _addSchema($, N, T, b = this.opts.validateSchema, E = this.opts.addUsedSchema) {
      let M;
      ...
      return z = new s.SchemaEnv({ schema: $, schemaId: q, meta: N, baseId: T, localRefs: X }), this._cache.set(z.schema, z), E && !T.startsWith("#") && (T && this._checkUnique(T), this.refs[T] = z), b && this.validateSchema($, !0), z;
    }
    _checkUnique($) {
  ...
  function w(L, $, N, T = "error") {
    for (const b in L) {
      const E = b;
      E in $ && this.logger[T](`${N}: option ${b}. ${L[E]}`);
    }
  }
  function I(L) {
  ...
  function F(L, $, N) {
    ...
    const { RULES: E } = this;
    let M = b ? E.post : E.rules.find(({ type: z }) => z === N);
    if (M || (M = { type: N, rules: [] }, E.rules.push(M)), E.keywords[L] = !0, !$)
      return;
    const q = {
      keyword: L,
      definition: {
    ...
    $.before ? H.call(this, M, q, $.before) : M.rules.push(q), E.all[L] = q, (T = $.implements) === null || T === void 0 || T.forEach((z) => this.addKeyword(z));
  }
  function H(L, $, N) {
...
const mg = dn, yl = ae, Me = ee, gn = Xe, gl = Le, qr = K, hg = {
  keyword: "$ref",
  schemaType: "string",
  code(e) {
    ...
    if ((n === "#" || n === "#/") && a === u.baseId)
      return p();
    const c = gl.resolveRef.call(l, u, a, n);
    if (c === void 0)
      throw new mg.default(r.opts.uriResolver, a, n);
    if (c instanceof gl.SchemaEnv)
      return f(c);
    return h(c);
    function p() {
      if (i === u)
        return ca(e, s, i, i.$async);
      const g = t.scopeValue("root", { ref: u });
      return ca(e, (0, Me._)`${g}.validate`, u, u.$async);
    }
    function f(g) {
      const m = Rd(e, g);
      ca(e, m, g, g.$async);
    ...
    function h(g) {
      const m = t.scopeValue("schema", o.code.source === !0 ? { ref: g, code: (0, Me.stringify)(g) } : { ref: g }), v = t.name("valid"), d = e.subschema({
        schema: g,
        dataTypes: [],
        schemaPath: Me.nil,
        topSchemaRef: m,
...
function Rd(e, t) {
  const { gen: n } = e;
  return t.validate ? n.scopeValue("validate", { ref: t.validate }) : (0, Me._)`${n.scopeValue("wrapper", { ref: t })}.validate`;
}
$t.getValidate = Rd;
function ca(e, t, n, r) {
  const { gen: a, it: i } = e, { allErrors: s, schemaEnv: o, opts: l } = i, u = l.passContext ? gn.default.this : Me.nil;
  r ? c() : p();
  function c() {
    if (!o.$async)
    ...
    const g = a.let("valid");
    a.try(() => {
      a.code((0, Me._)`await ${(0, yl.callValidateCode)(e, t, u)}`), h(t), s || a.assign(g, !0);
    }, (m) => {
      a.if((0, Me._)`!(${m} instanceof ${i.ValidationError})`, () => a.throw(m)), f(m), s || a.assign(g, !1);
    }), e.ok(g);
  }
  function p() {
    e.result((0, yl.callValidateCode)(e, t, u), () => h(t), () => f(t));
  }
  function f(g) {
    const m = (0, Me._)`${g}.errors`;
    a.assign(gn.default.vErrors, (0, Me._)`${gn.default.vErrors} === null ? ${m} : ${gn.default.vErrors}.concat(${m})`), a.assign(gn.default.errors, (0, Me._)`${gn.default.vErrors}.length`);
  }
  function h(g) {
    var m;
    ...
    if (i.props !== !0)
      if (v && !v.dynamicProps)
        v.props !== void 0 && (i.props = qr.mergeEvaluated.props(a, v.props, i.props));
      else {
        const d = a.var("props", (0, Me._)`${g}.evaluated.props`);
        i.props = qr.mergeEvaluated.props(a, d, i.props, Me.Name);
      }
    if (i.items !== !0)
      if (v && !v.dynamicItems)
        v.items !== void 0 && (i.items = qr.mergeEvaluated.items(a, v.items, i.items));
      else {
        const d = a.var("items", (0, Me._)`${g}.evaluated.items`);
        i.items = qr.mergeEvaluated.items(a, d, i.items, Me.Name);
      }
  }
}
$t.callRef = ca;
...
const ba = ee, Pt = ba.operators, _a = {
  ...
  message: ({ keyword: e, schemaCode: t }) => (0, ba.str)`must be ${_a[e].okStr} ${t}`,
  params: ({ keyword: e, schemaCode: t }) => (0, ba._)`{comparison: ${_a[e].okStr}, limit: ${t}}`
}, bg = {
  keyword: Object.keys(_a),
  type: "number",
  schemaType: "number",
  $data: !0,
  error: xg,
  code(e) {
    ...
    e.fail$data((0, ba._)`${n} ${_a[t].fail} ${r} || isNaN(${n})`);
  }
};
uo.default = bg;
var po = {};
...
const dr = ee, _g = {
  ...
  error: _g,
  code(e) {
    const { gen: t, data: n, schemaCode: r, it: a } = e, i = a.opts.multipleOfPrecision, s = t.let("res"), o = i ? (0, dr._)`Math.abs(Math.round(${s}) - ${s}) > 1e-${i}` : (0, dr._)`${s} !== parseInt(${s})`;
    e.fail$data((0, dr._)`(${r} === 0 || (${s} = ${n}/${r}, ${o}))`);
  }
};
po.default = wg;
var fo = {}, mo = {};
...
mo.default = Od;
Od.code = 'require("ajv/dist/runtime/ucs2length").default';
Object.defineProperty(fo, "__esModule", { value: !0 });
const Gt = ee, Eg = K, $g = mo, Sg = {
  message({ keyword: e, schemaCode: t }) {
    const n = e === "maxLength" ? "more" : "fewer";
    return (0, Gt.str)`must NOT have ${n} than ${t} characters`;
  },
  ...
  error: Sg,
  code(e) {
    const { keyword: t, data: n, schemaCode: r, it: a } = e, i = t === "maxLength" ? Gt.operators.GT : Gt.operators.LT, s = a.opts.unicode === !1 ? (0, Gt._)`${n}.length` : (0, Gt._)`${(0, Eg.useFunc)(e.gen, $g.default)}(${n})`;
    e.fail$data((0, Gt._)`${s} ${i} ${r}`);
  }
};
fo.default = Tg;
var ho = {};
Object.defineProperty(ho, "__esModule", { value: !0 });
const Rg = ae, Og = K, Tn = ee, Ag = {
  ...
  code(e) {
    ...
    if (r) {
      const { regExp: l } = s.opts.code, u = l.code === "new RegExp" ? (0, Tn._)`new RegExp` : (0, Og.useFunc)(t, l), c = t.let("valid");
      t.try(() => t.assign(c, (0, Tn._)`${u}(${i}, ${o}).test(${n})`), () => t.assign(c, !1)), e.fail$data((0, Tn._)`!${c}`);
    } else {
      const l = (0, Rg.usePattern)(e, a);
      e.fail$data((0, Tn._)`!${l}.test(${n})`);
    }
  }
};
ho.default = Pg;
...
const fr = ee, kg = {
  message({ keyword: e, schemaCode: t }) {
    ...
    return (0, fr.str)`must NOT have ${n} than ${t} properties`;
  },
  ...
  error: kg,
  code(e) {
    const { keyword: t, data: n, schemaCode: r } = e, a = t === "maxProperties" ? fr.operators.GT : fr.operators.LT;
    e.fail$data((0, fr._)`Object.keys(${n}).length ${a} ${r}`);
  }
};
vo.default = Ng;
var yo = {};
Object.defineProperty(yo, "__esModule", { value: !0 });
const er = ae, mr = ee, Ig = K, jg = {
  message: ({ params: { missingProperty: e } }) => (0, mr.str)`must have required property '${e}'`,
  params: ({ params: { missingProperty: e } }) => (0, mr._)`{missingProperty: ${e}}`
}, Cg = {
  keyword: "required",
  type: "object",
  schemaType: "array",
    ...
    if (s.allErrors ? u() : c(), o.strictRequired) {
      ...
      for (const m of n)
        if ((h == null ? void 0 : h[m]) === void 0 && !g.has(m)) {
          ...
          (0, Ig.checkStrictMode)(s, d, s.opts.strictRequired);
        }
    }
    function u() {
      if (l || i)
        e.block$data(mr.nil, p);
      else
        for (const h of n)
          (0, er.checkReportMissingProp)(e, h);
    }
    function c() {
      ...
      if (l || i) {
        ...
        t.if((0, er.checkMissingProp)(e, n, h)), (0, er.reportMissingProp)(e, h), t.else();
    }
    function p() {
      t.forOf("prop", r, (h) => {
        e.setParams({ missingProperty: h }), t.if((0, er.noPropertyInData)(t, a, h, o.ownProperties), () => e.error());
      });
    }
    function f(h, g) {
      e.setParams({ missingProperty: h }), t.forOf(h, r, () => {
        t.assign(g, (0, er.propertyInData)(t, a, h, o.ownProperties)), t.if((0, mr.not)(g), () => {
          e.error(), t.break();
        });
      }, mr.nil);
    }
...
const hr = ee, Lg = {
  message({ keyword: e, schemaCode: t }) {
    ...
    return (0, hr.str)`must NOT have ${n} than ${t} items`;
  },
  ...
  error: Lg,
  code(e) {
    const { keyword: t, data: n, schemaCode: r } = e, a = t === "maxItems" ? hr.operators.GT : hr.operators.LT;
    e.fail$data((0, hr._)`${n}.length ${a} ${r}`);
  }
};
go.default = Dg;
var xo = {}, Er = {};
...
const Ad = Wp;
Ad.code = 'require("ajv/dist/runtime/equal").default';
Er.default = Ad;
Object.defineProperty(xo, "__esModule", { value: !0 });
const Ti = we, Te = ee, Fg = K, Ug = Er, Mg = {
  ...
  code(e) {
    ...
    e.block$data(l, c, (0, Te._)`${s} === false`), e.ok(l);
    function c() {
      const g = t.let("i", (0, Te._)`${n}.length`), m = t.let("j");
      e.setParams({ i: g, j: m }), t.assign(l, !0), t.if((0, Te._)`${g} > 1`, () => (p() ? f : h)(g, m));
    }
    function p() {
      return u.length > 0 && !u.some((g) => g === "object" || g === "array");
    }
    function f(g, m) {
      const v = t.name("item"), d = (0, Ti.checkDataTypes)(u, v, o.opts.strictNumbers, Ti.DataType.Wrong), y = t.const("indices", (0, Te._)`{}`);
      t.for((0, Te._)`;${g}--;`, () => {
        t.let(v, (0, Te._)`${n}[${g}]`), t.if(d, (0, Te._)`continue`), u.length > 1 && t.if((0, Te._)`typeof ${v} == "string"`, (0, Te._)`${v} += "_"`), t.if((0, Te._)`typeof ${y}[${v}] == "number"`, () => {
          t.assign(m, (0, Te._)`${y}[${v}]`), e.error(), t.assign(l, !1).break();
        }).code((0, Te._)`${y}[${v}] = ${g}`);
      });
    }
    function h(g, m) {
      const v = (0, Fg.useFunc)(t, Ug.default), d = t.name("outer");
      t.label(d).for((0, Te._)`;${g}--;`, () => t.for((0, Te._)`${m} = ${g}; ${m}--;`, () => t.if((0, Te._)`${v}(${n}[${g}], ${n}[${m}])`, () => {
...
const xs = ee, qg = K, Bg = Er, Vg = {
  ...
  code(e) {
    ...
    r || i && typeof i == "object" ? e.fail$data((0, xs._)`!${(0, qg.useFunc)(t, Bg.default)}(${n}, ${a})`) : e.fail((0, xs._)`${i} !== ${n}`);
  }
};
bo.default = Hg;
var _o = {};
Object.defineProperty(_o, "__esModule", { value: !0 });
const ur = ee, Gg = K, Kg = Er, Xg = {
  ...
  code(e) {
    ...
    let l;
    const u = () => l ?? (l = (0, Gg.useFunc)(t, Kg.default));
    let c;
    if (o || r)
      ...
      c = (0, ur.or)(...a.map((g, m) => f(h, m)));
    }
    e.pass(c);
    function p() {
      t.assign(c, !1), t.forOf("v", i, (h) => t.if((0, ur._)`${u()}(${n}, ${h})`, () => t.assign(c, !0).break()));
    }
    function f(h, g) {
      const m = a[g];
      return typeof m == "object" && m !== null ? (0, ur._)`${u()}(${n}, ${h}[${g}])` : (0, ur._)`${n} === ${m}`;
    }
  }
};
_o.default = Wg;
Object.defineProperty(La, "__esModule", { value: !0 });
const Yg = uo, Jg = po, Qg = fo, Zg = ho, ex = vo, tx = yo, nx = go, rx = xo, ax = bo, ix = _o, sx = [
  // number
  Yg.default,
  Jg.default,
  // string
...
zn.validateAdditionalItems = void 0;
const Kt = ee, bs = K, ox = {
  ...
  code(e) {
    ...
    if (!Array.isArray(r)) {
      (0, bs.checkStrictMode)(n, '"additionalItems" is ignored when "items" is not an array of schemas');
      return;
    }
    Pd(e, r);
  }
...
function Pd(e, t) {
  ...
  s.items = !0;
  const o = n.const("len", (0, Kt._)`${a}.length`);
  if (r === !1)
    e.setParams({ len: t.length }), e.pass((0, Kt._)`${o} <= ${t.length}`);
  else if (typeof r == "object" && !(0, bs.alwaysValidSchema)(s, r)) {
    const u = n.var("valid", (0, Kt._)`${o} <= ${t.length}`);
    n.if((0, Kt.not)(u), () => l(u)), e.ok(u);
  }
  function l(u) {
    n.forRange("i", t.length, o, (c) => {
      e.subschema({ keyword: i, dataProp: c, dataPropType: bs.Type.Num }, u), s.allErrors || n.if((0, Kt.not)(u), () => n.break());
    });
  }
}
zn.validateAdditionalItems = Pd;
zn.default = cx;
var wo = {}, qn = {};
Object.defineProperty(qn, "__esModule", { value: !0 });
qn.validateTuple = void 0;
const xl = ee, la = K, lx = ae, ux = {
  keyword: "items",
  type: "array",
  ...
  code(e) {
    ...
    if (Array.isArray(t))
      return kd(e, "additionalItems", t);
    n.items = !0, !(0, la.alwaysValidSchema)(n, t) && e.ok((0, lx.validateArray)(e));
  }
};
function kd(e, t, n = e.schema) {
  const { gen: r, parentSchema: a, data: i, keyword: s, it: o } = e;
  c(a), o.opts.unevaluated && n.length && o.items !== !0 && (o.items = la.mergeEvaluated.items(r, n.length, o.items));
  const l = r.name("valid"), u = r.const("len", (0, xl._)`${i}.length`);
  n.forEach((p, f) => {
    (0, la.alwaysValidSchema)(o, p) || (r.if((0, xl._)`${u} > ${f}`, () => e.subschema({
      keyword: s,
      schemaProp: f,
      dataProp: f
    }, l)), e.ok(l));
  ...
  function c(p) {
    ...
    if (f.strictTuples && !m) {
      ...
      (0, la.checkStrictMode)(o, v, f.strictTuples);
    }
  }
}
qn.validateTuple = kd;
...
const px = qn, dx = {
  ...
  schemaType: ["array"],
  before: "uniqueItems",
  code: (e) => (0, px.validateTuple)(e, "items")
};
wo.default = dx;
var Eo = {};
Object.defineProperty(Eo, "__esModule", { value: !0 });
const bl = ee, fx = K, mx = ae, hx = zn, vx = {
  message: ({ params: { len: e } }) => (0, bl.str)`must NOT have more than ${e} items`,
  params: ({ params: { len: e } }) => (0, bl._)`{limit: ${e}}`
}, yx = {
  keyword: "items",
  type: "array",
  ...
  code(e) {
    ...
    r.items = !0, !(0, fx.alwaysValidSchema)(r, t) && (a ? (0, hx.validateAdditionalItems)(e, a) : e.ok((0, mx.validateArray)(e)));
  }
};
Eo.default = yx;
var $o = {};
...
const Ze = ee, Br = K, gx = {
  ...
  code(e) {
    ...
    if (e.setParams({ min: s, max: o }), o === void 0 && s === 0) {
      (0, Br.checkStrictMode)(i, '"minContains" == 0 without "maxContains": "contains" keyword ignored');
      return;
    }
    if (o !== void 0 && s > o) {
      (0, Br.checkStrictMode)(i, '"minContains" > "maxContains" is always invalid'), e.fail();
      return;
    }
    if ((0, Br.alwaysValidSchema)(i, n)) {
      let m = (0, Ze._)`${c} >= ${s}`;
...
const Vr = ae, st = ee, Ex = Xe, Hr = K, $x = {
  ...
  code(e) {
    ...
    if (s.props = !0, l.removeAdditional !== "all" && (0, Hr.alwaysValidSchema)(s, n))
      return;
    const u = (0, Vr.allSchemaProperties)(r.properties), c = (0, Vr.allSchemaProperties)(r.patternProperties);
    p(), e.ok((0, st._)`${i} === ${Ex.default.errors}`);
    function p() {
      t.forIn("key", a, (v) => {
    ...
    function f(v) {
      let d;
      if (u.length > 8) {
        const y = (0, Hr.schemaRefOrVal)(s, r.properties, "properties");
        d = (0, Vr.isOwnProperty)(t, y, v);
      } else u.length ? d = (0, st.or)(...u.map((y) => (0, st._)`${v} === ${y}`)) : d = st.nil;
      return c.length && (d = (0, st.or)(d, ...c.map((y) => (0, st._)`${(0, Vr.usePattern)(e, y)}.test(${v})`))), (0, st.not)(d);
    }
    function h(v) {
      t.code((0, st._)`delete ${a}[${v}]`);
    }
    function g(v) {
      ...
      if (typeof n == "object" && !(0, Hr.alwaysValidSchema)(s, n)) {
        const d = t.name("valid");
        l.removeAdditional === "failing" ? (m(v, d, !1), t.if((0, st.not)(d), () => {
          e.reset(), h(v);
        })) : (m(v, d), o || t.if((0, st.not)(d), () => t.break()));
      }
    }
    function m(v, d, y) {
      const _ = {
...
const Tx = et, _l = ae, Ri = K, wl = Ua, Rx = {
  keyword: "properties",
  type: "object",
  schemaType: "object",
  code(e) {
    const { gen: t, schema: n, parentSchema: r, data: a, it: i } = e;
    i.opts.removeAdditional === "all" && r.additionalProperties === void 0 && wl.default.code(new Tx.KeywordCxt(i, wl.default, "additionalProperties"));
    const s = (0, _l.allSchemaProperties)(n);
    for (const p of s)
    ...
    for (const p of o)
      u(p) ? c(p) : (t.if((0, _l.propertyInData)(t, a, p, i.opts.ownProperties)), c(p), i.allErrors || t.else().var(l, !0), t.endIf()), e.it.definedProperties.add(p), e.ok(l);
    function u(p) {
...
const El = ae, Gr = ee, $l = K, Sl = K, Ox = {
  keyword: "patternProperties",
  type: "object",
  schemaType: "object",
  code(e) {
    const { gen: t, schema: n, data: r, parentSchema: a, it: i } = e, { opts: s } = i, o = (0, El.allSchemaProperties)(n), l = o.filter((m) => (0, $l.alwaysValidSchema)(i, n[m]));
    ...
    i.props !== !0 && !(i.props instanceof Gr.Name) && (i.props = (0, Sl.evaluatedPropsToName)(t, i.props));
    const { props: p } = i;
    f();
    function f() {
      for (const m of o)
    ...
    function h(m) {
      for (const v in u)
        new RegExp(m).test(v) && (0, $l.checkStrictMode)(i, `property ${v} matches pattern ${m} (use allowMatchingProperties)`);
    }
    function g(m) {
      t.forIn("key", r, (v) => {
        t.if((0, Gr._)`${(0, El.usePattern)(e, m)}.test(${v})`, () => {
          const d = l.includes(m);
          d || e.subschema({
            keyword: "patternProperties",
            schemaProp: m,
            dataProp: v,
            dataPropType: Sl.Type.Str
          }, c), i.opts.unevaluated && p !== !0 ? t.assign((0, Gr._)`${p}[${v}]`, !0) : !d && !i.allErrors && t.if((0, Gr.not)(c), () => t.break());
        });
      });
    }
  }
...
const ua = ee, Ix = K, jx = {
  ...
  code(e) {
    ...
    function u() {
      i.forEach((c, p) => {
        let f;
        (0, Ix.alwaysValidSchema)(a, c) ? t.var(l, !0) : f = e.subschema({
          keyword: "oneOf",
          schemaProp: p,
          compositeRule: !0
        }, l), p > 0 && t.if((0, ua._)`${l} && ${s}`).assign(s, !1).assign(o, (0, ua._)`[${o}, ${p}]`).else(), t.if(l, () => {
          t.assign(s, !0), t.assign(o, p), f && e.mergeEvaluated(f, ua.Name);
        });
      });
    }
  }
...
const wa = ee, Id = K, Fx = {
  ...
  code(e) {
    ...
    n.then === void 0 && n.else === void 0 && (0, Id.checkStrictMode)(r, '"if" without "then" and "else" is ignored');
    const a = Tl(r, "then"), i = Tl(r, "else");
    if (!a && !i)
      return;
    ...
    } else a ? t.if(o, u("then")) : t.if((0, wa.not)(o), u("else"));
    e.pass(s, () => e.error(!0));
    function l() {
      const c = e.subschema({
        keyword: "if",
    ...
    function u(c, p) {
      return () => {
        ...
        t.assign(s, o), e.mergeValidEvaluated(f, s), p ? t.assign(p, (0, wa._)`${c}`) : e.setParams({ ifClause: c });
      };
    }
  }
};
function Tl(e, t) {
  const n = e.schema[t];
  return n !== void 0 && !(0, Id.alwaysValidSchema)(e, n);
}
No.default = Ux;
var Io = {};
...
const Mx = K, zx = {
  ...
  code({ keyword: e, parentSchema: t, it: n }) {
    t.if === void 0 && (0, Mx.checkStrictMode)(n, `"${e}" without "if" is ignored`);
  }
};
Io.default = zx;
Object.defineProperty(Da, "__esModule", { value: !0 });
const qx = zn, Bx = wo, Vx = qn, Hx = Eo, Gx = $o, Kx = Fa, Xx = So, Wx = Ua, Yx = To, Jx = Ro, Qx = Oo, Zx = Ao, eb = Po, tb = ko, nb = No, rb = Io;
function ab(e = !1) {
  const t = [
    // any
    Qx.default,
  ...
  return e ? t.push(Bx.default, Hx.default) : t.push(qx.default, Vx.default), t.push(Gx.default), t;
}
Da.default = ab;
var jo = {}, Bn = {};
Object.defineProperty(Bn, "__esModule", { value: !0 });
Bn.dynamicAnchor = void 0;
const Oi = ee, ib = Xe, Rl = Le, sb = $t, ob = {
  keyword: "$dynamicAnchor",
  schemaType: "string",
  code: (e) => jd(e, e.schema)
};
function jd(e, t) {
  const { gen: n, it: r } = e;
  r.schemaEnv.root.dynamicAnchors[t] = !0;
  const a = (0, Oi._)`${ib.default.dynamicAnchors}${(0, Oi.getProperty)(t)}`, i = r.errSchemaPath === "#" ? r.validateName : cb(e);
  n.if((0, Oi._)`!${a}`, () => n.assign(a, i));
}
Bn.dynamicAnchor = jd;
function cb(e) {
  const { schemaEnv: t, schema: n, self: r } = e.it, { root: a, baseId: i, localRefs: s, meta: o } = t.root, { schemaId: l } = r.opts, u = new Rl.SchemaEnv({ schema: n, schemaId: l, root: a, baseId: i, localRefs: s, meta: o });
  return Rl.compileSchema.call(r, u), (0, sb.getValidate)(e, u);
}
Bn.default = ob;
var Vn = {};
Object.defineProperty(Vn, "__esModule", { value: !0 });
Vn.dynamicRef = void 0;
const Ol = ee, lb = Xe, Al = $t, ub = {
  keyword: "$dynamicRef",
  schemaType: "string",
  code: (e) => Cd(e, e.schema)
};
function Cd(e, t) {
  ...
  function s(l) {
    if (a.schemaEnv.root.dynamicAnchors[i]) {
      const u = n.let("_v", (0, Ol._)`${lb.default.dynamicAnchors}${(0, Ol.getProperty)(i)}`);
...
const pb = Bn, db = K, fb = {
  ...
  schemaType: "boolean",
  code(e) {
    e.schema ? (0, pb.dynamicAnchor)(e, "") : (0, db.checkStrictMode)(e.it, "$recursiveAnchor: false is ignored");
  }
};
Co.default = fb;
var Lo = {};
...
const mb = Vn, hb = {
  ...
  schemaType: "string",
  code: (e) => (0, mb.dynamicRef)(e, e.schema)
};
Lo.default = hb;
Object.defineProperty(jo, "__esModule", { value: !0 });
const vb = Bn, yb = Vn, gb = Co, xb = Lo, bb = [vb.default, yb.default, gb.default, xb.default];
jo.default = bb;
var Do = {}, Fo = {};
Object.defineProperty(Fo, "__esModule", { value: !0 });
const Pl = Fa, _b = {
  ...
  schemaType: "object",
  error: Pl.error,
  code: (e) => (0, Pl.validatePropertyDeps)(e)
};
Fo.default = _b;
var Uo = {};
Object.defineProperty(Uo, "__esModule", { value: !0 });
const wb = Fa, Eb = {
  ...
  type: "object",
  schemaType: "object",
  code: (e) => (0, wb.validateSchemaDeps)(e)
};
Uo.default = Eb;
var Mo = {};
Object.defineProperty(Mo, "__esModule", { value: !0 });
const $b = K, Sb = {
  ...
  schemaType: "number",
  code({ keyword: e, parentSchema: t, it: n }) {
    t.contains === void 0 && (0, $b.checkStrictMode)(n, `"${e}" without "contains" is ignored`);
  }
};
Mo.default = Sb;
Object.defineProperty(Do, "__esModule", { value: !0 });
const Tb = Fo, Rb = Uo, Ob = Mo, Ab = [Tb.default, Rb.default, Ob.default];
Do.default = Ab;
var zo = {}, qo = {};
Object.defineProperty(qo, "__esModule", { value: !0 });
const kt = ee, kl = K, Pb = Xe, kb = {
  ...
  code(e) {
    ...
    o instanceof kt.Name ? t.if((0, kt._)`${o} !== true`, () => t.forIn("key", r, (p) => t.if(u(o, p), () => l(p)))) : o !== !0 && t.forIn("key", r, (p) => o === void 0 ? l(p) : t.if(c(o, p), () => l(p))), i.props = !0, e.ok((0, kt._)`${a} === ${Pb.default.errors}`);
    function l(p) {
      if (n === !1) {
        e.setParams({ unevaluatedProperty: p }), e.error(), s || t.break();
        return;
      }
      if (!(0, kl.alwaysValidSchema)(i, n)) {
...
const Xt = ee, Nl = K, Ib = {
  ...
  code(e) {
    ...
    if (i === !0)
      return;
    const s = t.const("len", (0, Xt._)`${r}.length`);
    if (n === !1)
      e.setParams({ len: i }), e.fail((0, Xt._)`${s} > ${i}`);
    else if (typeof n == "object" && !(0, Nl.alwaysValidSchema)(a, n)) {
      const l = t.var("valid", (0, Xt._)`${s} <= ${i}`);
      t.if((0, Xt.not)(l), () => o(l, i)), e.ok(l);
    }
    a.items = !0;
    function o(l, u) {
      t.forRange("i", u, s, (c) => {
        e.subschema({ keyword: "unevaluatedItems", dataProp: c, dataPropType: Nl.Type.Num }, l), a.allErrors || t.if((0, Xt.not)(l), () => t.break());
      });
    }
  }
};
...
const Cb = qo, Lb = Bo, Db = [Cb.default, Lb.default];
zo.default = Db;
var Ma = {}, Vo = {};
Object.defineProperty(Vo, "__esModule", { value: !0 });
const be = ee, Fb = {
  ...
  code(e, t) {
    ...
    function f() {
      ...
      function _() {
        const x = c.$async ? (0, be._)`(${m}.async ? await ${d}(${r}) : ${d}(${r}))` : (0, be._)`${d}(${r})`, w = (0, be._)`(typeof ${d} == "function" ? ${x} : ${d}.test(${r}))`;
        return (0, be._)`${d} && ${d} !== true && ${v} === ${t} && !${w}`;
      }
    }
    function h() {
      const g = p.formats[i];
      ...
      function _(w) {
        const I = w instanceof RegExp ? (0, be.regexpCode)(w) : l.code.formats ? (0, be._)`${l.code.formats}${(0, be.getProperty)(i)}` : void 0, C = n.scopeValue("formats", { key: i, ref: w, code: I });
        return typeof w == "object" && !(w instanceof RegExp) ? [w.type || "string", w.validate, (0, be._)`${C}.validate`] : ["string", w, C];
      }
      function x() {
...
Object.defineProperty(co, "__esModule", { value: !0 });
const qb = Ca, Bb = La, Vb = Da, Hb = jo, Gb = Do, Kb = zo, Xb = Ma, Il = on, Wb = [
  Hb.default,
  qb.default,
  Bb.default,
  (0, Vb.default)(!0),
  Xb.default,
  Il.metadataVocabulary,
  Il.contentVocabulary,
  Gb.default,
  Kb.default
];
co.default = Wb;
...
const wn = ee, _s = qa, Cl = Le, Yb = dn, Jb = K, Qb = {
  message: ({ params: { discrError: e, tagName: t } }) => e === _s.DiscrError.Tag ? `tag "${t}" must be string` : `value of tag "${t}" must be in oneOf`,
  ...
  code(e) {
    ...
    t.if((0, wn._)`typeof ${u} == "string"`, () => c(), () => e.error(!1, { discrError: _s.DiscrError.Tag, tag: u, tagName: o })), e.ok(l);
    function c() {
      const h = f();
      t.if(!1);
      for (const g in h)
        t.elseIf((0, wn._)`${u} === ${g}`), t.assign(l, p(h[g]));
      t.else(), e.error(!1, { discrError: _s.DiscrError.Mapping, tag: u, tagName: o }), t.endIf();
    }
    function p(h) {
    ...
    function f() {
      ...
      for (let x = 0; x < s.length; x++) {
        let w = s[x];
        if (w != null && w.$ref && !(0, Jb.schemaHasRulesButRef)(w, i.self.RULES)) {
          const C = w.$ref;
          if (w = Cl.resolveRef.call(i.self, i.schemaEnv.root, i.baseId, C), w instanceof Cl.SchemaEnv && (w = w.schema), w === void 0)
            throw new Yb.default(i.opts.uriResolver, i.baseId, C);
        }
        const I = (h = w == null ? void 0 : w.properties) === null || h === void 0 ? void 0 : h[o];
        if (typeof I != "object")
...
za.default = Zb;
var Ho = {};
const e0 = "https://json-schema.org/draft/2020-12/schema", t0 = "https://json-schema.org/draft/2020-12/schema", n0 = {
...
}, r0 = "meta", a0 = "Core and Validation specifications meta-schema", i0 = [
  {
    $ref: "meta/core"
  },
  {
  ...
  "object",
  "boolean"
], o0 = "This meta-schema also defines keywords that have appeared in previous drafts in order to prevent incompatible extensions as they remain in common use.", c0 = {
  definitions: {
  ...
  $comment: o0,
  properties: c0
}, u0 = "https://json-schema.org/draft/2020-12/schema", p0 = "https://json-schema.org/draft/2020-12/meta/applicator", d0 = {
  "https://json-schema.org/draft/2020-12/vocab/applicator": !0
}, f0 = "meta", m0 = "Applicator vocabulary meta-schema", h0 = [
  "object",
  "boolean"
], v0 = {
  prefixItems: {
  ...
  properties: v0,
  $defs: y0
}, x0 = "https://json-schema.org/draft/2020-12/schema", b0 = "https://json-schema.org/draft/2020-12/meta/unevaluated", _0 = {
  "https://json-schema.org/draft/2020-12/vocab/unevaluated": !0
}, w0 = "meta", E0 = "Unevaluated applicator vocabulary meta-schema", $0 = [
  "object",
  "boolean"
], S0 = {
  unevaluatedItems: {
  ...
  type: $0,
  properties: S0
}, R0 = "https://json-schema.org/draft/2020-12/schema", O0 = "https://json-schema.org/draft/2020-12/meta/content", A0 = {
  "https://json-schema.org/draft/2020-12/vocab/content": !0
}, P0 = "meta", k0 = "Content vocabulary meta-schema", N0 = [
  "object",
  "boolean"
], I0 = {
  contentEncoding: {
  ...
  type: N0,
  properties: I0
}, C0 = "https://json-schema.org/draft/2020-12/schema", L0 = "https://json-schema.org/draft/2020-12/meta/core", D0 = {
  "https://json-schema.org/draft/2020-12/vocab/core": !0
}, F0 = "meta", U0 = "Core vocabulary meta-schema", M0 = [
  "object",
  "boolean"
], z0 = {
  $id: {
  ...
  properties: z0,
  $defs: q0
}, V0 = "https://json-schema.org/draft/2020-12/schema", H0 = "https://json-schema.org/draft/2020-12/meta/format-annotation", G0 = {
  "https://json-schema.org/draft/2020-12/vocab/format-annotation": !0
}, K0 = "meta", X0 = "Format vocabulary meta-schema for annotation results", W0 = [
  "object",
  "boolean"
], Y0 = {
  format: {
  ...
  type: W0,
  properties: Y0
}, Q0 = "https://json-schema.org/draft/2020-12/schema", Z0 = "https://json-schema.org/draft/2020-12/meta/meta-data", e_ = {
  "https://json-schema.org/draft/2020-12/vocab/meta-data": !0
}, t_ = "meta", n_ = "Meta-data vocabulary meta-schema", r_ = [
  "object",
  "boolean"
], a_ = {
  title: {
  ...
  type: r_,
  properties: a_
}, s_ = "https://json-schema.org/draft/2020-12/schema", o_ = "https://json-schema.org/draft/2020-12/meta/validation", c_ = {
  "https://json-schema.org/draft/2020-12/vocab/validation": !0
}, l_ = "meta", u_ = "Validation vocabulary meta-schema", p_ = [
  "object",
  "boolean"
], d_ = {
  type: {
...
Object.defineProperty(Ho, "__esModule", { value: !0 });
const h_ = l0, v_ = g0, y_ = T0, g_ = j0, x_ = B0, b_ = J0, __ = i_, w_ = m_, E_ = ["/properties"];
function $_(e) {
  return [
    h_,
    v_,
  ...
  function t(n, r) {
    return e ? n.$dataMetaSchema(r, E_) : r;
  }
}
Ho.default = $_;
(function(e, t) {
  Object.defineProperty(t, "__esModule", { value: !0 }), t.MissingRefError = t.ValidationError = t.CodeGen = t.Name = t.nil = t.stringify = t.str = t._ = t.KeywordCxt = t.Ajv2020 = void 0;
  const n = Ks, r = co, a = za, i = Ho, s = "https://json-schema.org/draft/2020-12/schema";
...
var S_ = ms.exports, ws = { exports: {} }, Ld = {};
(function(e) {
  ...
  const x = -2147483648, w = 2 ** 31 - 1;
  function I(O) {
    return Number.isInteger(O) && O <= w && O >= x;
  }
  function C(O) {
...
var Dd = {}, Es = { exports: {} }, Go = {};
Object.defineProperty(Go, "__esModule", { value: !0 });
const T_ = Ca, R_ = La, O_ = Da, A_ = Ma, Ll = on, P_ = [
  T_.default,
  R_.default,
  (0, O_.default)(),
  A_.default,
...
Go.default = P_;
const k_ = "http://json-schema.org/draft-07/schema#", N_ = "http://json-schema.org/draft-07/schema#", I_ = "Core schema meta-schema", j_ = {
  schemaArray: {
    type: "array",
    minItems: 1,
    items: {
...
(function(e, t) {
  ...
  const n = Ks, r = Go, a = za, i = D_, s = ["/properties"], o = "http://json-schema.org/draft-07/schema";
...
(function(e) {
  ...
  const t = F_, n = ee, r = n.operators, a = {
...
(function(e, t) {
  Object.defineProperty(t, "__esModule", { value: !0 });
  const n = Ld, r = Dd, a = ee, i = new a.Name("fullFormats"), s = new a.Name("fastFormats"), o = (u, c = { keywords: !0 }) => {
    if (Array.isArray(c))
...
var U_ = ws.exports;
const M_ = /* @__PURE__ */ Un(U_), z_ = (e, t, n, r) => {
  ...
  !q_(a, i) && r || Object.defineProperty(e, n, i);
}, q_ = function(e, t) {
...
${t}`, H_ = Object.getOwnPropertyDescriptor(Function.prototype, "toString"), G_ = Object.getOwnPropertyDescriptor(Function.prototype.toString, "name"), K_ = (e, t, n) => {
  const r = n === "" ? "" : `with ${n.trim()}() `, a = V_.bind(null, r, t.toString());
  Object.defineProperty(a, "name", G_);
  const { writable: i, enumerable: s, configurable: o } = H_;
  Object.defineProperty(e, "toString", { value: a, writable: i, enumerable: s, configurable: o });
};
function X_(e, t, { ignoreNonConfigurable: n = !1 } = {}) {
  const { name: r } = e;
  for (const a of Reflect.ownKeys(t))
    z_(e, t, a, n);
  return B_(e, t), K_(e, t, r), e;
}
const Dl = (e, t = {}) => {
  ...
  return X_(u, e), u.cancel = () => {
    s && (clearTimeout(s), s = void 0), o && (clearTimeout(o), o = void 0);
  }, u;
};
var $s = { exports: {} };
const W_ = "2.0.0", Fd = 256, Y_ = Number.MAX_SAFE_INTEGER || /* istanbul ignore next */
9007199254740991, J_ = 16, Q_ = Fd - 6, Z_ = [
  "major",
  "premajor",
  "minor",
  "preminor",
...
var Sr = $s.exports;
const tw = Object.freeze({ loose: !0 }), nw = Object.freeze({}), rw = (e) => e ? typeof e != "object" ? tw : e : nw;
var Ko = rw;
const Fl = /^[0-9]+$/, Ud = (e, t) => {
  if (typeof e == "number" && typeof t == "number")
    return e === t ? 0 : e < t ? -1 : 1;
  const n = Fl.test(e), r = Fl.test(t);
  return n && r && (e = +e, t = +t), e === t ? 0 : n && !r ? -1 : r && !n ? 1 : e < t ? -1 : 1;
}, aw = (e, t) => Ud(t, e);
var Md = {
  compareIdentifiers: Ud,
  rcompareIdentifiers: aw
};
const Kr = Ba, { MAX_LENGTH: Ul, MAX_SAFE_INTEGER: Xr } = $r, { safeRe: Wr, t: Yr } = Sr, iw = Ko, { compareIdentifiers: Ai } = Md;
let sw = class dt {
  constructor(t, n) {
    if (n = iw(n), t instanceof dt) {
  ...
  compare(t) {
    if (Kr("SemVer.compare", this.version, this.options, t), !(t instanceof dt)) {
  ...
  comparePre(t) {
    ...
    do {
      ...
      if (Kr("prerelease compare", n, r, a), r === void 0 && a === void 0)
        return 0;
      if (a === void 0)
        return 1;
      if (r === void 0)
  ...
  compareBuild(t) {
    ...
    do {
      ...
      if (Kr("build compare", n, r, a), r === void 0 && a === void 0)
        return 0;
      if (a === void 0)
        return 1;
      if (r === void 0)
  ...
  inc(t, n, r) {
    if (t.startsWith("pre")) {
      ...
      if (n) {
        const a = `-${n}`.match(this.options.loose ? Wr[Yr.PRERELEASELOOSE] : Wr[Yr.PRERELEASE]);
        if (!a || a[1] !== n)
...
var uw = lw;
const pw = fn, dw = (e, t) => {
  const n = pw(e.trim().replace(/^[=v]+/, ""), t);
  return n ? n.version : null;
};
var fw = dw;
const zl = je, mw = (e, t, n, r, a) => {
  typeof n == "string" && (a = r, r = n, n = void 0);
  try {
    return new zl(
      e instanceof zl ? e.version : e,
      n
    ).inc(t, r, a).version;
  } catch {
    return null;
...
var hw = mw;
const ql = fn, vw = (e, t) => {
  const n = ql(e, null, !0), r = ql(t, null, !0), a = n.compare(r);
  if (a === 0)
    return null;
...
var yw = vw;
const gw = je, xw = (e, t) => new gw(e, t).major;
var bw = xw;
const _w = je, ww = (e, t) => new _w(e, t).minor;
var Ew = ww;
const $w = je, Sw = (e, t) => new $w(e, t).patch;
var Tw = Sw;
const Rw = fn, Ow = (e, t) => {
  const n = Rw(e, t);
...
var Aw = Ow;
const Bl = je, Pw = (e, t, n) => new Bl(e, n).compare(new Bl(t, n));
var ct = Pw;
const kw = ct, Nw = (e, t, n) => kw(t, e, n);
var Iw = Nw;
const jw = ct, Cw = (e, t) => jw(e, t, !0);
var Lw = Cw;
const Vl = je, Dw = (e, t, n) => {
  const r = new Vl(e, n), a = new Vl(t, n);
  return r.compare(a) || r.compareBuild(a);
};
var Xo = Dw;
const Fw = Xo, Uw = (e, t) => e.sort((n, r) => Fw(n, r, t));
var Mw = Uw;
const zw = Xo, qw = (e, t) => e.sort((n, r) => zw(r, n, t));
var Bw = qw;
const Vw = ct, Hw = (e, t, n) => Vw(e, t, n) > 0;
var Va = Hw;
const Gw = ct, Kw = (e, t, n) => Gw(e, t, n) < 0;
var Wo = Kw;
const Xw = ct, Ww = (e, t, n) => Xw(e, t, n) === 0;
var zd = Ww;
const Yw = ct, Jw = (e, t, n) => Yw(e, t, n) !== 0;
var qd = Jw;
const Qw = ct, Zw = (e, t, n) => Qw(e, t, n) >= 0;
var Yo = Zw;
const eE = ct, tE = (e, t, n) => eE(e, t, n) <= 0;
var Jo = tE;
const nE = zd, rE = qd, aE = Va, iE = Yo, sE = Wo, oE = Jo, cE = (e, t, n, r) => {
  switch (t) {
    case "===":
...
var Bd = cE;
const lE = je, uE = fn, { safeRe: Jr, t: Qr } = Sr, pE = (e, t) => {
  if (e instanceof lE)
    return e;
  ...
  let n = null;
  if (!t.rtl)
    n = e.match(t.includePrerelease ? Jr[Qr.COERCEFULL] : Jr[Qr.COERCE]);
  else {
    const l = t.includePrerelease ? Jr[Qr.COERCERTLFULL] : Jr[Qr.COERCERTL];
    let u;
  ...
  return uE(`${r}.${a}.${i}${s}${o}`, t);
};
var dE = pE;
const fE = fn, mE = $r, hE = je, vE = (e, t, n) => {
  if (!mE.RELEASE_TYPES.includes(t))
    return null;
  const r = yE(e, n);
  return r && gE(r, t);
}, yE = (e, t) => {
  const n = e instanceof hE ? e.version : e;
  return fE(n, t);
}, gE = (e, t) => {
  if (xE(t))
    return e.version;
...
function lt() {
  ...
  class t {
    ...
    parseRange(F) {
      ...
      let $ = F.split(" ").map((E) => y(E, this.options)).join(" ").split(/\s+/).map((E) => O(E, this.options));
      B && ($ = $.filter((E) => (s("loose invalid filter", E, this.options), !!E.match(l[u.COMPARATORLOOSE])))), s("range list", $);
      const N = /* @__PURE__ */ new Map(), T = $.map((E) => new i(E, this.options));
      for (const E of T) {
        if (m(E))
          return [E];
        N.set(E.value, E);
  ...
  Pi = t;
  const n = wE, r = new n(), a = Ko, i = Ha(), s = Ba, o = je, {
    safeRe: l,
    t: u,
    comparatorTrimReplace: c,
    tildeTrimReplace: p,
    caretTrimReplace: f
  } = Sr, { FLAG_INCLUDE_PRERELEASE: h, FLAG_LOOSE: g } = $r, m = (P) => P.value === "<0.0.0-0", v = (P) => P.value === "", d = (P, F) => {
    let H = !0;
    const j = P.slice();
    let U = j.pop();
    for (; H && j.length; )
    ...
    return H;
  }, y = (P, F) => (P = P.replace(l[u.BUILD], ""), s("comp", P, F), P = I(P, F), s("caret", P), P = x(P, F), s("tildes", P), P = D(P, F), s("xrange", P), P = R(P, F), s("stars", P), P), _ = (P) => !P || P.toLowerCase() === "x" || P === "*", x = (P, F) => P.trim().split(/\s+/).map((H) => w(H, F)).join(" "), w = (P, F) => {
    ...
    return P.replace(H, (j, U, B, L, $, N) => {
      ...
      const T = _(B), b = T || _(L), E = b || _($), M = E;
      return U === "=" && M && (U = ""), N = F.includePrerelease ? "-0" : "", T ? U === ">" || U === "<" ? j = "<0.0.0-0" : j = "*" : U && M ? (b && (L = 0), $ = 0, U === ">" ? (U = ">=", b ? (B = +B + 1, L = 0, $ = 0) : (L = +L + 1, $ = 0)) : U === "<=" && (U = "<", b ? B = +B + 1 : L = +L + 1), U === "<" && (N = "-0"), j = `${U + B}.${L}.${$}${N}`) : b ? j = `>=${B}.0.0${N} <${+B + 1}.0.0-0` : E && (j = `>=${B}.${L}.0${N} <${B}.${+L + 1}.0-0`), s("xRange return", j), j;
    });
  }, R = (P, F) => (s("replaceStars", P, F), P.trim().replace(l[u.STAR], "")), O = (P, F) => (s("replaceGTE0", P, F), P.trim().replace(l[F.includePrerelease ? u.GTE0PRE : u.GTE0], "")), k = (P) => (F, H, j, U, B, L, $, N, T, b, E, M) => (_(j) ? H = "" : _(U) ? H = `>=${j}.0.0${P ? "-0" : ""}` : _(B) ? H = `>=${j}.${U}.0${P ? "-0" : ""}` : L ? H = `>=${H}` : H = `>=${H}${P ? "-0" : ""}`, _(T) ? N = "" : _(b) ? N = `<${+T + 1}.0.0-0` : _(E) ? N = `<${T}.${+b + 1}.0-0` : M ? N = `<=${T}.${b}.${E}-${M}` : P ? N = `<${T}.${b}.${+E + 1}-0` : N = `<=${N}`, `${H} ${N}`.trim()), V = (P, F, H) => {
...
function Ha() {
  ...
  ki = t;
  const n = Ko, { safeRe: r, t: a } = Sr, i = Bd, s = Ba, o = je, l = lt();
  return ki;
}
const EE = lt(), $E = (e, t, n) => {
  try {
...
var Ga = $E;
const SE = lt(), TE = (e, t) => new SE(e, t).set.map((n) => n.map((r) => r.value).join(" ").trim().split(" "));
var RE = TE;
const OE = je, AE = lt(), PE = (e, t, n) => {
  ...
  return e.forEach((s) => {
    i.test(s) && (!r || a.compare(s) === -1) && (r = s, a = new OE(r, n));
  }), r;
};
var kE = PE;
const NE = je, IE = lt(), jE = (e, t, n) => {
  let r = null, a = null, i = null;
  try {
    i = new IE(t, n);
  } catch {
  ...
  return e.forEach((s) => {
    i.test(s) && (!r || a.compare(s) === 1) && (r = s, a = new NE(r, n));
  }), r;
};
var CE = jE;
const Ni = je, LE = lt(), Kl = Va, DE = (e, t) => {
  e = new LE(e, t);
  let n = new Ni("0.0.0");
...
var zE = ME;
const qE = je, Vd = Ha(), { ANY: BE } = Vd, VE = lt(), HE = Ga, Xl = Va, Wl = Wo, GE = Jo, KE = Yo, XE = (e, t, n, r) => {
  e = new qE(e, r), t = new VE(t, r);
  let a, i, s, o, l;
  switch (n) {
    case ">":
      a = Xl, i = GE, s = Wl, o = ">", l = ">=";
  ...
  for (let u = 0; u < t.set.length; ++u) {
    ...
    let p = null, f = null;
    if (c.forEach((h) => {
      h.semver === BE && (h = new Vd(">=0.0.0")), p = p || h, f = f || h, a(h.semver, p.semver, r) ? p = h : s(h.semver, f.semver, r) && (f = h);
...
var Qo = XE;
const WE = Qo, YE = (e, t, n) => WE(e, t, ">", n);
var JE = YE;
const QE = Qo, ZE = (e, t, n) => QE(e, t, "<", n);
var e$ = ZE;
const Yl = lt(), t$ = (e, t, n) => (e = new Yl(e, n), t = new Yl(t, n), e.intersects(t, n));
var n$ = t$;
const r$ = Ga, a$ = ct;
var i$ = (e, t, n) => {
  const r = [];
  let a = null, i = null;
  const s = e.sort((c, p) => a$(c, p, n));
  for (const c of s)
    r$(c, t, n) ? (i = c, a || (a = c)) : (i && r.push([a, i]), i = null, a = null);
  a && r.push([a, null]);
  const o = [];
  for (const [c, p] of r)
...
const Jl = lt(), Zo = Ha(), { ANY: Ii } = Zo, tr = Ga, ec = ct, s$ = (e, t, n = {}) => {
  if (e === t)
    return !0;
  e = new Jl(e, n), t = new Jl(t, n);
  let r = !1;
  ...
  return !0;
}, o$ = [new Zo(">=0.0.0-0")], Ql = [new Zo(">=0.0.0")], c$ = (e, t, n) => {
  if (e === t)
    return !0;
  if (e.length === 1 && e[0].semver === Ii) {
    if (t.length === 1 && t[0].semver === Ii)
      return !0;
    n.includePrerelease ? e = o$ : e = Ql;
  }
  if (t.length === 1 && t[0].semver === Ii) {
  ...
  let a, i;
  for (const h of e)
    h.operator === ">" || h.operator === ">=" ? a = Zl(a, h, n) : h.operator === "<" || h.operator === "<=" ? i = eu(i, h, n) : r.add(h.semver);
  if (r.size > 1)
    return null;
  let s;
  if (a && i) {
    if (s = ec(a.semver, i.semver, n), s > 0)
      return null;
    if (s === 0 && (a.operator !== ">=" || i.operator !== "<="))
      return null;
  }
  for (const h of r) {
    if (a && !tr(h, String(a), n) || i && !tr(h, String(i), n))
      return null;
    for (const g of t)
      if (!tr(h, String(g), n))
        return !1;
  ...
  for (const h of t) {
    if (c = c || h.operator === ">" || h.operator === ">=", u = u || h.operator === "<" || h.operator === "<=", a) {
      if (f && h.semver.prerelease && h.semver.prerelease.length && h.semver.major === f.major && h.semver.minor === f.minor && h.semver.patch === f.patch && (f = !1), h.operator === ">" || h.operator === ">=") {
        if (o = Zl(a, h, n), o === h && o !== a)
          return !1;
      } else if (a.operator === ">=" && !tr(a.semver, String(h), n))
        return !1;
    }
    if (i) {
      if (p && h.semver.prerelease && h.semver.prerelease.length && h.semver.major === p.major && h.semver.minor === p.minor && h.semver.patch === p.patch && (p = !1), h.operator === "<" || h.operator === "<=") {
        if (l = eu(i, h, n), l === h && l !== i)
          return !1;
      } else if (i.operator === "<=" && !tr(i.semver, String(h), n))
        return !1;
    }
  ...
  if (!e)
    return t;
  const r = ec(e.semver, t.semver, n);
...
var l$ = s$;
const ji = Sr, tu = $r, u$ = je, nu = Md, p$ = fn, d$ = uw, f$ = fw, m$ = hw, h$ = yw, v$ = bw, y$ = Ew, g$ = Tw, x$ = Aw, b$ = ct, _$ = Iw, w$ = Lw, E$ = Xo, $$ = Mw, S$ = Bw, T$ = Va, R$ = Wo, O$ = zd, A$ = qd, P$ = Yo, k$ = Jo, N$ = Bd, I$ = dE, j$ = bE, C$ = Ha(), L$ = lt(), D$ = Ga, F$ = RE, U$ = kE, M$ = CE, z$ = FE, q$ = zE, B$ = Qo, V$ = JE, H$ = e$, G$ = n$, K$ = i$, X$ = l$;
var W$ = {
  parse: p$,
  valid: d$,
  clean: f$,
  ...
  src: ji.src,
  tokens: ji.t,
  SEMVER_SPEC_VERSION: tu.SEMVER_SPEC_VERSION,
  RELEASE_TYPES: tu.RELEASE_TYPES,
  compareIdentifiers: nu.compareIdentifiers,
  rcompareIdentifiers: nu.rcompareIdentifiers
};
const xn = /* @__PURE__ */ Un(W$), Y$ = Object.prototype.toString, J$ = "[object Uint8Array]", Q$ = "[object ArrayBuffer]";
function Hd(e, t, n) {
  return e ? e.constructor === t ? !0 : Y$.call(e) === n : !1;
}
function Gd(e) {
  return Hd(e, Uint8Array, J$);
}
function Z$(e) {
  return Hd(e, ArrayBuffer, Q$);
}
function eS(e) {
  return Gd(e) || Z$(e);
}
...
function ea(e, t = "utf8") {
  return nS(e), Zr[t] ?? (Zr[t] = new globalThis.TextDecoder(t)), Zr[t].decode(e);
}
function rS(e) {
  if (typeof e != "string")
    throw new TypeError(`Expected \`string\`, got \`${typeof e}\``);
}
const aS = new globalThis.TextEncoder();
function Li(e) {
  return rS(e), aS.encode(e);
}
Array.from({ length: 256 }, (e, t) => t.toString(16).padStart(2, "0"));
const ru = "aes-256-cbc", Kd = /* @__PURE__ */ new Set([
  "aes-256-cbc",
  "aes-256-gcm",
  "aes-256-ctr"
]), iS = (e) => typeof e == "string" && Kd.has(e), bt = () => /* @__PURE__ */ Object.create(null), au = (e) => e !== void 0, Di = (e, t) => {
...
var jt, Ct, Jt, Ue, Qe, Qt, Zt, kn, ft, Se, Xd, Wd, Yd, Jd, Qd, Zd, ef, tf;
class sS {
  constructor(t = {}) {
    rt(this, Se);
    ce(this, "path");
    ...
    rt(this, kn);
    rt(this, ft);
    ce(this, "_deserialize", (t) => JSON.parse(t));
    ce(this, "_serialize", (t) => JSON.stringify(t, void 0, "	"));
    const n = yt(this, Se, Xd).call(this, t);
    Fe(this, Ue, n), yt(this, Se, Wd).call(this, n), yt(this, Se, Jd).call(this, n), yt(this, Se, Qd).call(this, n), this.events = new EventTarget(), Fe(this, Ct, n.encryptionKey), Fe(this, Jt, n.encryptionAlgorithm ?? ru), this.path = yt(this, Se, Zd).call(this, n), yt(this, Se, ef).call(this, n), n.watch && this._watch();
  }
  get(t, n) {
    if (te(this, Ue).accessPropertiesByDotNotation)
  ...
  set(t, n) {
    ...
    const { store: r } = this, a = (i, s) => {
      if (Di(i, s), te(this, Ue).accessPropertiesByDotNotation)
        Mr(r, i, s);
      else {
  ...
  has(t) {
    return te(this, Ue).accessPropertiesByDotNotation ? _i(this.store, t) : t in this.store;
  }
  appendToArray(t, n) {
    Di(t, n);
    const r = te(this, Ue).accessPropertiesByDotNotation ? this._get(t, []) : t in this.store ? this.store[t] : [];
    if (!Array.isArray(r))
  ...
  reset(...t) {
    for (const n of t)
      au(te(this, Qe)[n]) && this.set(n, te(this, Qe)[n]);
  }
  delete(t) {
    const { store: n } = this;
    te(this, Ue).accessPropertiesByDotNotation ? sh(n, t) : delete n[t], this.store = n;
  }
  /**
      Delete all items.
  ...
  clear() {
    ...
    for (const n of Object.keys(te(this, Qe)))
      au(te(this, Qe)[n]) && (Di(n, te(this, Qe)[n]), te(this, Ue).accessPropertiesByDotNotation ? Mr(t, n, te(this, Qe)[n]) : t[n] = te(this, Qe)[n]);
    this.store = t;
  }
  onDidChange(t, n) {
    if (typeof t != "string")
  ...
  get store() {
    ...
    try {
      ...
      return ((i) => {
        ...
        return te(this, Qt) || this._validate(s), Object.assign(bt(), s);
      })(r);
    } catch (n) {
      if ((n == null ? void 0 : n.code) === "ENOENT")
        return this._ensureDirectory(), bt();
      if (te(this, Ue).clearInvalidConfig) {
        const r = n;
  ...
  set store(t) {
    if (this._ensureDirectory(), !_i(t, Nt))
      try {
        ...
        _i(a, Nt) && Mr(t, Nt, nl(a, Nt));
      } catch {
      }
    te(this, Qt) || this._validate(t), this._write(t), this.events.dispatchEvent(new Event("change"));
  }
  *[Symbol.iterator]() {
  ...
  */
  _closeWatcher() {
    te(this, Zt) && (te(this, Zt).close(), Fe(this, Zt, void 0)), te(this, kn) && (ne.unwatchFile(this.path), Fe(this, kn, !1)), Fe(this, ft, void 0);
  }
  _decryptData(t) {
    const n = te(this, Ct);
    if (!n)
      return typeof t == "string" ? t : ea(t);
    const r = te(this, Jt), a = r === "aes-256-gcm" ? 16 : 0, i = ":".codePointAt(0), s = typeof t == "string" ? t.codePointAt(16) : t[16];
    if (!(i !== void 0 && s === i)) {
      if (r === "aes-256-cbc")
        return typeof t == "string" ? t : ea(t);
    ...
    const l = (h) => {
      ...
      const { ciphertext: g, authenticationTag: m } = l(p), v = Yn.pbkdf2Sync(n, h, 1e4, 32, "sha512"), d = Yn.createDecipheriv(r, v, u);
      return m && d.setAuthTag(m), ea(Ci([d.update(g), d.final()]));
    };
    try {
      return f(u);
    } catch {
  ...
  _ensureDirectory() {
    ne.mkdirSync(Z.dirname(this.path), { recursive: !0 });
  }
  _write(t) {
    let n = this._serialize(t);
    const r = te(this, Ct);
    if (r) {
      const a = Yn.randomBytes(16), i = Yn.pbkdf2Sync(r, a, 1e4, 32, "sha512"), s = Yn.createCipheriv(te(this, Jt), i, a), o = Ci([s.update(Li(n)), s.final()]), l = [a, Li(":"), o];
      te(this, Jt) === "aes-256-gcm" && l.push(s.getAuthTag()), n = Ci(l);
    }
    if (he.env.SNAP)
      ne.writeFileSync(this.path, n, { mode: te(this, Ue).configFileMode });
    else
      try {
        Ip(this.path, n, { mode: te(this, Ue).configFileMode });
      } catch (a) {
  ...
  _watch() {
    if (this._ensureDirectory(), ne.existsSync(this.path) || this._write(bt()), he.platform === "win32" || he.platform === "darwin") {
      te(this, ft) ?? Fe(this, ft, Dl(() => {
        this.events.dispatchEvent(new Event("change"));
      }, { wait: 100 }));
      const t = Z.dirname(this.path), n = Z.basename(this.path);
      Fe(this, Zt, ne.watch(t, { persistent: !1, encoding: "utf8" }, (r, a) => {
        a && a !== n || typeof te(this, ft) == "function" && te(this, ft).call(this);
      }));
    } else
      te(this, ft) ?? Fe(this, ft, Dl(() => {
        this.events.dispatchEvent(new Event("change"));
      }, { wait: 1e3 })), ne.watchFile(this.path, { persistent: !1 }, (t, n) => {
        typeof te(this, ft) == "function" && te(this, ft).call(this);
      }), Fe(this, kn, !0);
  }
  _migrate(t, n, r) {
    ...
    (this._isVersionInRangeFormat(a) || !xn.eq(a, n)) && this._set(Fi, n);
  }
  ...
  _isVersionInRangeFormat(t) {
    return xn.clean(t) === null;
  }
  _shouldPerformMigration(t, n, r) {
    return this._isVersionInRangeFormat(t) ? n !== "0.0.0" && xn.satisfies(n, t) ? !1 : xn.satisfies(r, t) : !(xn.lte(t, n) || xn.gt(t, r));
  }
  _get(t, n) {
    return nl(this.store, t, n);
  }
  _set(t, n) {
    const { store: r } = this;
    Mr(r, t, n), this.store = r;
  }
}
jt = new WeakMap(), Ct = new WeakMap(), Jt = new WeakMap(), Ue = new WeakMap(), Qe = new WeakMap(), Qt = new WeakMap(), Zt = new WeakMap(), kn = new WeakMap(), ft = new WeakMap(), Se = new WeakSet(), Xd = function(t) {
  const n = {
    configName: "config",
    fileExtension: "json",
  ...
  if (n.encryptionAlgorithm ?? (n.encryptionAlgorithm = ru), !iS(n.encryptionAlgorithm))
    throw new TypeError(`The \`encryptionAlgorithm\` option must be one of: ${[...Kd].join(", ")}`);
  if (!n.cwd) {
    if (!n.projectName)
      throw new Error("Please specify the `projectName` option.");
    n.cwd = uh(n.projectName, { suffix: n.projectSuffix }).config;
  }
  ...
  const n = M_.default, r = new S_.Ajv2020({
    allErrors: !0,
    useDefaults: !0,
    ...t.ajvOptions
  });
  ...
  Fe(this, jt, r.compile(a)), yt(this, Se, Yd).call(this, t.schema);
}, Yd = function(t) {
  ...
  return Z.resolve(t.cwd, `${t.configName ?? "config"}${r}`);
}, ef = function(t) {
  if (t.migrations) {
    yt(this, Se, tf).call(this, t), this._validate(this.store);
    return;
  }
...
const { app: pa, ipcMain: Ss, shell: oS } = dp;
let iu = !1;
const su = () => {
  if (!Ss || !pa)
  ...
  return iu || (Ss.on("electron-store-get-data", (t) => {
    t.returnValue = e;
  }), iu = !0), e;
};
class cS extends sS {
  constructor(t) {
    let n, r;
    if (he.type === "renderer") {
      const a = dp.ipcRenderer.sendSync("electron-store-get-data");
      if (!a)
        throw new Error("Electron Store: You need to call `.initRenderer()` from the main process.");
      ({ defaultCwd: n, appVersion: r } = a);
    } else Ss && pa && ({ defaultCwd: n, appVersion: r } = su());
    t = {
      name: "config",
      ...t
    }, t.projectVersion || (t.projectVersion = r), t.cwd ? t.cwd = Z.isAbsolute(t.cwd) ? t.cwd : Z.join(n, t.cwd) : t.cwd = n, t.configName = t.name, delete t.name, super(t);
  }
  static initRenderer() {
    su();
  }
...
const mn = new cS(), nf = [5, 10, 15, 30, 45, 60], ou = [
  ...nf,
  null
], lS = [5, 10, 15, 30, 60], uS = ["primary", "all"], rf = {
  general: {
...
function tc(e, t) {
  ...
  for (const r of Object.keys(t)) {
    const a = t[r];
    a !== void 0 && (cu(n[r]) && cu(a) ? n[r] = tc(n[r], a) : n[r] = a);
  }
  return n;
}
function nr(e, t, n) {
...
function af(e) {
  nr(
    ...
    ou
  ), Ke("take screenshots", e.general.takeScreenshots), Ke("take camshots", e.general.takeCamshots), Ke(
    ...
    lS
  ), Ke("randomized tracking", e.advanced.randomizedTracking), Ke(
  ...
  ), Ke("count keyboard hits", e.trackingSources.countKeyboardHits), Ke("count mouse clicks", e.trackingSources.countMouseClicks), nr(
    "screenshots source",
    e.trackingSources.screenshotsFrom,
    uS
  ), Ke("play sounds", e.other.playSounds), Ke("show dock icon", e.other.showDockIcon), Ke("open at login", e.other.openAtLogin);
}
function nc(e) {
  if (ze.isPackaged || e.other.openAtLogin)
    try {
      ze.setLoginItemSettings({
        openAtLogin: e.other.openAtLogin
      });
    } catch (t) {
      console.warn("Unable to update login item settings:", t);
    }
  process.platform === "darwin" && ze.dock && (e.other.showDockIcon ? ze.dock.show() : ze.dock.hide());
}
function _t() {
  const e = tc(rf, mn.get("config"));
  return af(e), e;
}
function pS(e) {
  const t = tc(_t(), e);
  return af(t), mn.set("config", t), nc(t), t;
}
function dS() {
  const e = structuredClone(rf);
  return mn.set("config", e), nc(e), e;
}
function fS(e) {
  return e.trim().replace(/\/+$/, "");
}
function mS() {
  const e = mn.get("credentials");
  if (!e)
    throw new Error("Please login first.");
  return e;
}
function hS(e) {
  mn.set("credentials", e);
}
function vS() {
  mn.delete("credentials");
}
function yS(e) {
  return e.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");
}
function sf(e, t) {
  return t.general.reviewImagesBeforeUpload ? new Promise((n) => {
    const r = `image-review-${Date.now()}-${Math.random().toString(36).slice(2)}`, a = gm.getPrimaryDisplay(), i = 320, s = 260, o = 16, l = new Mt({
      width: i,
      height: s,
      x: a.workArea.x + a.workArea.width - i - o,
      y: a.workArea.y + o,
      frame: !1,
      resizable: !1,
      alwaysOnTop: !0,
    ...
    let u = !1;
    function c() {
      le.removeListener("image-review:allow", f), le.removeListener("image-review:reject", h);
    }
    function p(g) {
      u || (u = !0, c(), n(g), l.isDestroyed() || l.close());
    }
    function f(g, m) {
      m.reviewId === r && p(!0);
    }
    ...
    le.on("image-review:allow", f), le.on("image-review:reject", h), l.on("closed", () => {
      p(!0);
    }), setTimeout(() => {
      p(!0);
    }, t.advanced.screenshotReviewSeconds * 1e3), l.loadURL(
        ...
        <html>
          ...
          <body>
            ...
            <script>
              ...
              const reviewId = ${JSON.stringify(r)};
...
function gS() {
  const e = De.join(process.cwd(), "screenshots");
  return tt.existsSync(e) || tt.mkdirSync(e, { recursive: !0 }), e;
}
...
async function of(e = _t()) {
  if (!e.general.takeScreenshots)
    return [];
  const t = gS(), n = Date.now();
  if (e.trackingSources.screenshotsFrom === "all") {
    const a = await xi.listDisplays(), i = await Promise.all(
      a.map(async (s, o) => {
        const l = De.join(
          t,
          `${n}-${o}.jpg`
    ...
    return Promise.all(i.map((s) => lu(s, e)));
  }
  const r = De.join(
    t,
    `${n}.jpg`
...
async function xS() {
  return (await of()).filter((t) => t.approved).map((t) => t.filePath);
}
async function cf(e) {
  ...
  const n = e.screenshotPaths === void 0 ? await xS() : e.screenshotPaths;
  return Cm({
    ...e,
    description: t,
    screenshotPaths: n,
...
function lf(e, t) {
  const n = uu(t.title), r = uu(t.projectLabel || t.project);
  return `
    <!doctype html>
    <html>
      <head>
...
function bS(e) {
  return new Promise((t, n) => {
    const r = `activity-${Date.now()}-${Math.random().toString(36).slice(2)}`, a = xp() || void 0, i = new Mt({
      width: 420,
      height: 220,
      parent: a,
      modal: !!a,
    ...
    let s = !1;
    function o() {
      le.removeListener("activity-prompt:submit", c), le.removeListener("activity-prompt:cancel", u);
    }
    function l() {
    ...
    le.on("activity-prompt:submit", c), le.on("activity-prompt:cancel", u), i.on("closed", () => {
      s || (s = !0, o(), t(null));
    }), i.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        lf(r, e)
...
function _S(e) {
  return new Promise((t) => {
    const n = `activity-description-${Date.now()}-${Math.random().toString(36).slice(2)}`, r = xp() || void 0, a = new Mt({
      width: 420,
      height: 220,
      parent: r,
      modal: !!r,
    ...
    let i = !1;
    function s() {
      le.removeListener("activity-prompt:submit", u), le.removeListener("activity-prompt:cancel", l);
    }
    function o() {
      a.isDestroyed() || a.close();
    }
    function l(c, p) {
      p.promptId !== n || i || (i = !0, s(), t(null), o());
    }
    function u(c, p) {
      p.promptId !== n || i || (i = !0, s(), t(p.description.trim()), o());
    }
    le.on("activity-prompt:submit", u), le.on("activity-prompt:cancel", l), a.on("closed", () => {
      i || (i = !0, s(), t(null));
    }), a.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        lf(n, e)
...
function wS() {
  le.handle("activities:create", async (e, t) => cf(t)), le.handle("activities:list-recent-unsynced", async (e, t) => zm(t)), le.handle("activities:list-recent-sessions", async (e, t) => qm(t)), le.handle(
    "activities:list-media",
    async (e, t, n) => Bm(t, n)
  ), le.handle("activities:prompt", async (e, t) => {
    if (!t.project)
      throw new Error("Please select a project first.");
    return bS(t);
  }), le.handle(
...
function ES() {
  ...
  }), le.handle("auth:login", async (e, t) => {
    const n = fS(t.siteUrl), r = t.apiKey.trim(), a = t.apiSecret.trim();
    if (!n || !r || !a)
  ...
  }), le.handle("auth:logout", async () => (vS(), {
    success: !0
  }));
}
function $S(e) {
  function t(n) {
    Mt.getAllWindows().forEach((r) => {
      r.webContents.send("config:updated", n);
    });
  }
  le.handle("config:open-window", async () => (bp(e), {
    success: !0
  })), le.handle("config:get", async () => _t()), le.handle("config:save", async (n, r) => {
    const a = pS(r);
    return t(a), a;
  }), le.handle("config:reset", async () => {
    const n = dS();
    return t(n), n;
  });
}
...
const { toString: SS } = Object.prototype, { getPrototypeOf: Ka } = Object, { iterator: Xa, toStringTag: pf } = Symbol, Wa = /* @__PURE__ */ ((e) => (t) => {
  const n = SS.call(t);
  return e[n] || (e[n] = n.slice(8, -1).toLowerCase());
})(/* @__PURE__ */ Object.create(null)), ut = (e) => (e = e.toLowerCase(), (t) => Wa(t) === e), Ya = (e) => (t) => typeof t === e, { isArray: Hn } = Array, Ln = Ya("undefined");
function Tr(e) {
  return e !== null && !Ln(e) && e.constructor !== null && !Ln(e.constructor) && qe(e.constructor.isBuffer) && e.constructor.isBuffer(e);
}
const df = ut("ArrayBuffer");
function TS(e) {
  let t;
  return typeof ArrayBuffer < "u" && ArrayBuffer.isView ? t = ArrayBuffer.isView(e) : t = e && e.buffer && df(e.buffer), t;
}
const RS = Ya("string"), qe = Ya("function"), ff = Ya("number"), Rr = (e) => e !== null && typeof e == "object", OS = (e) => e === !0 || e === !1, da = (e) => {
  if (Wa(e) !== "object")
    return !1;
  const t = Ka(e);
  return (t === null || t === Object.prototype || Object.getPrototypeOf(t) === null) && !(pf in e) && !(Xa in e);
}, AS = (e) => {
  if (!Rr(e) || Tr(e))
    return !1;
  try {
...
}, PS = ut("Date"), kS = ut("File"), NS = (e) => !!(e && typeof e.uri < "u"), IS = (e) => e && typeof e.getParts < "u", jS = ut("Blob"), CS = ut("FileList"), LS = (e) => Rr(e) && qe(e.pipe);
function DS() {
  return typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : typeof global < "u" ? global : {};
}
const pu = DS(), du = typeof pu.FormData < "u" ? pu.FormData : void 0, FS = (e) => {
  if (!e) return !1;
  if (du && e instanceof du) return !0;
  const t = Ka(e);
  if (!t || t === Object.prototype || !qe(e.append)) return !1;
  const n = Wa(e);
  return n === "formdata" || // detect form-data instance
  n === "object" && qe(e.toString) && e.toString() === "[object FormData]";
}, US = ut("URLSearchParams"), [MS, zS, qS, BS] = [
  "ReadableStream",
  "Request",
  "Response",
  "Headers"
].map(ut), VS = (e) => e.trim ? e.trim() : e.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, "");
...
const Wt = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof window < "u" ? window : global, hf = (e) => !Ln(e) && e !== Wt;
function Ts(...e) {
  const { caseless: t, skipUndefined: n } = hf(this) && this || {}, r = {}, a = (i, s) => {
    if (s === "__proto__" || s === "constructor" || s === "prototype")
      return;
    const o = t && mf(r, s) || s, l = Rs(r, o) ? r[o] : void 0;
    da(l) && da(i) ? r[o] = Ts(l, i) : da(i) ? r[o] = Ts({}, i) : Hn(i) ? r[o] = i.slice() : (!n || !Ln(i)) && (r[o] = i);
  };
  for (let i = 0, s = e.length; i < s; i++)
    e[i] && Or(e[i], a);
  return r;
}
const HS = (e, t, n, { allOwnKeys: r } = {}) => (Or(
  t,
  (a, i) => {
    n && qe(a) ? Object.defineProperty(e, i, {
  ...
  { allOwnKeys: r }
), e), GS = (e) => (e.charCodeAt(0) === 65279 && (e = e.slice(1)), e), KS = (e, t, n, r) => {
  ...
  for (; t-- > 0; )
    n[t] = e[t];
  return n;
}, JS = /* @__PURE__ */ ((e) => (t) => e && t instanceof e)(typeof Uint8Array < "u" && Ka(Uint8Array)), QS = (e, t) => {
  const r = (e && e[Xa]).call(e);
  let a;
  for (; (a = r.next()) && !a.done; ) {
    const i = a.value;
    t.call(e, i[0], i[1]);
  ...
  for (; (n = e.exec(t)) !== null; )
    r.push(n);
  return r;
}, e1 = ut("HTMLFormElement"), t1 = (e) => e.toLowerCase().replace(/[-_\s]([a-z\d])(\w*)/g, function(n, r, a) {
  return r.toUpperCase() + a;
}), Rs = (({ hasOwnProperty: e }) => (t, n) => e.call(t, n))(Object.prototype), n1 = ut("RegExp"), vf = (e, t) => {
  ...
  vf(e, (t, n) => {
    if (qe(e) && ["arguments", "caller", "callee"].includes(n))
      return !1;
    const r = e[n];
    if (qe(r)) {
...
}, s1 = (e, t) => e != null && Number.isFinite(e = +e) ? e : t;
function o1(e) {
  return !!(e && qe(e.append) && e[pf] === "FormData" && e[Xa]);
}
const c1 = (e) => {
  ...
  return n(e);
}, l1 = ut("AsyncFunction"), u1 = (e) => e && (Rr(e) || qe(e)) && qe(e.then) && qe(e.catch), yf = ((e, t) => e ? setImmediate : t ? ((n, r) => (Wt.addEventListener(
  "message",
  ({ source: a, data: i }) => {
    a === Wt && i === n && r.length && r.shift()();
  },
  !1
), (a) => {
  r.push(a), Wt.postMessage(n, "*");
}))(`axios@${Math.random()}`, []) : (n) => setTimeout(n))(typeof setImmediate == "function", qe(Wt.postMessage)), p1 = typeof queueMicrotask < "u" ? queueMicrotask.bind(Wt) : typeof process < "u" && process.nextTick || yf, d1 = (e) => e != null && qe(e[Xa]), A = {
  isArray: Hn,
  isArrayBuffer: df,
  isBuffer: Tr,
  isFormData: FS,
  ...
  return e && e.split(`
`).forEach(function(s) {
    a = s.indexOf(":"), n = s.substring(0, a).trim().toLowerCase(), r = s.substring(a + 1).trim(), !(!n || t[n] && f1[n]) && (n === "set-cookie" ? t[n] ? t[n].push(r) : t[n] = [r] : t[n] = t[n] ? t[n] + ", " + r : r);
  }), t;
};
function h1(e) {
  let t = 0, n = e.length;
...
const v1 = new RegExp("[\\u0000-\\u0008\\u000a-\\u001f\\u007f]+", "g"), y1 = new RegExp("[^\\u0009\\u0020-\\u007e\\u0080-\\u00ff]+", "g");
function rc(e, t) {
  return A.isArray(e) ? e.map((n) => rc(n, t)) : h1(String(e).replace(t, ""));
}
const g1 = (e) => rc(e, v1), x1 = (e) => rc(e, y1);
function ac(e) {
...
const fu = Symbol("internals");
function rr(e) {
  return e && String(e).trim().toLowerCase();
}
function fa(e) {
  return e === !1 || e == null ? e : A.isArray(e) ? e.map(fa) : g1(String(e));
}
function b1(e) {
...
const _1 = (e) => /^[-_a-zA-Z0-9^`|~,!#$%&'*+.]+$/.test(e.trim());
...
let $e = class {
  ...
  set(t, n, r) {
    ...
    function i(o, l, u) {
      ...
      (!p || a[p] === void 0 || u === !0 || u === void 0 && a[p] !== !1) && (a[p || l] = fa(o));
    }
    const s = (o, l) => A.forEach(o, (u, c) => i(u, c, l));
    if (A.isPlainObject(t) || t instanceof this.constructor)
      s(t, n);
    else if (A.isString(t) && (t = t.trim()) && !_1(t))
      s(m1(t), n);
  ...
  normalize(t) {
    ...
    return A.forEach(this, (a, i) => {
      ...
      const o = t ? w1(i) : String(i).trim();
      o !== i && delete n[i], n[o] = fa(a), r[o] = !0;
    }), this;
  }
  concat(...t) {
...
A.reduceDescriptors($e.prototype, ({ value: e }, t) => {
...
function T1(e, t) {
  const n = new Set(t.map((i) => String(i).toLowerCase())), r = [], a = (i) => {
    ...
    i instanceof $e && (i = i.toJSON()), r.push(i);
    let s;
    if (A.isArray(i))
      s = [], i.forEach((o, l) => {
        const u = a(o);
      ...
      if (!A.isPlainObject(i) && S1(i))
        return r.pop(), i;
      s = /* @__PURE__ */ Object.create(null);
      for (const [o, l] of Object.entries(i)) {
        const u = n.has(o.toLowerCase()) ? $1 : a(l);
...
let G = class gf extends Error {
  static from(t, n, r, a, i, s) {
    const o = new gf(t.message, n || t.code, r, a, i);
  ...
  toJSON() {
    const t = this.config, n = t && A.hasOwnProp(t, "redact") ? t.redact : void 0, r = A.isArray(n) && n.length > 0 ? T1(t, n) : A.toJSONObject(t);
    return {
      // Standard
...
Object.defineProperty(pt.prototype, "readable", {
  configurable: !0,
  enumerable: !0,
  get: function() {
...
pt.prototype.setEncoding = function() {
...
pt.prototype._handleEmit = function(e) {
  if (this._released) {
    this.emit.apply(this, e);
    return;
  }
...
pt.prototype._checkIfMaxDataSizeExceeded = function() {
...
var A1 = pn, bf = Re.Stream, mu = O1, P1 = ge;
function ge() {
...
ge.prototype.pipe = function(e, t) {
  return bf.prototype.pipe.call(this, e, t), this.resume(), e;
};
 ...
 */
(function(e) {
  var t = N1, n = De.extname, r = /^\s*([^;\s]*)(?:;|\s|$)/, a = /^text\//i;
...
var Ef = L1;
function L1(e) {
  Object.keys(e.jobs).forEach(D1.bind(e)), e.jobs = {};
}
function D1(e) {
  typeof this.jobs[e] == "function" && this.jobs[e]();
}
var vu = wf, F1 = Ef, $f = U1;
function U1(e, t, n, r) {
  var a = n.keyedList ? n.keyedList[n.index] : n.index;
  n.jobs[a] = M1(t, a, e[a], function(i, s) {
    a in n.jobs && (delete n.jobs[a], i ? F1(n) : n.results[a] = s, r(i, n.results));
  });
}
function M1(e, t, n, r) {
  var a;
  return e.length == 2 ? a = e(n, vu(r)) : a = e(n, t, vu(r)), a;
}
var Sf = z1;
function z1(e, t) {
...
var q1 = Ef, B1 = wf, Tf = V1;
function V1(e) {
  Object.keys(this.jobs).length && (this.index = this.size, q1(this), B1(e)(null, this.results));
}
var H1 = $f, G1 = Sf, K1 = Tf, X1 = W1;
function W1(e, t, n) {
  for (var r = G1(e); r.index < (r.keyedList || e).length; )
    H1(e, t, r, function(a, i) {
      if (a) {
        n(a, i);
        return;
      }
...
var Ja = { exports: {} }, yu = $f, Y1 = Sf, J1 = Tf;
Ja.exports = Q1;
Ja.exports.ascending = Rf;
Ja.exports.descending = Z1;
function Q1(e, t, n, r) {
  var a = Y1(e, n);
  return yu(e, t, a, function i(s, o) {
    if (s) {
      r(s, o);
      return;
    }
...
var rT = {
  ...
  serial: tT,
  serialOrdered: Of
}, Af = Object, aT = Error, iT = EvalError, sT = RangeError, oT = ReferenceError, cT = SyntaxError, ic = TypeError, lT = URIError, uT = Math.abs, pT = Math.floor, dT = Math.max, fT = Math.min, mT = Math.pow, hT = Math.round, vT = Number.isNaN || function(t) {
  return t !== t;
}, yT = vT, gT = function(t) {
  return yT(t) || t === 0 ? t : t < 0 ? -1 : 1;
}, xT = Object.getOwnPropertyDescriptor, ma = xT;
if (ma)
  try {
    ma([], "length");
  } catch {
...
var Pf = ma, ha = Object.defineProperty || !1;
if (ha)
  try {
    ha({}, "a", { value: 1 });
  } catch {
...
function _T() {
  if (xu) return zi;
  xu = 1;
  var e = typeof Symbol < "u" && Symbol, t = kf();
  return zi = function() {
...
var qi, bu;
function Nf() {
  return bu || (bu = 1, qi = typeof Reflect < "u" && Reflect.getPrototypeOf || null), qi;
}
var Bi, _u;
function If() {
  if (_u) return Bi;
...
var wT = "Function.prototype.bind called on incompatible ", ET = Object.prototype.toString, $T = Math.max, ST = "[object Function]", wu = function(t, n) {
  ...
  var n = this;
  if (typeof n != "function" || ET.apply(n) !== ST)
    throw new TypeError(wT + n);
  for (var r = TT(arguments, 1), a, i = function() {
  ...
  }, s = $T(0, n.length - r.length), o = [], l = 0; l < s; l++)
    o[l] = "$" + l;
  if (a = Function("binder", "return function (" + RT(o, ",") + "){ return binder.apply(this,arguments); }")(i), n.prototype) {
    var u = function() {
    };
    u.prototype = n.prototype, a.prototype = new u(), u.prototype = null;
  }
  return a;
}, AT = OT, Qa = Function.prototype.bind || AT, Vi, Eu;
function sc() {
  return Eu || (Eu = 1, Vi = Function.prototype.call), Vi;
}
var Hi, $u;
function jf() {
  return $u || ($u = 1, Hi = Function.prototype.apply), Hi;
}
var Gi, Su;
function PT() {
  return Su || (Su = 1, Gi = typeof Reflect < "u" && Reflect && Reflect.apply), Gi;
}
var Ki, Tu;
function kT() {
  if (Tu) return Ki;
  Tu = 1;
  var e = Qa, t = jf(), n = sc(), r = PT();
  return Ki = r || e.call(n, t), Ki;
}
var Xi, Ru;
function NT() {
  if (Ru) return Xi;
  Ru = 1;
  var e = Qa, t = ic, n = sc(), r = kT();
...
function IT() {
  ...
  return Wi = r && typeof r.get == "function" ? e([r.get]) : typeof i == "function" ? (
    /** @type {import('./get')} */
    function(o) {
...
var CT = Function.prototype.call, LT = Object.prototype.hasOwnProperty, DT = Qa, oc = DT.call(CT, LT), se, FT = Af, UT = aT, MT = iT, zT = sT, qT = oT, Dn = cT, Pn = ic, BT = lT, VT = uT, HT = pT, GT = dT, KT = fT, XT = mT, WT = hT, YT = gT, Cf = Function, Ji = function(e) {
  try {
    return Cf('"use strict"; return (' + e + ").constructor;")();
  } catch {
  }
}, yr = Pf, JT = bT, Qi = function() {
  throw new Pn();
}, QT = yr ? function() {
  try {
...
}() : Qi, bn = _T()(), Ee = jT(), ZT = If(), eR = Nf(), Lf = jf(), Ar = sc(), En = {}, tR = typeof Uint8Array > "u" || !Ee ? se : Ee(Uint8Array), en = {
  __proto__: null,
  "%AggregateError%": typeof AggregateError > "u" ? se : AggregateError,
  "%Array%": Array,
  "%ArrayBuffer%": typeof ArrayBuffer > "u" ? se : ArrayBuffer,
  "%ArrayIteratorPrototype%": bn && Ee ? Ee([][Symbol.iterator]()) : se,
  "%AsyncFromSyncIteratorPrototype%": se,
  "%AsyncFunction%": En,
  "%AsyncGenerator%": En,
  "%AsyncGeneratorFunction%": En,
  "%AsyncIteratorPrototype%": En,
  ...
  "%isFinite%": isFinite,
  "%isNaN%": isNaN,
  "%IteratorPrototype%": bn && Ee ? Ee(Ee([][Symbol.iterator]())) : se,
  "%JSON%": typeof JSON == "object" ? JSON : se,
  "%Map%": typeof Map > "u" ? se : Map,
  "%MapIteratorPrototype%": typeof Map > "u" || !bn || !Ee ? se : Ee((/* @__PURE__ */ new Map())[Symbol.iterator]()),
  "%Math%": Math,
  "%Number%": Number,
  "%Object%": FT,
  ...
  "%SetIteratorPrototype%": typeof Set > "u" || !bn || !Ee ? se : Ee((/* @__PURE__ */ new Set())[Symbol.iterator]()),
  "%SharedArrayBuffer%": typeof SharedArrayBuffer > "u" ? se : SharedArrayBuffer,
  "%String%": String,
  "%StringIteratorPrototype%": bn && Ee ? Ee(""[Symbol.iterator]()) : se,
  "%Symbol%": bn ? Symbol : se,
  "%SyntaxError%": Dn,
  "%ThrowTypeError%": QT,
  "%TypedArray%": tR,
  ...
  "%Function.prototype.apply%": Lf,
  "%Object.defineProperty%": JT,
  "%Object.getPrototypeOf%": ZT,
  "%Math.abs%": VT,
  "%Math.floor%": HT,
  "%Math.max%": GT,
  "%Math.min%": KT,
...
var rR = function e(t) {
...
}, Pr = Qa, Ea = oc, aR = Pr.call(Ar, Array.prototype.concat), iR = Pr.call(Lf, Array.prototype.splice), ku = Pr.call(Ar, String.prototype.replace), $a = Pr.call(Ar, String.prototype.slice), sR = Pr.call(Ar, RegExp.prototype.exec), oR = /[^%.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|%$))/g, cR = /\\(\\)?/g, lR = function(t) {
  var n = $a(t, 0, 1), r = $a(t, -1);
  if (n === "%" && r !== "%")
    throw new Dn("invalid intrinsic syntax, expected closing `%`");
  if (r === "%" && n !== "%")
    throw new Dn("invalid intrinsic syntax, expected opening `%`");
  var a = [];
  return ku(t, oR, function(i, s, o, l) {
    a[a.length] = o ? ku(l, cR, "$1") : s || i;
  }), a;
}, uR = function(t, n) {
  var r = t, a;
  if (Ea(Pu, r) && (a = Pu[r], r = "%" + a[0] + "%"), Ea(en, r)) {
    var i = en[r];
    if (i === En && (i = rR(r)), typeof i > "u" && !n)
  ...
  var r = lR(t), a = r.length > 0 ? r[0] : "", i = uR("%" + a + "%", n), s = i.name, o = i.value, l = !1, u = i.alias;
  u && (a = u[0], iR(r, aR([0, 1], u)));
  for (var c = 1, p = !0; c < r.length; c += 1) {
    var f = r[c], h = $a(f, 0, 1), g = $a(f, -1);
    if ((h === '"' || h === "'" || h === "`" || g === '"' || g === "'" || g === "`") && h !== g)
      throw new Dn("property names with quotes must have matching quotes");
    if ((f === "constructor" || !p) && (l = !0), a += "." + f, s = "%" + a + "%", Ea(en, s))
      o = en[s];
    else if (o != null) {
      if (!(f in o)) {
        if (!n)
...
var fR = pR, Iu = fR("%Object.defineProperty%", !0), mR = dR()(), hR = oc, vR = ic, ta = mR ? Symbol.toStringTag : null, yR = function(t, n) {
...
}, cc = P1, xR = pn, es = De, bR = Ms, _R = zs, wR = Aa.parse, ER = tt, $R = Re.Stream, SR = hp, ts = _f, TR = rT, RR = yR, Ut = oc, Os = gR;
function oe(e) {
  if (!(this instanceof oe))
    return new oe(e);
  this._overheadLength = 0, this._valueLength = 0, this._valuesToMeasure = [], cc.call(this), e = e || {};
  for (var t in e)
    this[t] = e[t];
}
xR.inherits(oe, cc);
...
oe.prototype.append = function(e, t, n) {
  ...
  var r = cc.prototype.append.bind(this);
...
oe.prototype._trackLength = function(e, t, n) {
  var r = 0;
  n.knownLength != null ? r += Number(n.knownLength) : Buffer.isBuffer(t) ? r = t.length : typeof t == "string" && (r = Buffer.byteLength(t)), this._valueLength += r, this._overheadLength += Buffer.byteLength(e) + oe.LINE_BREAK.length, !(!t || !t.path && !(t.readable && Ut(t, "httpVersion")) && !(t instanceof $R)) && (n.knownLength || this._valuesToMeasure.push(t));
};
oe.prototype._lengthRetriever = function(e, t) {
  Ut(e, "fd") ? e.end != null && e.end != 1 / 0 && e.start != null ? t(null, e.end + 1 - (e.start ? e.start : 0)) : ER.stat(e.path, function(n, r) {
    if (n) {
      t(n);
      return;
    }
    ...
    t(null, a);
  }) : Ut(e, "httpVersion") ? t(null, Number(e.headers["content-length"])) : Ut(e, "httpModule") ? (e.on("response", function(n) {
...
oe.prototype._multiPartHeader = function(e, t, n) {
  ...
  typeof n.header == "object" && Os(s, n.header);
  var o;
  for (var l in s)
    if (Ut(s, l)) {
      if (o = s[l], o == null)
...
oe.prototype._getContentDisposition = function(e, t) {
  var n;
  if (typeof t.filepath == "string" ? n = es.normalize(t.filepath).replace(/\\/g, "/") : t.filename || e && (e.name || e.path) ? n = es.basename(t.filename || e && (e.name || e.path)) : e && e.readable && Ut(e, "httpVersion") && (n = es.basename(e.client._httpMessage.path || "")), n)
    return 'filename="' + n + '"';
};
oe.prototype._getContentType = function(e, t) {
  var n = t.contentType;
  return !n && e && e.name && (n = ts.lookup(e.name)), !n && e && e.path && (n = ts.lookup(e.path)), !n && e && e.readable && Ut(e, "httpVersion") && (n = e.headers["content-type"]), !n && (t.filepath || t.filename) && (n = ts.lookup(t.filepath || t.filename)), !n && e && typeof e == "object" && (n = oe.DEFAULT_CONTENT_TYPE), n;
};
...
oe.prototype._generateBoundary = function() {
  this._boundary = "--------------------------" + SR.randomBytes(12).toString("hex");
};
...
oe.prototype.getLength = function(e) {
  ...
  TR.parallel(this._valuesToMeasure, this._lengthRetriever, function(n, r) {
    if (n) {
      e(n);
      return;
    }
...
oe.prototype.submit = function(e, t) {
  ...
  return typeof e == "string" ? (e = wR(e), r = Os({
    port: e.port,
    path: e.pathname,
    host: e.hostname,
    protocol: e.protocol
  }, a)) : (r = Os(e, a), r.port || (r.port = r.protocol === "https:" ? 443 : 80)), r.headers = this.getHeaders(e.headers), r.protocol === "https:" ? n = _R.request(r) : n = bR.request(r), this.getLength((function(i, s) {
...
RR(oe.prototype, "FormData");
var OR = oe;
const Df = /* @__PURE__ */ Un(OR);
function As(e) {
...
function ns(e, t, n) {
  return e ? e.concat(t).map(function(a, i) {
    return a = Ff(a), !n && i ? "[" + a + "]" : a;
  }).join(n ? "." : "") : t;
}
function AR(e) {
  return A.isArray(e) && !e.some(As);
}
...
function Za(e, t, n) {
  ...
  t = t || new (Df || FormData)(), n = A.toFlatObject(
    n,
    {
      metaTokens: !0,
      dots: !1,
  ...
  function p(m, v, d) {
    ...
    if (A.isReactNative(t) && A.isReactNativeBlob(m))
      return t.append(ns(d, v, i), c(m)), !1;
    if (m && !d && typeof m == "object") {
      if (A.endsWith(v, "{}"))
        v = r ? v : v.slice(0, -2), m = JSON.stringify(m);
      else if (A.isArray(m) && AR(m) || (A.isFileList(m) || A.endsWith(v, "[]")) && (y = A.toArray(m)))
        return v = Ff(v), y.forEach(function(x, w) {
          !(A.isUndefined(x) || x === null) && t.append(
            // eslint-disable-next-line no-nested-ternary
            s === !0 ? ns([v], w, i) : s === null ? v : v + "[]",
            c(x)
          );
        }), !1;
    }
    return As(m) ? !0 : (t.append(ns(d, v, i), c(m)), !1);
  }
  const f = [], h = Object.assign(PR, {
    defaultVisitor: p,
    convertValue: c,
    isVisitable: As
  });
...
function lc(e, t, n) {
  ...
  let s;
  if (i ? s = i(t, a) : s = A.isURLSearchParams(t) ? t.toString() : new Uf(t, a).toString(r), s) {
    const o = e.indexOf("#");
...
}, NR = Aa.URLSearchParams, rs = "abcdefghijklmnopqrstuvwxyz", Lu = "0123456789", zf = {
  DIGIT: Lu,
  ALPHA: rs,
  ALPHA_DIGIT: rs + rs.toUpperCase() + Lu
}, IR = (e = 16, t = zf.ALPHA_DIGIT) => {
  let n = "";
...
}, uc = typeof window < "u" && typeof document < "u", Ps = typeof navigator == "object" && navigator || void 0, CR = uc && (!Ps || ["ReactNative", "NativeScript", "NS"].indexOf(Ps.product) < 0), LR = typeof WorkerGlobalScope < "u" && // eslint-disable-next-line no-undef
self instanceof WorkerGlobalScope && typeof self.importScripts == "function", DR = uc && window.location.href || "http://localhost", FR = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  hasBrowserEnv: uc,
...
function UR(e, t) {
  return Za(e, new ye.classes.URLSearchParams(), {
...
function qf(e) {
  function t(n, r, a, i) {
    ...
    return s = !s && A.isArray(a) ? a.length : s, l ? (A.hasOwnProp(a, s) ? a[s] = A.isArray(a[s]) ? a[s].concat(r) : [a[s], r] : a[s] = r, !o) : ((!A.hasOwnProp(a, s) || !A.isObject(a[s])) && (a[s] = []), t(n, r, a[s], i) && A.isArray(a[s]) && (a[s] = zR(a[s])), !o);
  }
...
const _n = (e, t) => e != null && A.hasOwnProp(e, t) ? e[t] : void 0;
function qR(e, t, n) {
  if (A.isString(e))
    try {
  ...
  transformRequest: [
    function(t, n) {
      ...
      if (i && A.isHTMLForm(t) && (t = new FormData(t)), A.isFormData(t))
        return a ? JSON.stringify(qf(t)) : t;
      ...
      return i || a ? (n.setContentType("application/json", !1), qR(t)) : t;
    }
  ],
  transformResponse: [
    function(t) {
      const n = _n(this, "transitional") || kr.transitional, r = n && n.forcedJSONParsing, a = _n(this, "responseType"), i = a === "json";
      if (A.isResponse(t) || A.isReadableStream(t))
        return t;
      if (t && A.isString(t) && (r && !a || i)) {
        const o = !(n && n.silentJSONParsing) && i;
        try {
          return JSON.parse(t, _n(this, "parseReviver"));
        } catch (l) {
          if (o)
            throw l.name === "SyntaxError" ? G.from(l, G.ERR_BAD_RESPONSE, this, null, _n(this, "response")) : l;
        }
      }
      return t;
    }
...
function as(e, t) {
  const n = this || kr, r = t || n, a = $e.from(r.headers);
  let i = r.data;
...
function KR(e) {
  var t = (typeof e == "string" ? GR(e) : e) || {}, n = t.protocol, r = t.host, a = t.port;
  if (typeof r != "string" || !r || typeof n != "string" || (n = n.split(":", 1)[0], r = r.replace(/:\d*$/, ""), a = parseInt(a) || HR[n] || 0, !XR(r, a)))
    return "";
  var i = ks(n + "_proxy") || ks("all_proxy");
  return i && i.indexOf("://") === -1 && (i = n + "://" + i), i;
}
function XR(e, t) {
  var n = ks("no_proxy").toLowerCase();
...
var dc = {}, Ns = { exports: {} }, na = { exports: {} }, is, Du;
function WR() {
  if (Du) return is;
  Du = 1;
...
function Vf() {
  ...
  Fu = 1;
  function e(t) {
    r.debug = r, r.default = r, r.coerce = u, r.disable = o, r.enable = i, r.enabled = l, r.humanize = WR(), r.destroy = c, Object.keys(t).forEach((p) => {
      r[p] = t[p];
...
var ra = { exports: {} }, os, Mu;
function JR() {
  return Mu || (Mu = 1, os = (e, t = process.argv) => {
...
function QR() {
  if (zu) return cs;
  zu = 1;
  const e = ya, t = vp, n = JR(), { env: r } = process;
  let a;
...
typeof process > "u" || process.type === "renderer" || process.browser === !0 || process.__nwjs ? Ns.exports = YR() : Ns.exports = ZR();
var ti = Ns.exports, fc = {};
...
fc.default = eO;
var Hf = vt && vt.__importDefault || function(e) {
  return e && e.__esModule ? e : { default: e };
};
const tO = fp, nO = Hf(ti), rO = Hf(fc), ar = nO.default("agent-base");
function aO(e) {
...
function Sa(e, t) {
  return new Sa.Agent(e, t);
}
(function(e) {
  class t extends tO.EventEmitter {
    constructor(r, a) {
      super();
      let i = a;
    ...
    get defaultPort() {
      return typeof this.explicitDefaultPort == "number" ? this.explicitDefaultPort : ls() ? 443 : 80;
    }
    set defaultPort(r) {
      this.explicitDefaultPort = r;
    }
    get protocol() {
      return typeof this.explicitProtocol == "string" ? this.explicitProtocol : ls() ? "https:" : "http:";
    }
    set protocol(r) {
    ...
    addRequest(r, a) {
      ...
      const l = i.timeout || this.timeout, u = (h) => {
        ...
        if (s)
          return;
        if (o != null && (clearTimeout(o), o = null), aO(h)) {
          ar("Callback returned another Agent instance %o", h.constructor.name), h.addRequest(r, i);
          return;
        }
        if (h) {
          h.once("free", () => {
      ...
      this.promisifiedCallback || (this.callback.length >= 3 ? (ar("Converting legacy callback function to promise"), this.promisifiedCallback = rO.default(this.callback)) : this.promisifiedCallback = this.callback), typeof l == "number" && l > 0 && (o = setTimeout(c, l)), "port" in i && typeof i.port != "number" && (i.port = Number(i.port));
      try {
        ar("Resolving socket for %o request: %o", i.protocol, `${r.method} ${r.path}`), Promise.resolve(this.promisifiedCallback(r, i)).then(f, p);
      } catch (h) {
        Promise.reject(h).catch(p);
      }
    }
    freeSocket(r, a) {
      ar("Freeing socket %o %o", r.constructor.name, a), r.destroy();
    }
    destroy() {
      ar("Destroying agent %o", this.constructor.name);
    }
  }
  e.Agent = t, e.prototype = e.Agent.prototype;
})(Sa || (Sa = {}));
var iO = Sa, mc = {}, sO = vt && vt.__importDefault || function(e) {
  return e && e.__esModule ? e : { default: e };
};
Object.defineProperty(mc, "__esModule", { value: !0 });
const oO = sO(ti), ir = oO.default("https-proxy-agent:parse-proxy-response");
function cO(e) {
  return new Promise((t, n) => {
    let r = 0;
    const a = [];
...
\r
`) === -1) {
        ir("have not received end of HTTP headers yet..."), i();
        return;
      }
      const g = f.toString("ascii", 0, f.indexOf(`\r
`)), m = +g.split(" ")[1];
      ir("got proxy server response: %o", g), t({
        statusCode: m,
        buffered: f
      });
    }
...
mc.default = cO;
var lO = vt && vt.__awaiter || function(e, t, n, r) {
  function a(i) {
...
}, Gn = vt && vt.__importDefault || function(e) {
  return e && e.__esModule ? e : { default: e };
};
Object.defineProperty(dc, "__esModule", { value: !0 });
const Bu = Gn(Sm), Vu = Gn(Tm), uO = Gn(Aa), pO = Gn(_r), dO = Gn(ti), fO = iO, mO = Gn(mc), sr = dO.default("https-proxy-agent:agent");
let hO = class extends fO.Agent {
  constructor(t) {
    let n;
    if (typeof t == "string" ? n = uO.default.parse(t) : n = t, !n)
      throw new Error("an HTTP(S) proxy server `host` and `port` must be specified!");
    sr("creating new HttpsProxyAgent instance: %o", n), super(n);
    const r = Object.assign({}, n);
    this.secureProxy = n.secureProxy || gO(r.protocol), r.host = r.hostname || r.host, typeof r.port == "string" && (r.port = parseInt(r.port, 10)), !r.port && r.host && (r.port = this.secureProxy ? 443 : 80), this.secureProxy && !("ALPNProtocols" in r) && (r.ALPNProtocols = ["http 1.1"]), r.host && r.path && (delete r.path, delete r.pathname), this.proxy = r;
  }
  /**
  ...
  callback(t, n) {
    return lO(this, void 0, void 0, function* () {
      ...
      let i;
      a ? (sr("Creating `tls.Socket`: %o", r), i = Vu.default.connect(r)) : (sr("Creating `net.Socket`: %o", r), i = Bu.default.connect(r));
...
`;
      ...
      yO(c, p) || (u += `:${c}`), s.Host = u, s.Connection = "close";
      for (const v of Object.keys(s))
        l += `${v}: ${s[v]}\r
`;
      const f = mO.default(i);
      i.write(`${l}\r
`);
      const { statusCode: h, buffered: g } = yield f;
      if (h === 200) {
        if (t.once("socket", vO), n.secureEndpoint) {
          sr("Upgrading socket connection to TLS");
          const v = n.servername || n.host;
          return Vu.default.connect(Object.assign(Object.assign({}, xO(n, "host", "hostname", "path", "port")), {
            socket: i,
            servername: v
          }));
        }
      ...
      i.destroy();
      const m = new Bu.default.Socket({ writable: !1 });
      return m.readable = !0, t.once("socket", (v) => {
        sr("replaying proxy buffer for failed request"), pO.default(v.listenerCount("data") > 0), v.push(g), v.push(null);
      }), m;
    });
  }
};
...
var bO = vt && vt.__importDefault || function(e) {
  return e && e.__esModule ? e : { default: e };
};
const Is = bO(dc);
function js(e) {
...
(function(e) {
  e.HttpsProxyAgent = Is.default, e.prototype = Is.default.prototype;
})(js || (js = {}));
var _O = js;
const Gf = /* @__PURE__ */ Un(_O);
var hc = { exports: {} }, or, wO = function() {
  if (!or) {
    try {
      or = ti("follow-redirects");
    } catch {
    }
...
}, Nr = Aa, gr = Nr.URL, EO = Ms, $O = zs, vc = Re.Writable, yc = _r, Kf = wO;
(function() {
...
var SO = [
  ...
  "search",
  "hash"
], xc = ["abort", "aborted", "connect", "error", "socket", "timeout"], bc = /* @__PURE__ */ Object.create(null);
...
var Cs = Ir(
  ...
  "write after end"
), PO = vc.prototype.destroy || Wf;
function Ve(e, t) {
  vc.call(this), this._sanitizeOptions(e), this._options = e, this._ended = !1, this._ending = !1, this._redirectCount = 0, this._redirects = [], this._requestBodyLength = 0, this._requestBodyBuffers = [], t && this.on("response", t);
  var n = this;
  this._onNativeResponse = function(r) {
    try {
      n._processResponse(r);
    } catch (a) {
      n.emit("error", a instanceof Ls ? a : new Ls({ cause: a }));
    }
  }, this._headerFilter = new RegExp("^(?:" + SO.concat(e.sensitiveHeaders).map(LO).join("|") + ")$", "i"), this._performRequest();
}
Ve.prototype = Object.create(vc.prototype);
Ve.prototype.abort = function() {
  wc(this._currentRequest), this._currentRequest.abort(), this.emit("abort");
};
Ve.prototype.destroy = function(e) {
  return wc(this._currentRequest, e), PO.call(this, e), this;
};
Ve.prototype.write = function(e, t, n) {
  ...
  this._requestBodyLength + e.length <= this._options.maxBodyLength ? (this._requestBodyLength += e.length, this._requestBodyBuffers.push({ data: e, encoding: t }), this._currentRequest.write(e, t, n)) : (this.emit("error", new OO()), this.abort());
};
Ve.prototype.end = function(e, t, n) {
...
Ve.prototype.setHeader = function(e, t) {
  this._options.headers[e] = t, this._currentRequest.setHeader(e, t);
};
Ve.prototype.removeHeader = function(e) {
  delete this._options.headers[e], this._currentRequest.removeHeader(e);
};
Ve.prototype.setTimeout = function(e, t) {
  var n = this;
  function r(s) {
...
Ve.prototype._sanitizeOptions = function(e) {
  if (e.headers || (e.headers = {}), IO(e.sensitiveHeaders) || (e.sensitiveHeaders = []), e.host && (e.hostname || (e.hostname = e.host), delete e.host), !e.pathname && e.path) {
...
Ve.prototype._performRequest = function() {
  ...
  for (var a of xc)
    r.on(a, bc[a]);
  if (this._currentUrl = /^\//.test(this._options.path) ? Nr.format(this._options) : (
...
Ve.prototype._processResponse = function(e) {
  ...
  if (wc(this._currentRequest), e.destroy(), ++this._redirectCount > this._options.maxRedirects)
    throw new RO();
  ...
  t === 303 && !/^(?:GET|HEAD)$/.test(this._options.method)) && (this._options.method = "GET", this._requestBodyBuffers = [], us(/^content-/i, this._options.headers));
  var s = us(/^host$/i, this._options.headers), o = _c(this._currentUrl), l = s || o.host, u = /^\w+:/.test(n) ? this._currentUrl : Nr.format(Object.assign(o, { host: l })), c = kO(n, u);
  if (Kf("redirecting to", c.href), this._isRedirect = !0, Ds(c, this._options), (c.protocol !== o.protocol && c.protocol !== "https:" || c.host !== l && !NO(c.host, l)) && us(this._headerFilter, this._options.headers), ln(a)) {
    var p = {
      headers: e.headers,
      statusCode: t
    }, f = {
...
function Xf(e) {
  ...
  return Object.keys(e).forEach(function(r) {
    ...
    function o(u, c, p) {
      return CO(u) ? u = Ds(u) : tn(u) ? u = Ds(_c(u)) : (p = c, c = Yf(u), u = { protocol: a }), ln(c) && (p = c, c = null), c = Object.assign({
        maxRedirects: t.maxRedirects,
        maxBodyLength: t.maxBodyLength
      }, u, c), c.nativeProtocols = n, !tn(c.host) && !tn(c.hostname) && (c.hostname = "::1"), yc.equal(c.protocol, a, "protocol mismatch"), Kf("options", c), new Ve(c, p);
    }
    function l(u, c, p) {
...
function _c(e) {
  ...
  if (gc)
    t = new gr(e);
  else if (t = Yf(Nr.parse(e)), !tn(t.protocol))
    throw new Cs({ input: e });
  return t;
}
function kO(e, t) {
  return gc ? new gr(e, t) : _c(Nr.resolve(t, e));
}
function Yf(e) {
  if (/^\[/.test(e.hostname) && !/^\[[:0-9a-f]+\]$/i.test(e.hostname))
    throw new Cs({ input: e.href || e });
...
function wc(e, t) {
  for (var n of xc)
    e.removeListener(n, bc[n]);
  e.on("error", Wf), e.destroy(t);
}
function NO(e, t) {
  yc(tn(e) && tn(t));
...
hc.exports = Xf({ http: EO, https: $O });
hc.exports.wrap = Xf;
var DO = hc.exports;
const FO = /* @__PURE__ */ Un(DO), xr = "1.16.1";
function Jf(e) {
...
const UO = /^([^,;]+\/[^,;]+)?((?:;[^,;=]+=[^,;]+)*)(;base64)?,([\s\S]*)$/;
function MO(e, t, n) {
  const r = n && n.Blob || ye.classes.Blob, a = Jf(e);
...
const ps = Symbol("internals");
class Hu extends Re.Transform {
  constructor(t) {
    t = A.toFlatObject(
      t,
      {
  ...
  _transform(t, n, r) {
    const a = this[ps], i = a.maxRate, s = this.readableHighWaterMark, o = a.timeWindow, l = 1e3 / o, u = i / l, c = a.minChunkSize !== !1 ? Math.max(a.minChunkSize, u * 0.01) : 0, p = (h, g) => {
...
const { asyncIterator: Gu } = Symbol, Qf = async function* (e) {
  e.stream ? yield* e.stream() : e.arrayBuffer ? yield await e.arrayBuffer() : e[Gu] ? yield* e[Gu]() : yield e;
}, zO = ye.ALPHABET.ALPHA_DIGIT + "-_", br = typeof TextEncoder == "function" ? new TextEncoder() : new pn.TextEncoder(), Yt = `\r
`, qO = br.encode(Yt), BO = 2;
class VO {
  constructor(t, n) {
    const { escapeName: r } = this.constructor, a = A.isString(n);
    let i = `Content-Disposition: form-data; name="${r(t)}"${!a && n.name ? `; filename="${r(n.name)}"` : ""}${Yt}`;
    if (a)
      n = br.encode(String(n).replace(/\r?\n|\r\n?/g, Yt));
    else {
      const s = String(n.type || "application/octet-stream").replace(/[\r\n]/g, "");
      i += `Content-Type: ${s}${Yt}`;
    }
    this.headers = br.encode(i + Yt), this.contentLength = a ? n.byteLength : n.size, this.size = this.headers.byteLength + this.contentLength + BO, this.name = t, this.value = n;
  }
  async *encode() {
    yield this.headers;
    const { value: t } = this;
    A.isTypedArray(t) ? yield t : yield* Qf(t), yield qO;
  }
  static escapeName(t) {
...
const HO = (e, t, n) => {
  const {
    ...
    size: a = 25,
    boundary: i = r + "-" + ye.generateString(a, zO)
  } = n || {};
  if (!A.isFormData(e))
    throw TypeError("FormData instance required");
  if (i.length < 1 || i.length > 70)
    throw Error("boundary must be 1-70 characters long");
  const s = br.encode("--" + i + Yt), o = br.encode("--" + i + "--" + Yt);
  let l = o.byteLength;
  ...
  return Number.isFinite(l) && (c["Content-Length"] = l), t && t(c), $m.from(
    async function* () {
      for (const p of u)
...
const KO = (e, t) => A.isAsyncFn(e) ? function(...n) {
  const r = n.pop();
...
} : e, XO = /* @__PURE__ */ new Set(["localhost"]), Zf = (e) => {
  const t = e.split(".");
  ...
  return !1;
}, Ku = (e) => e ? XO.has(e) || Zf(e) ? !0 : WO(e) : !1, YO = {
  http: 80,
  https: 443,
  ws: 80,
  wss: 443,
...
}, QO = /^(?:::|(?:0{1,4}:){1,4}:|(?:0{1,4}:){5})ffff:(\d+\.\d+\.\d+\.\d+)$/i, ZO = /^(?:::|(?:0{1,4}:){1,4}:|(?:0{1,4}:){5})ffff:([0-9a-f]{1,4}):([0-9a-f]{1,4})$/i, eA = (e) => {
  ...
  return e;
}, Xu = (e) => e && (e.charAt(0) === "[" && e.charAt(e.length - 1) === "]" && (e = e.slice(1, -1)), eA(e.replace(/\.+$/, "")));
function tA(e) {
  let t;
  try {
    t = new URL(e);
  ...
  if (n === "*")
    return !0;
  const r = Number.parseInt(t.port, 10) || YO[t.protocol.split(":", 1)[0]] || 0, a = Xu(t.hostname.toLowerCase());
  return n.split(/[\s,]+/).some((i) => {
    if (!i)
      return !1;
    let [s, o] = JO(i);
    return s = Xu(s), !s || o && o !== r ? !1 : (s.charAt(0) === "*" && (s = s.slice(1)), s.charAt(0) === "." ? a.endsWith(s) : a === s || Ku(a) && Ku(s));
  });
}
function nA(e, t) {
  e = e || 10;
...
}, Yu = A.isFunction(Dt.createBrotliDecompress), { http: iA, https: sA } = FO, tm = /https:?/, oA = ["content-type", "content-length"];
function cA(e, t, n) {
  if (n !== "content-only") {
    e.set(t);
    return;
  ...
  Object.entries(t).forEach(([r, a]) => {
    oA.includes(r.toLowerCase()) && e.set(r, a);
  });
}
const Ju = Symbol("axios.http.socketListener"), aa = Symbol("axios.http.currentReq"), nm = Symbol("axios.http.installedTunnel"), lA = /* @__PURE__ */ new Map(), Qu = /* @__PURE__ */ new WeakMap();
function uA(e, t) {
  const n = e.protocol + "//" + e.hostname + ":" + (e.port || "") + "#" + (e.auth || ""), r = t ? Qu.get(t) || Qu.set(t, /* @__PURE__ */ new Map()).get(t) : lA;
  let a = r.get(n);
  if (a) return a;
  const i = t && t.options ? { ...t.options, ...e } : e;
  return a = new Gf(i), a[nm] = !0, r.set(n, a), a;
}
const Zu = ye.protocols.map((e) => e + ":"), ep = (e) => {
  if (!A.isString(e))
    return e;
  try {
...
class pA {
  ...
  getSession(t, n) {
    ...
    if (r) {
      ...
      for (let p = 0; p < c; p++) {
        const [f, h] = r[p];
        if (!f.destroyed && !f.closed && pn.isDeepStrictEqual(h, n))
          return f;
      }
    }
    const a = yp.connect(t, n);
    let i;
    const s = () => {
      if (i)
        return;
...
function rm(e, t, n, r, a) {
  ...
  if (r && e.agent && e.agent[nm] && (e.agent = void 0), i) {
...
const mA = typeof process < "u" && A.kindOf(process) === "process", hA = (e) => new Promise((t, n) => {
  let r, a;
  const i = (l, u) => {
    a || (a = !0, r && r(l, u));
  }, s = (l) => {
...
}, np = (e, t) => vA(A.isObject(e) ? e : { address: e, family: t }), yA = {
  request(e, t) {
    const n = e.protocol + "//" + e.hostname + ":" + (e.port || (e.protocol === "https:" ? 443 : 80)), { http2Options: r, headers: a } = e, i = dA.getSession(n, r), { HTTP2_HEADER_SCHEME: s, HTTP2_HEADER_METHOD: o, HTTP2_HEADER_PATH: l, HTTP2_HEADER_STATUS: u } = yp.constants, c = {
  ...
  return hA(async function(r, a, i) {
    ...
    const _ = c === 2;
    if (l) {
      const q = KO(l, (z) => A.isArray(z) ? z : [z]);
      l = (z, X, W) => {
        q(z, X, (J, ue, de) => {
          if (J)
            return W(J);
          const ie = A.isArray(ue) ? ue.map((He) => np(He)) : [np(ue, de)];
    ...
    x.once("abort", a);
    const D = () => {
      I(), t.cancelToken && t.cancelToken.unsubscribe(w), t.signal && t.signal.removeEventListener("abort", w), x.removeAllListeners();
    };
    (t.cancelToken || t.signal) && (t.cancelToken && t.cancelToken.subscribe(w), t.signal && (t.signal.aborted ? w() : t.signal.addEventListener("abort", w))), i((q, z) => {
      if (m = !0, I(), z) {
        v = !0, D();
        return;
      }
      const { data: X } = q;
      if (X instanceof Re.Readable || X instanceof Re.Duplex) {
        const W = Re.finished(X, () => {
          W(), D();
        });
      } else
        D();
    });
    const S = pc(t.baseURL, t.url, t.allowAbsoluteUrls), R = new URL(S, ye.hasBrowserEnv ? ye.origin : void 0), O = R.protocol || Zu[0];
    if (O === "data:") {
      if (t.maxContentLength > -1) {
        const z = String(t.url || S || "");
        if (em(z) > t.maxContentLength)
          return a(
            new G(
      ...
      return f === "text" ? (q = q.toString(h), (!h || h === "utf8") && (q = A.stripBOM(q))) : f === "stream" && (q = Re.Readable.from(q)), Rn(r, a, {
        data: q,
        status: 200,
        statusText: "OK",
        headers: new $e(),
    ...
    const k = $e.from(t.headers).normalize();
    k.set("User-Agent", "axios/" + xr, !1);
    ...
    if (A.isSpecCompliantForm(o)) {
      ...
      o = HO(
        ...
        {
          tag: `axios-${xr}-boundary`,
          boundary: q && q[1] || void 0
        }
      );
    } else if (A.isFormData(o) && A.isFunction(o.getHeaders) && o.getHeaders !== Object.prototype.getHeaders) {
      if (cA(k, o.getHeaders(), s("formDataHeaderPolicy")), !k.hasContentLength())
        try {
          const q = await pn.promisify(o.getLength).call(o);
          Number.isFinite(q) && q >= 0 && k.setContentLength(q);
        } catch {
        }
    } else if (A.isBlob(o) || A.isFile(o))
      o.size && k.setContentType(o.type || "application/octet-stream"), k.setContentLength(o.size || 0), o = Re.Readable.from(Qf(o));
    ...
    A.isArray(F) ? (H = F[0], j = F[1]) : H = j = F, o && (V || H) && (A.isStream(o) || (o = Re.Readable.from(o, { objectMode: !1 })), o = Re.pipeline(
      [
        o,
        new Hu({
    ...
    if (!B && R.username) {
      const q = ep(R.username), z = ep(R.password);
      B = q + ":" + z;
    }
    ...
    if (!A.isUndefined(l) && (N.lookup = l), t.socketPath) {
      ...
      if (t.allowedSocketPaths != null) {
        const q = Array.isArray(t.allowedSocketPaths) ? t.allowedSocketPaths : [t.allowedSocketPaths], z = Oc(t.socketPath);
        if (!q.some(
          (W) => typeof W == "string" && Oc(W) === z
        ))
          return a(
            new G(
      ...
      N.hostname = R.hostname.startsWith("[") ? R.hostname.slice(1, -1) : R.hostname, N.port = R.port, rm(
        N,
        t.proxy,
    ...
    let T, b = !1;
    const E = tm.test(N.protocol);
    if (N.agent == null && (N.agent = E ? t.httpsAgent : t.httpAgent), _)
      T = yA;
    else {
      const q = s("transport");
      if (q)
        ...
        z && (N.beforeRedirects.config = z), T = E ? sA : iA;
      }
    }
    t.maxBodyLength > -1 ? N.maxBodyLength = t.maxBodyLength : N.maxBodyLength = 1 / 0, N.insecureHTTPParser = !!s("insecureHTTPParser"), d = T.request(N, function(z) {
      ...
      if (t.decompress !== !1 && z.headers["content-encoding"])
        switch ((g === "HEAD" || z.statusCode === 204) && delete z.headers["content-encoding"], (z.headers["content-encoding"] || "").toLowerCase()) {
          ...
          case "compress":
          case "x-compress":
            X.push(Dt.createUnzip(Wu)), delete z.headers["content-encoding"];
            break;
          case "deflate":
            X.push(new GO()), X.push(Dt.createUnzip(Wu)), delete z.headers["content-encoding"];
            break;
          case "br":
            Yu && (X.push(Dt.createBrotliDecompress(aA)), delete z.headers["content-encoding"]);
        }
      J = X.length > 1 ? Re.pipeline(X, A.noop) : X[0];
      const de = {
        status: z.statusCode,
        statusText: z.statusMessage,
        headers: new $e(z.headers),
        config: t,
        request: ue
      };
      if (f === "stream") {
        if (t.maxContentLength > -1) {
          const ie = t.maxContentLength, He = J;
          async function* nt() {
            let fe = 0;
            for await (const We of He) {
              if (fe += We.length, fe > ie)
                throw new G(
        ...
        const ie = [];
        let He = 0;
        J.on("data", function(fe) {
          ie.push(fe), He += fe.length, t.maxContentLength > -1 && He > t.maxContentLength && (v = !0, J.destroy(), w(
            new G(
              "maxContentLength size of " + t.maxContentLength + " exceeded",
              G.ERR_BAD_RESPONSE,
              t,
          ...
          v || a(G.from(fe, null, t, ue, de));
        }), J.on("end", function() {
          try {
            let fe = ie.length === 1 ? ie[0] : Buffer.concat(ie);
            f !== "arraybuffer" && (fe = fe.toString(h), (!h || h === "utf8") && (fe = A.stripBOM(fe))), de.data = fe;
          } catch (fe) {
            return a(G.from(fe, null, t, de.request, de));
          }
          Rn(r, a, de);
        });
      }
    ...
    if (d.on("socket", function(z) {
      z.setKeepAlive(!0, 1e3 * 60), z[Ju] || (z.on("error", function(W) {
        const J = z[aa];
        J && !J.destroyed && J.destroy(W);
      }), z[Ju] = !0), z[aa] = d, M.add(z);
    ...
    if (A.isStream(o)) {
      ...
      o.on("end", () => {
        ...
        !q && !z && w(new cn("Request stream has been aborted", t, d));
      });
      let X = o;
...
}, xA = ye.hasStandardBrowserEnv ? /* @__PURE__ */ ((e, t) => (n) => (n = new URL(n, ye.origin), e.protocol === n.protocol && e.host === n.host && (t || e.port === n.port)))(
  new URL(ye.origin),
  ye.navigator && /(msie|trident)/i.test(ye.navigator.userAgent)
) : () => !0, bA = ye.hasStandardBrowserEnv ? (
...
const _A = ["content-type", "content-length"];
function wA(e, t, n) {
  if (n !== "content-only") {
    e.set(t);
    return;
  ...
  Object.entries(t).forEach(([r, a]) => {
    _A.includes(r.toLowerCase()) && e.set(r, a);
  });
}
const EA = (e) => encodeURIComponent(e).replace(
  ...
  const t = un({}, e), n = (f) => A.hasOwnProp(t, f) ? t[f] : void 0, r = n("data");
  let a = n("withXSRFToken");
  const i = n("xsrfHeaderName"), s = n("xsrfCookieName");
  let o = n("headers");
  const l = n("auth"), u = n("baseURL"), c = n("allowAbsoluteUrls"), p = n("url");
  if (t.headers = o = $e.from(o), t.url = lc(
    pc(u, p, c),
    e.params,
    e.paramsSerializer
  ), l && o.set(
    "Authorization",
    "Basic " + btoa((l.username || "") + ":" + (l.password ? EA(l.password) : ""))
  ), A.isFormData(r) && (ye.hasStandardBrowserEnv || ye.hasStandardBrowserWebWorkerEnv ? o.setContentType(void 0) : A.isFunction(r.getHeaders) && wA(o, r.getHeaders(), n("formDataHeaderPolicy"))), ye.hasStandardBrowserEnv && (A.isFunction(a) && (a = a(t)), a === !0 || a == null && xA(t.url))) {
    const h = i && s && bA.read(s);
    h && o.set(i, h);
  }
  return t;
}, $A = typeof XMLHttpRequest < "u", SA = $A && function(e) {
  return new Promise(function(n, r) {
    const a = am(e);
    let i = a.data;
    const s = $e.from(a.headers).normalize();
    ...
    "onloadend" in v ? v.onloadend = d : v.onreadystatechange = function() {
      ...
      const w = x && x.message ? x.message : "Network Error", I = new G(w, G.ERR_NETWORK, e, v);
      I.event = x || null, r(I), m(), v = null;
    }, v.ontimeout = function() {
      let x = a.timeout ? "timeout of " + a.timeout + "ms exceeded" : "timeout exceeded";
      const w = a.transitional || ei;
    ...
    }, i === void 0 && s.setContentType(null), "setRequestHeader" in v && A.forEach(ac(s), function(x, w) {
      v.setRequestHeader(w, x);
    }), A.isUndefined(a.withCredentials) || (v.withCredentials = !!a.withCredentials), o && o !== "json" && (v.responseType = a.responseType), u && ([f, g] = Fn(u, !0), v.addEventListener("progress", f)), l && v.upload && ([p, h] = Fn(l), v.upload.addEventListener("progress", p), v.upload.addEventListener("loadend", h)), (a.cancelToken || a.signal) && (c = (_) => {
      v && (r(!_ || _.type ? new cn(null, e, v) : _), v.abort(), m(), v = null);
  ...
  const a = function(l) {
    if (!r) {
      ...
      n.abort(
        u instanceof G ? u : new cn(u instanceof Error ? u.message : u)
      );
    }
  };
  let i = t && setTimeout(() => {
...
}, ip = 64 * 1024, { isFunction: ia } = A, sp = (e, ...t) => {
  try {
    return !!e(...t);
  } catch {
    return !1;
  ...
  const { fetch: a, Request: i, Response: s } = e, o = a ? ia(a) : typeof fetch == "function", l = ia(i), u = ia(s);
  if (!o)
    return !1;
  const c = o && ia(n), p = o && (typeof r == "function" ? /* @__PURE__ */ ((d) => (y) => d.encode(y))(new r()) : async (d) => new Uint8Array(await new i(d).arrayBuffer())), f = l && c && sp(() => {
    let d = !1;
  ...
  }), h = u && c && sp(() => A.isReadableStream(new s("").body)), g = {
    stream: h && ((d) => d.body)
  };
  ...
  return async (d) => {
    ...
    let N;
    try {
      if (H && typeof y == "string" && y.startsWith("data:") && em(y) > P)
        throw new G(
      ...
      O.set("User-Agent", "axios/" + xr, !1);
      const b = {
        ...V,
        signal: B,
        method: _.toUpperCase(),
        headers: ac(O.normalize()),
        body: x,
        duplex: "half",
        credentials: T ? k : void 0
      };
      L = l && new i(y, b);
      let E = await (l ? U(L, V) : U(y, b));
      if (H) {
        const z = A.toFiniteNumber(E.headers.get("content-length"));
        if (z != null && z > P)
          throw new G(
            "maxContentLength size of " + P + " exceeded",
            G.ERR_BAD_RESPONSE,
      ...
      if (h && E.body && (D || H || M && $)) {
        ...
        const X = A.toFiniteNumber(E.headers.get("content-length")), [W, J] = D && Ta(
          X,
          Fn(Ra(D), !0)
        ) || [];
        let ue = 0;
...
}, kA = /* @__PURE__ */ new Map(), im = (e) => {
  let t = e && e.env || {};
  const { fetch: n, Request: r, Response: a } = t, i = [r, a, n];
  let s = i.length, o = s, l, u, c = kA;
  for (; o--; )
    l = i[o], u = c.get(l), u === void 0 && c.set(l, u = o ? /* @__PURE__ */ new Map() : PA(t)), c = u;
  return u;
};
im();
const Ec = {
...
const op = (e) => `- ${e}`, NA = (e) => A.isFunction(e) || e === null || e === !1;
function IA(e, t) {
  e = A.isArray(e) ? e : [e];
  const { length: n } = e;
  let r, a;
  ...
  for (let s = 0; s < n; s++) {
    r = e[s];
    let o;
    if (a = r, !NA(r) && (a = Ec[(o = String(r)).toLowerCase()], a === void 0))
...
` + s.map(op).join(`
`) : " " + op(s[0]) : "as no adapter specified";
    throw new G(
...
function cp(e) {
  return ds(e), e.headers = $e.from(e.headers), e.data = as.call(e, e.transformRequest), ["post", "put", "patch"].indexOf(e.method) !== -1 && e.headers.setContentType("application/x-www-form-urlencoded", !1), sm.getAdapter(e.adapter || kr.adapter, e)(e).then(
    function(r) {
      ds(e), e.response = r;
      try {
      ...
      return r.headers = $e.from(r.headers), r;
    },
    function(r) {
      if (!Bf(r) && (ds(e), r && r.response)) {
        e.response = r.response;
        try {
          r.response.data = as.call(
            e,
            e.transformResponse,
            r.response
          );
...
ni.transitional = function(t, n, r) {
  function a(i, s) {
    return "[Axios v" + xr + "] Transitional option '" + i + "'" + s + (r ? ". " + r : "");
  }
  return (i, s, o) => {
    if (t === !1)
      throw new G(
    ...
    return n && !lp[s] && (lp[s] = !0, console.warn(
      a(
        s,
...
` + i);
  ...
  _request(t, n) {
    ...
    r !== void 0 && va.assertOptions(
      ...
      !0
    )), n.allowAbsoluteUrls !== void 0 || (this.defaults.allowAbsoluteUrls !== void 0 ? n.allowAbsoluteUrls = this.defaults.allowAbsoluteUrls : n.allowAbsoluteUrls = !0), va.assertOptions(
      n,
      {
  ...
  getUri(t) {
    ...
    const n = pc(t.baseURL, t.url, t.allowAbsoluteUrls);
...
A.forEach(["post", "put", "patch", "query"], function(t) {
  ...
  nn.prototype[t] = n(), t !== "query" && (nn.prototype[t + "Form"] = n(!0));
});
let CA = class om {
  constructor(t) {
    if (typeof t != "function")
    ...
    this.promise.then((a) => {
      ...
      r.reason || (r.reason = new cn(i, s, o), n(r.reason));
    });
  }
  /**
...
function cm(e) {
  const t = new nn(e), n = uf(nn.prototype.request, t);
  return A.extend(n, nn.prototype, t, { allOwnKeys: !0 }), A.extend(n, t, null, { allOwnKeys: !0 }), n.create = function(a) {
    return cm(un(e, a));
  }, n;
}
const xe = cm(kr);
...
xe.mergeConfig = un;
xe.AxiosHeaders = $e;
xe.formToJSON = (e) => qf(A.isHTMLForm(e) ? new FormData(e) : e);
xe.getAdapter = sm.getAdapter;
xe.HttpStatusCode = Fs;
xe.default = xe;
const {
  Axios: UP,
  AxiosError: MP,
...
function FA() {
  le.handle("user:get-logged-user", async () => {
    try {
      return (await pp(
...
async function qA(e) {
  if (!e.general.takeCamshots)
    return null;
  const t = Z.join(process.cwd(), "camshots");
  await kc.mkdir(t, { recursive: !0 });
  const n = Date.now(), r = Z.join(t, `${n}.jpg`), a = e.trackingSources.cameraId, i = await BA(a);
  if (!i)
    return null;
  await kc.writeFile(r, i);
  const s = await sf(r, e);
  return {
...
class GA {
  constructor() {
    ce(this, "timer", null);
    ce(this, "idleStartedAt", "");
    ce(this, "isIdle", !1);
  }
  start(t, n, r) {
    const a = n.general.idleTimeoutMinutes;
    a && (this.timer = setInterval(() => {
      const i = xm.getSystemIdleTime(), s = a * 60, o = (/* @__PURE__ */ new Date()).toISOString();
      if (!this.isIdle && i >= s) {
        this.isIdle = !0, this.idleStartedAt = o, Mm({ sessionId: t, startTime: o }), In({
          sessionId: t,
          type: "idle_started",
          timestamp: o
        }), fs(t, "idle"), r.onIdleStarted();
        return;
      }
      if (this.isIdle && i < s) {
        const l = this.idleStartedAt;
        this.isIdle = !1, this.idleStartedAt = "", Ic({ sessionId: t, startTime: l, endTime: o }), In({
          sessionId: t,
          type: "idle_ended",
          timestamp: o
        }), fs(t, "active"), r.onIdleEnded();
      }
    }, HA));
  }
  stop(t) {
    this.timer && (clearInterval(this.timer), this.timer = null), this.isIdle && this.idleStartedAt && Ic({
      sessionId: t,
...
const KA = 6e4, XA = Rm(import.meta.url);
class WA {
  constructor() {
    ce(this, "keyboardCount", 0);
    ce(this, "mouseClickCount", 0);
    ce(this, "flushTimer", null);
    ce(this, "isPaused", !1);
    ce(this, "sessionId", "");
    ce(this, "hook", null);
    ce(this, "keyListener", () => {
  ...
  async start(t, n) {
    if (this.sessionId = t, this.isPaused = !1, this.keyboardCount = 0, this.mouseClickCount = 0, !(!n.trackingSources.countKeyboardHits && !n.trackingSources.countMouseClicks)) {
      if (process.platform === "darwin" && !Rc.isTrustedAccessibilityClient(!1)) {
        Rc.isTrustedAccessibilityClient(!0), console.warn(
          "Keyboard/mouse activity tracking is paused until Accessibility permission is granted for this app."
        );
        return;
      }
      try {
        const r = XA("uiohook-napi");
        if (this.hook = r.uIOhook || null, !this.hook)
          return;
        n.trackingSources.countKeyboardHits && this.hook.on("keydown", this.keyListener), n.trackingSources.countMouseClicks && this.hook.on("mousedown", this.mouseListener), this.hook.start(), this.flushTimer = setInterval(() => this.flush(), KA);
      } catch (r) {
...
class QA {
  constructor() {
    ce(this, "activeSession", null);
    ce(this, "uploadQueue", new JA());
    ce(this, "screenshotService", new YA(this.uploadQueue));
    ce(this, "camshotService", new zA(this.uploadQueue));
    ce(this, "inputTrackingService", new WA());
    ce(this, "idleDetectionService", new GA());
    ce(this, "autosaveTimer", null);
  }
  async start(t) {
  ...
  async stop() {
    ...
    this.screenshotService.stop(), this.camshotService.stop(), this.inputTrackingService.stop(), this.idleDetectionService.stop(t.id), this.autosaveTimer && (clearInterval(this.autosaveTimer), this.autosaveTimer = null), In({
      sessionId: t.id,
...
function ZA() {
  le.handle(
    "tracker:start",
    async (e, t) => Oa.start(t)
  ), le.handle("tracker:stop", async () => Oa.stop());
}
function eP(e) {
  wS(), ES(), $S(e), FA(), UA(), MA(), ZA();
}
const um = Z.dirname(bm(import.meta.url));
process.env.APP_ROOT = Z.join(um, "..");
const pm = process.env.VITE_DEV_SERVER_URL, nk = Z.join(process.env.APP_ROOT, "dist-electron"), tP = Z.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = pm ? Z.join(process.env.APP_ROOT, "public") : tP;
const Us = {
  dirname: um,
  isDev: process.env.NODE_ENV === "development",
  viteDevServerUrl: pm
...
ze.on("before-quit", () => {
  Oa.getActiveSession() && Oa.stop().catch((e) => {
