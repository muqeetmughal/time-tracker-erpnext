...
import electron, { BrowserWindow, app as app$1, nativeImage, Tray, Menu, ipcMain as ipcMain$1 } from "electron";
...
import require$$1$2, { resolve as resolve$3 } from "path";
...
const url_1 = __importDefault$1(require$$5$1);
const assert_1 = __importDefault$1(require$$3$3);
  ...
  ipcMain$1.handle("tracker:stop", async () => {
    try {
      ...
      const durationSeconds = Math.max(
        0,
        Math.floor(
          (new Date(stoppedAt).getTime() - new Date(activeSession.startedAt).getTime()) / 1e3
        )
      );
