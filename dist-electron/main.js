...
import electron, { BrowserWindow, app as app$1, nativeImage, Tray, Menu, screen, ipcMain as ipcMain$1, powerMonitor, systemPreferences } from "electron";
import { fileURLToPath } from "node:url";
import path$1 from "node:path";
import fs$2 from "node:fs/promises";
import Database from "better-sqlite3";
import require$$0$3 from "child_process";
import require$$1$1, { resolve as resolve$3 } from "path";
import require$$0$2 from "fs";
import require$$2$1 from "constants";
import require$$3$3 from "os";
import require$$5$1 from "assert";
import require$$3$2, { EventEmitter } from "events";
...
function createWindow(options) {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      preload: path$1.join(options.dirname, "preload.cjs"),
  ...
  if (options.isDev && options.viteDevServerUrl) {
    ...
    mainWindow.loadFile(path$1.join(options.dirname, "../renderer/index.html"));
  }
...
function createTray(dirname) {
  const icon = nativeImage.createFromPath(
    path$1.join(dirname, "../../resources/icon.png")
  );
  tray = new Tray(icon);
...
function showOrCreateWindow(options) {
...
function openConfigurationWindow(options) {
  ...
  if (options.isDev && options.viteDevServerUrl) {
    const url2 = new URL(options.viteDevServerUrl);
    url2.searchParams.set("window", "config");
    configWindow.loadURL(url2.toString());
  } else {
    configWindow.loadFile(path$1.join(options.dirname, "../renderer/index.html"), {
      query: {
        window: "config"
      }
    });
...
function addColumnIfMissing(database, table, definition) {
  try {
    ...
    if (!(error2 instanceof Error) || !error2.message.toLowerCase().includes("duplicate column")) {
      throw error2;
    }
  }
}
function columnSql(database, table, column, fallback) {
  const columns = database.prepare(`PRAGMA table_info(${table})`).all();
  return columns.some((item) => item.name === column) ? column : fallback;
}
function migrateActivityMediaIfNeeded(database) {
  const columns = database.prepare("PRAGMA table_info(activity_media)").all();
...
function toActivityMediaRecord(row) {
  return {
    id: row.id,
...
function toActivitySessionRecord(row) {
  return {
    id: row.id,
...
function toActivitySessionSummaryRecord(row) {
  return {
...
function getDb() {
  ...
  const db_path = path$1.join(app$1.getPath("userData"), "time-tracker.sqlite");
...
function createActivitySession(input) {
...
function updateActivitySessionStatus(sessionId, status) {
  getDb().prepare(
    `
        ...
        WHERE id = ?
      `
  ).run(status, (/* @__PURE__ */ new Date()).toISOString(), sessionId);
}
function stopActivitySession(sessionId, endTime) {
  ...
  const duration = Math.max(
    0,
    Math.floor(
      (new Date(endTime).getTime() - new Date(session.startTime).getTime()) / 1e3
    )
  );
  getDb().prepare(
    `
        ...
        WHERE id = ?
      `
  ).run(endTime, duration, (/* @__PURE__ */ new Date()).toISOString(), sessionId);
  return {
    ...session,
    endTime,
    duration,
...
function getActivitySession(sessionId) {
...
function insertActivitySessionMedia(input) {
  const result = getDb().prepare(
    `
        INSERT INTO activity_media (
          activity_id,
...
function insertActivityTimeline(input) {
...
function insertInputActivity(input) {
  if (input.count <= 0) {
    return;
  }
  getDb().prepare(
...
function startIdlePeriod(input) {
  getDb().prepare(
    `
        ...
        VALUES (?, ?, ?)
      `
  ).run(input.sessionId, input.startTime, (/* @__PURE__ */ new Date()).toISOString());
}
function endIdlePeriod(input) {
  const duration = Math.max(
    0,
    Math.floor(
      (new Date(input.endTime).getTime() - new Date(input.startTime).getTime()) / 1e3
    )
  );
  getDb().prepare(
    `
...
function listRecentUnsyncedActivities(limit2 = 50) {
...
function listRecentActivitySessions(limit2 = 50) {
  const normalizedLimit = Math.min(Math.max(Math.floor(limit2 || 50), 1), 200);
  const rows = getDb().prepare(
    `
        SELECT
          sessions.id,
...
function listActivityMedia(filter2 = "screenshot", limit2 = 100) {
  const normalizedFilter = filter2 === "camshot" || filter2 === "all" ? filter2 : "screenshot";
  const normalizedLimit = Math.min(Math.max(Math.floor(limit2 || 100), 1), 500);
  const whereClause = normalizedFilter === "all" ? "" : "WHERE media.media_type = ?";
  const params = normalizedFilter === "all" ? [normalizedLimit] : [normalizedFilter, normalizedLimit];
  const rows = getDb().prepare(
    `
        SELECT
          media.id,
...
function requireLinux() {
  if (hasRequiredLinux) return linux$1;
  hasRequiredLinux = 1;
  const exec = require$$0$3.exec;
...
DP-2-3 disconnected (normal left inverted right x axis y axis)`;
  ...
  function maxBuffer(screens) {
    ...
    screens.forEach((screen2) => {
      total += screen2.height * screen2.width;
    });
  ...
  function linuxSnapshot(options = {}) {
    return new Promise((resolve2, reject) => {
      listDisplays().then((screens) => {
        const screen2 = screens.find(options.screen ? (screen3) => screen3.id === options.screen : (screen3) => screen3.primary || screen3.id === "default") || screens[0];
        ...
        switch (options.linuxLibrary) {
          ...
          default:
            cmd = "import";
            args = ["-silent", "-window", "root", "-crop", screen2.crop, "-screen", filetype + ":" + filename];
            break;
        }
        require$$0$3.execFile(cmd, args, execOptions, (err, stdout) => {
          if (err) {
            return reject(err);
          } else {
...
function requireBraceExpansion() {
  ...
  function expandTop(str, options) {
    ...
    return expand(escapeBraces(str), max2, true).map(unescapeBraces);
  }
  ...
  function expand(str, max2, isTop) {
    var expansions = [];
    var m = balanced("{", "}", str);
    ...
    if (isSequence) {
      ...
      if (n.length === 1) {
        n = expand(n[0], max2, false).map(embrace);
        if (n.length === 1) {
          var post = m.post.length ? expand(m.post, max2, false) : [""];
...
function requireMinimatch() {
  ...
  var SUBPARSE = {};
  function parse2(pattern2, isSub) {
    assertValidPattern(pattern2);
    var options = this.options;
    if (pattern2 === "**") {
...
function requireTemp() {
  ...
  (function(module, exports) {
    ...
    let promisify2 = function(callback) {
    ...
    const createWriteStream = (affixes) => {
      ...
      let stream2 = fs2.createWriteStream(path3, { flags: RDWR_EXCL, mode: 384 });
      deleteFileOnExit(path3);
      return stream2;
    };
    exports.dir = dir;
...
function requireDarwin() {
  ...
  function darwinSnapshot(options = {}) {
    const performScreenCapture = (displays) => new Promise((resolve2, reject) => {
      ...
      require$$0$3.execFile("screencapture", args, function(err, stdOut) {
        if (err) {
          return reject(err);
...
function requireWin32() {
  ...
  function windowsSnapshot(options = {}) {
    return new Promise((resolve2, reject) => {
      ...
      require$$0$3.execFile("cmd.exe", args, {
...
function getProperty(object, path2, value) {
  if (!isObject$2(object) || typeof path2 !== "string" && !Array.isArray(path2)) {
...
function hasProperty(object, path2) {
  ...
  for (const key of pathArray) {
    if (!isObject$2(object) || !(key in object)) {
      return false;
    }
    object = object[key];
  }
...
const { normalizeIPv6, removeDotSegments, recomposeAuthority, normalizePercentEncoding, normalizePathEncoding, escapePreservingEscapes, reescapeHostDelimiters, isIPv4, nonSimpleDomain } = utils$2;
...
const minuteIntervals = [1, 5, 10, 15, 30, 45, 60];
const optionalMinuteIntervals = [
  ...minuteIntervals,
  null
];
const screenshotReviewSeconds = [5, 10, 15, 30, 60];
const screenshotSources = ["primary", "all"];
const defaultConfig = {
  general: {
...
function mergeConfig$2(base, updates) {
  if (!updates) {
  ...
  for (const key of Object.keys(updates)) {
    ...
    if (isObject$1(merged[key]) && isObject$1(updateValue)) {
      merged[key] = mergeConfig$2(merged[key], updateValue);
    } else {
...
function applyConfigSideEffects(config) {
  if (app$1.isPackaged || config.other.openAtLogin) {
    try {
      app$1.setLoginItemSettings({
        openAtLogin: config.other.openAtLogin
      });
    } catch (error2) {
      console.warn("Unable to update login item settings:", error2);
    }
  }
...
function getConfig() {
  const config = mergeConfig$2(defaultConfig, store.get("config"));
  validateConfig(config);
  return config;
}
function saveConfig(partialConfig) {
  const config = mergeConfig$2(getConfig(), partialConfig);
  validateConfig(config);
...
async function reviewImageBeforeUpload(imagePath, config) {
  ...
  try {
    ...
    console.warn("Unable to load image preview for review:", error2);
    return true;
  }
  return new Promise((resolve2) => {
    const reviewId = `image-review-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    ...
    let settled = false;
    function cleanup() {
      ipcMain$1.removeListener("image-review:allow", handleAllow);
      ipcMain$1.removeListener("image-review:reject", handleReject);
    }
    function settle2(allowed) {
      if (settled) {
        return;
      }
      settled = true;
    ...
    function handleAllow(_, payload) {
      if (payload.reviewId === reviewId) {
        settle2(true);
      }
    }
    function handleReject(_, payload) {
      if (payload.reviewId === reviewId) {
        settle2(false);
      }
    }
    ipcMain$1.on("image-review:allow", handleAllow);
    ipcMain$1.on("image-review:reject", handleReject);
    reviewWindow.on("closed", () => {
      settle2(true);
    });
    setTimeout(() => {
      settle2(true);
    }, config.advanced.screenshotReviewSeconds * 1e3);
        ...
        <html>
          ...
          <body>
            <div class="wrap">
              ...
              <img src="${escapeHtml$1(imageDataUrl)}" />
              <div class="actions">
...
async function getImageDataUrl(imagePath) {
  const extension = path$1.extname(imagePath).toLowerCase();
  const mimeType = imageMimeTypes$1[extension] || "image/jpeg";
  const image = await fs$2.readFile(path$1.resolve(imagePath));
  return `data:${mimeType};base64,${image.toString("base64")}`;
}
function ensureScreenshotDir() {
  const dir = require$$1$1.join(process.cwd(), "screenshots");
  if (!require$$0$2.existsSync(dir)) {
    require$$0$2.mkdirSync(dir, { recursive: true });
  }
  return dir;
}
async function reviewCapturedFile(filePath, config) {
...
async function captureScreenshotsDetailed(config = getConfig()) {
  ...
  if (config.trackingSources.screenshotsFrom === "all") {
    ...
    return Promise.all(files.map((file2) => reviewCapturedFile(file2, config)));
  }
  const file = require$$1$1.join(
    dir,
    `${timestamp}.jpg`
  );
  await screenshot({
...
async function captureScreenshot() {
  ...
  return captures.filter((capture) => capture.approved).map((capture) => capture.filePath);
}
...
async function attachMediaPreviews(records) {
  return Promise.all(
    records.map(async (record) => {
      try {
        const extension = path$1.extname(record.filePath).toLowerCase();
        const mimeType = imageMimeTypes[extension] || "image/jpeg";
        const image = await fs$2.readFile(record.filePath);
        return {
          ...record,
...
async function createActivity(input) {
  const description2 = input.description.trim();
  if (!input.project) {
  ...
  const screenshotPaths = input.screenshotPaths === void 0 ? await captureScreenshot() : input.screenshotPaths;
...
function buildPromptHtml(promptId, input) {
  const title2 = escapeHtml(input.title);
...
function openActivityPrompt(input) {
  return new Promise((resolve2, reject) => {
    const promptId = `activity-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    ...
    let settled = false;
    function cleanup() {
      ipcMain$1.removeListener("activity-prompt:submit", handleSubmit);
      ipcMain$1.removeListener("activity-prompt:cancel", handleCancel);
    }
    ...
    function handleCancel(_, payload) {
    ...
    function handleSubmit(_, payload) {
      if (payload.promptId !== promptId || settled) {
        return;
      }
      settled = true;
    ...
    ipcMain$1.on("activity-prompt:submit", handleSubmit);
    ipcMain$1.on("activity-prompt:cancel", handleCancel);
...
function openActivityDescriptionPrompt(input) {
  return new Promise((resolve2) => {
    const promptId = `activity-description-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const promptWindow = new BrowserWindow({
      width: 420,
      height: 220,
      center: true,
    ...
    function handleSubmit(_, payload) {
      ...
      settled = true;
      cleanup();
      resolve2(payload.description.trim());
      closeWindow();
    }
    ipcMain$1.on("activity-prompt:submit", handleSubmit);
    ipcMain$1.on("activity-prompt:cancel", handleCancel);
...
function registerActivityHandlers() {
  ipcMain$1.handle("activities:create", async (_, input) => {
    return createActivity(input);
  });
  ipcMain$1.handle("activities:list-recent-unsynced", async (_, limit2) => {
    return listRecentUnsyncedActivities(limit2);
  });
  ipcMain$1.handle("activities:list-recent-sessions", async (_, limit2) => {
    return listRecentActivitySessions(limit2);
  });
  ipcMain$1.handle(
    "activities:list-media",
    async (_, filter2, limit2) => {
      return attachMediaPreviews(listActivityMedia(filter2, limit2));
    }
  );
  ipcMain$1.handle("activities:prompt", async (_, input) => {
    if (!input.project) {
      throw new Error("Please select a project first.");
    }
...
function registerAuthHandlers() {
  ipcMain$1.handle("auth:get", async () => {
    const credentials = store.get("credentials");
    if (!credentials) {
      return {
        loggedIn: false,
  ...
  ipcMain$1.handle("auth:login", async (_, credentials) => {
...
function registerConfigHandlers(options) {
  function broadcastConfig(config) {
    BrowserWindow.getAllWindows().forEach((window2) => {
      window2.webContents.send("config:updated", config);
    });
  }
  ipcMain$1.handle("config:open-window", async () => {
  ...
  ipcMain$1.handle("config:save", async (_, partialConfig) => {
    const config = saveConfig(partialConfig);
    broadcastConfig(config);
    return config;
  });
  ipcMain$1.handle("config:reset", async () => {
...
const URLSearchParams = require$$5$2.URLSearchParams;
...
const url_1 = __importDefault$1(require$$5$2);
const assert_1 = __importDefault$1(require$$5$1);
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve2, reject, onDone) {
    ...
    if (config.socketPath) {
      ...
      if (config.allowedSocketPaths != null) {
        ...
        const resolvedSocket = resolve$3(config.socketPath);
        const isAllowed = allowed.some(
          (entry) => typeof entry === "string" && resolve$3(entry) === resolvedSocket
        );
        if (!isAllowed) {
          return reject(
            new AxiosError$1(
    ...
    if (isHttp2) {
      ...
      if (configTransport) {
        ...
        transport = isHttpsRequest ? require$$4$1 : require$$3$4;
...
function getApiErrorMessage(error2) {
  var _a;
  if (axios.isAxiosError(error2)) {
    const data = (_a = error2.response) == null ? void 0 : _a.data;
    return (data == null ? void 0 : data.message) || (data == null ? void 0 : data.exception) || (data == null ? void 0 : data.exc_type) || error2.message || "ERPNext API request failed.";
  }
...
async function frappeGet(endpoint, config = {}) {
  const credentials = getCredentials();
  const response = await axios.get(`${credentials.siteUrl}${endpoint}`, {
    ...config,
    headers: {
      ...config.headers,
...
function registerFrappeHandlers() {
  ipcMain$1.handle("user:get-logged-user", async () => {
    try {
      const response = await frappeGet(
        "/api/method/frappe.auth.get_logged_user"
      );
      return response.message;
    } catch (error2) {
      const message = getApiErrorMessage(error2);
      console.error("Failed to fetch logged user:", message);
      throw new Error(message);
    }
  });
  ipcMain$1.handle("projects:get", async () => {
    try {
      const response = await frappeGet(
...
function registerSettingsHandlers() {
  ipcMain$1.handle("settings:get", async () => {
...
function registerTaskHandlers() {
  ipcMain$1.handle("tasks:get", async (_, _project) => {
    try {
      return [
        {
          name: "TASK-0001",
...
function getNextCaptureDelay(config) {
  const intervalMs = config.general.trackingIntervalMinutes * 60 * 1e3;
  if (!config.advanced.randomizedTracking) {
    return intervalMs;
  }
  return Math.max(6e4, Math.floor(Math.random() * intervalMs));
}
class CamshotService {
  ...
  start(sessionId, getConfigSnapshot) {
    this.isRunning = true;
    this.isPaused = false;
    this.schedule(sessionId, getConfigSnapshot, 1500);
  }
  pause() {
    this.isPaused = true;
  }
  ...
  schedule(sessionId, getConfigSnapshot, delayMs = getNextCaptureDelay(getConfigSnapshot())) {
    if (!this.isRunning) {
      return;
    }
  ...
  async capture(sessionId, config) {
    try {
      ...
      console.error("Camshot capture failed:", error2);
    }
  }
}
async function captureCamshot(config) {
  if (!config.general.takeCamshots) {
    return null;
  }
  const dir = path$1.join(process.cwd(), "camshots");
  await fs$2.mkdir(dir, { recursive: true });
  const timestampMs = Date.now();
  const filePath = path$1.join(dir, `${timestampMs}.jpg`);
  const cameraId = config.trackingSources.cameraId;
  const imageData = await captureCameraImage(cameraId);
  if (!imageData) {
    return null;
  }
  await fs$2.writeFile(filePath, imageData);
  const approved = await reviewImageBeforeUpload(filePath, config);
  return {
...
function captureCameraImage(cameraId) {
  return new Promise((resolve2) => {
    ...
    window2.webContents.once("ipc-message", (_event, channel, payload) => {
      ...
      resolve2(Buffer.from(payload.replace("data:image/jpeg;base64,", ""), "base64"));
      window2.close();
    });
    window2.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
  });
}
function buildCameraCaptureHtml(cameraId) {
  return `
    <!doctype html>
    <html>
      <body>
...
const CHECK_INTERVAL_MS = 15e3;
class IdleDetectionService {
  constructor() {
    __publicField(this, "timer", null);
    __publicField(this, "idleStartedAt", "");
    __publicField(this, "isIdle", false);
  }
  start(sessionId, config, callbacks) {
...
class InputTrackingService {
  constructor() {
    __publicField(this, "keyboardCount", 0);
    __publicField(this, "mouseClickCount", 0);
    __publicField(this, "flushTimer", null);
    __publicField(this, "isPaused", false);
    __publicField(this, "sessionId", "");
    __publicField(this, "hook", null);
    __publicField(this, "keyListener", () => {
      if (!this.isPaused) {
        this.keyboardCount += 1;
      }
    });
    __publicField(this, "mouseListener", () => {
      if (!this.isPaused) {
  ...
  async start(sessionId, config) {
    ...
    try {
      const module = require$1("uiohook-napi");
      ...
      console.warn("Input tracking is unavailable:", error2);
    }
  }
  pause() {
    this.flush();
  ...
  flushKind(kind, count, timestamp) {
    if (count <= 0) {
      return;
    }
    insertInputActivity({
...
class ScreenshotService {
  ...
  start(sessionId, getConfigSnapshot) {
    this.isRunning = true;
    this.isPaused = false;
    this.schedule(sessionId, getConfigSnapshot, 1e3);
  }
  pause() {
    this.isPaused = true;
  }
  ...
  async capture(sessionId, config) {
    try {
      ...
      console.error("Screenshot capture failed:", error2);
    }
  }
}
    ...
    while (this.queue.length > 0) {
      ...
      try {
        ...
        if (item.attempts < 3) {
          ...
          console.error("Media upload failed:", error2);
        }
      }
    }
...
class ActivitySessionService {
  constructor() {
    __publicField(this, "activeSession", null);
    __publicField(this, "uploadQueue", new UploadQueueService());
    __publicField(this, "screenshotService", new ScreenshotService(this.uploadQueue));
    __publicField(this, "camshotService", new CamshotService(this.uploadQueue));
    __publicField(this, "inputTrackingService", new InputTrackingService());
    __publicField(this, "idleDetectionService", new IdleDetectionService());
...
function registerTrackerHandlers() {
  ...
  ipcMain$1.handle("tracker:stop", async () => {
    return activitySessionService.stop();
  });
  ipcMain$1.handle("tracker:status", async () => {
...
app$1.on("before-quit", () => {
  if (activitySessionService.getActiveSession()) {
    void activitySessionService.stop().catch((error2) => {
      console.error("Failed to stop active tracking session before quit:", error2);
    });
  }
});
export {
